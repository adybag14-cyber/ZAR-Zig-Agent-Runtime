...
if ($null -eq $qemu -or $null -eq $gdb) {
    ...
    Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE=skipped"
    return
}
...
$optionsPath = Join-Path $releaseDir "qemu-timer-quantum-probe-options.zig"
$rootModulePath = (Join-Path $repo "src/baremetal_main.zig").Replace('\\', '/')
$optionsModulePath = $optionsPath.Replace('\\', '/')
$mainObj = Join-Path $releaseDir "openclaw-zig-baremetal-main-timer-quantum-probe.o"
$bootObj = Join-Path $releaseDir "openclaw-zig-pvh-boot-timer-quantum-probe.o"
$artifact = Join-Path $releaseDir "openclaw-zig-baremetal-pvh-timer-quantum-probe.elf"
$bootSource = Join-Path $repo "scripts\baremetal\pvh_boot.S"
$linkerScript = Join-Path $repo "scripts\baremetal\pvh_lld.ld"
$gdbScript = Join-Path $releaseDir "qemu-timer-quantum-probe.gdb"
$gdbStdout = Join-Path $releaseDir "qemu-timer-quantum-probe.gdb.stdout.log"
$gdbStderr = Join-Path $releaseDir "qemu-timer-quantum-probe.gdb.stderr.log"
$qemuStdout = Join-Path $releaseDir "qemu-timer-quantum-probe.qemu.stdout.log"
$qemuStderr = Join-Path $releaseDir "qemu-timer-quantum-probe.qemu.stderr.log"
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj `
        ...
        --name "openclaw-zig-baremetal-main-timer-quantum-probe" `
        "-femit-bin=$mainObj"
    if ($LASTEXITCODE -ne 0) {
        throw "zig build-obj for timer-quantum probe runtime failed with exit code $LASTEXITCODE"
    }

    & $clang -c -target x86_64-unknown-elf $bootSource -o $bootObj
    if ($LASTEXITCODE -ne 0) {
        throw "clang assemble for timer-quantum probe PVH shim failed with exit code $LASTEXITCODE"
    }

    & $lld -flavor gnu -m elf_x86_64 -o $artifact -T $linkerScript $mainObj $bootObj $compilerRt
    if ($LASTEXITCODE -ne 0) {
        throw "lld link for timer-quantum probe PVH artifact failed with exit code $LASTEXITCODE"
    }
}
...
set `$stage = 0
set `$armed_ticks = 0
set `$armed_next_fire_tick = 0
set `$expected_boundary_tick = 0
set `$pre_boundary_tick = 0
set `$pre_boundary_wake_count = 0
set `$pre_boundary_task_state = 0
set `$pre_boundary_dispatch_count = 0
...
if `$stage == 7
  if *(unsigned int*)(0x$statusAddress+$statusCommandSeqAckOffset) == 7 && *(unsigned int*)(0x$wakeQueueCountAddress) == 0 && *(unsigned char*)(0x$timerStateAddress+$timerEntryCountOffset) == 1 && *(unsigned char*)(0x$timerEntriesAddress+$timerEntryStateOffset) == 1
    set `$armed_ticks = *(unsigned long long*)(0x$statusAddress+$statusTicksOffset)
    set `$armed_next_fire_tick = *(unsigned long long*)(0x$timerEntriesAddress+$timerEntryNextFireTickOffset)
    set `$expected_boundary_tick = ((`$armed_ticks / $timerQuantum) + 1) * $timerQuantum
    set `$stage = 8
  end
  continue
end
if `$stage == 8
  if *(unsigned long long*)(0x$statusAddress+$statusTicksOffset) >= (`$expected_boundary_tick - 1)
    set `$pre_boundary_tick = *(unsigned long long*)(0x$statusAddress+$statusTicksOffset)
    set `$pre_boundary_wake_count = *(unsigned int*)(0x$wakeQueueCountAddress)
    set `$pre_boundary_task_state = *(unsigned char*)(0x$schedulerTasksAddress+$taskStateOffset)
    set `$pre_boundary_dispatch_count = *(unsigned long long*)(0x$timerStateAddress+$timerDispatchCountOffset)
    set `$stage = 9
  end
  continue
end
if `$stage == 9
  if *(unsigned int*)(0x$wakeQueueCountAddress) == 1 && *(unsigned char*)(0x$schedulerTasksAddress+$taskStateOffset) == 1 && *(unsigned char*)(0x$timerEntriesAddress+$timerEntryStateOffset) == 2
...
printf "ARMED_NEXT_FIRE_TICK=%llu\n", `$armed_next_fire_tick
printf "EXPECTED_BOUNDARY_TICK=%llu\n", `$expected_boundary_tick
printf "PRE_BOUNDARY_TICK=%llu\n", `$pre_boundary_tick
printf "PRE_BOUNDARY_WAKE_COUNT=%u\n", `$pre_boundary_wake_count
printf "PRE_BOUNDARY_TASK_STATE=%u\n", `$pre_boundary_task_state
printf "PRE_BOUNDARY_DISPATCH_COUNT=%llu\n", `$pre_boundary_dispatch_count
...
$armedTicks = $null
$armedNextFireTick = $null
$expectedBoundaryTick = $null
$preBoundaryTick = $null
$preBoundaryWakeCount = $null
$preBoundaryTaskState = $null
$preBoundaryDispatchCount = $null
$postWakeTick = $null
$timerEnabled = $null
...
if (Test-Path $gdbStdout) {
    ...
    $hitAfterTimerQuantum = $out -match "AFTER_TIMER_QUANTUM"
    ...
    $armedNextFireTick = Extract-IntValue -Text $out -Name "ARMED_NEXT_FIRE_TICK"
    $expectedBoundaryTick = Extract-IntValue -Text $out -Name "EXPECTED_BOUNDARY_TICK"
    $preBoundaryTick = Extract-IntValue -Text $out -Name "PRE_BOUNDARY_TICK"
    $preBoundaryWakeCount = Extract-IntValue -Text $out -Name "PRE_BOUNDARY_WAKE_COUNT"
    $preBoundaryTaskState = Extract-IntValue -Text $out -Name "PRE_BOUNDARY_TASK_STATE"
    $preBoundaryDispatchCount = Extract-IntValue -Text $out -Name "PRE_BOUNDARY_DISPATCH_COUNT"
    $postWakeTick = Extract-IntValue -Text $out -Name "POST_WAKE_TICK"
...
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_START_ADDR=0x$startAddress"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_SPINPAUSE_ADDR=0x$spinPauseAddress"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_STATUS_ADDR=0x$statusAddress"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_MAILBOX_ADDR=0x$commandMailboxAddress"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_SCHEDULER_STATE_ADDR=0x$schedulerStateAddress"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TASKS_ADDR=0x$schedulerTasksAddress"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TIMER_STATE_ADDR=0x$timerStateAddress"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TIMER_ENTRIES_ADDR=0x$timerEntriesAddress"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_WAKE_QUEUE_ADDR=0x$wakeQueueAddress"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_WAKE_QUEUE_COUNT_ADDR=0x$wakeQueueCountAddress"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_HIT_START=$hitStart"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_HIT_AFTER_TIMER_QUANTUM=$hitAfterTimerQuantum"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_ACK=$ack"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_LAST_OPCODE=$lastOpcode"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_LAST_RESULT=$lastResult"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TICKS=$ticks"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_MAILBOX_OPCODE=$mailboxOpcode"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_MAILBOX_SEQ=$mailboxSeq"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_SCHED_TASK_COUNT=$schedulerTaskCount"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TASK0_ID=$task0Id"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TASK0_STATE=$task0State"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TASK0_PRIORITY=$task0Priority"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TASK0_RUN_COUNT=$task0RunCount"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TASK0_BUDGET=$task0Budget"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TASK0_BUDGET_REMAINING=$task0BudgetRemaining"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_ARMED_TICKS=$armedTicks"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_ARMED_NEXT_FIRE_TICK=$armedNextFireTick"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_EXPECTED_BOUNDARY_TICK=$expectedBoundaryTick"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_PRE_BOUNDARY_TICK=$preBoundaryTick"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_PRE_BOUNDARY_WAKE_COUNT=$preBoundaryWakeCount"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_PRE_BOUNDARY_TASK_STATE=$preBoundaryTaskState"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_PRE_BOUNDARY_DISPATCH_COUNT=$preBoundaryDispatchCount"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_POST_WAKE_TICK=$postWakeTick"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TIMER_ENABLED=$timerEnabled"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TIMER_ENTRY_COUNT=$timerEntryCount"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_PENDING_WAKE_COUNT=$pendingWakeCount"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TIMER_DISPATCH_COUNT=$timerDispatchCount"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TIMER_LAST_WAKE_TICK=$timerLastWakeTick"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TIMER_QUANTUM=$timerQuantumOut"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_WAKE_QUEUE_COUNT=$wakeQueueCount"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TIMER0_ID=$timer0Id"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TIMER0_TASK_ID=$timer0TaskId"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TIMER0_STATE=$timer0State"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TIMER0_NEXT_FIRE_TICK=$timer0NextFireTick"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TIMER0_FIRE_COUNT=$timer0FireCount"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TIMER0_LAST_FIRE_TICK=$timer0LastFireTick"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_WAKE0_SEQ=$wake0Seq"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_WAKE0_TASK_ID=$wake0TaskId"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_WAKE0_TIMER_ID=$wake0TimerId"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_WAKE0_REASON=$wake0Reason"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_WAKE0_VECTOR=$wake0Vector"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_WAKE0_TICK=$wake0Tick"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_GDB_STDOUT=$gdbStdout"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_GDB_STDERR=$gdbStderr"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_QEMU_STDOUT=$qemuStdout"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_QEMU_STDERR=$qemuStderr"
Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE_TIMED_OUT=$timedOut"

$pass = (
    $hitStart -and
    $hitAfterTimerQuantum -and
    (-not $timedOut) -and
    $ack -eq 7 -and
    $lastOpcode -eq $timerScheduleOpcode -and
    $lastResult -eq 0 -and
    $ticks -ge ($postWakeTick + $postWakeSlackTicks) -and
    $mailboxOpcode -eq $timerScheduleOpcode -and
    $mailboxSeq -eq 7 -and
    ...
    $armedTicks -ge 1 -and
    $armedNextFireTick -eq $armedTicks -and
    $expectedBoundaryTick -eq (([int64]([math]::Floor($armedTicks / $timerQuantum)) + 1) * $timerQuantum) -and
    $expectedBoundaryTick -gt $armedNextFireTick -and
    $preBoundaryTick -eq ($expectedBoundaryTick - 1) -and
    $preBoundaryWakeCount -eq 0 -and
    $preBoundaryTaskState -eq 6 -and
    $preBoundaryDispatchCount -eq 0 -and
    $postWakeTick -eq ($expectedBoundaryTick + 1) -and
    $timerEnabled -eq 1 -and
    $timerEntryCount -eq 0 -and
    $pendingWakeCount -eq 1 -and
    $timerDispatchCount -eq 1 -and
    ...
    $timer0NextFireTick -eq $armedNextFireTick -and
    $timer0FireCount -eq 1 -and
    $timer0LastFireTick -eq $wake0Tick -and
    $wake0Seq -eq 1 -and
    $wake0TaskId -eq 1 -and
    $wake0TimerId -eq 1 -and
    $wake0Reason -eq 1 -and
    $wake0Vector -eq 0 -and
    $wake0Tick -eq $expectedBoundaryTick
)

if ($pass) {
    Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE=pass"
    exit 0
}

Write-Output "BAREMETAL_QEMU_TIMER_QUANTUM_PROBE=fail"
if (Test-Path $gdbStdout) {
