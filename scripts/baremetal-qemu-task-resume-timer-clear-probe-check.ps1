...
if ($null -eq $qemu -or $null -eq $gdb) {
    ...
    Write-Output "BAREMETAL_QEMU_TASK_RESUME_TIMER_CLEAR_PROBE=skipped"
    return
}
...
$optionsPath = Join-Path $releaseDir "qemu-task-resume-timer-clear-probe-options.zig"
$rootModulePath = (Join-Path $repo "src/baremetal_main.zig").Replace('\\', '/')
$optionsModulePath = $optionsPath.Replace('\\', '/')
$mainObj = Join-Path $releaseDir "openclaw-zig-baremetal-main-task-resume-timer-clear-probe.o"
$bootObj = Join-Path $releaseDir "openclaw-zig-pvh-boot-task-resume-timer-clear-probe.o"
$artifact = Join-Path $releaseDir "openclaw-zig-baremetal-pvh-task-resume-timer-clear-probe.elf"
$bootSource = Join-Path $repo "scripts\baremetal\pvh_boot.S"
$linkerScript = Join-Path $repo "scripts\baremetal\pvh_lld.ld"
$gdbScript = Join-Path $releaseDir "qemu-task-resume-timer-clear-probe.gdb"
$gdbStdout = Join-Path $releaseDir "qemu-task-resume-timer-clear-probe.gdb.stdout.log"
$gdbStderr = Join-Path $releaseDir "qemu-task-resume-timer-clear-probe.gdb.stderr.log"
$qemuStdout = Join-Path $releaseDir "qemu-task-resume-timer-clear-probe.qemu.stdout.log"
$qemuStderr = Join-Path $releaseDir "qemu-task-resume-timer-clear-probe.qemu.stderr.log"
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj `
        ...
        --name "openclaw-zig-baremetal-main-task-resume-timer-clear-probe" `
        "-femit-bin=$mainObj"
...
printf "AFTER_TASK_RESUME_TIMER_CLEAR\n"
...
if (Test-Path $gdbStdout) {
    ...
    $hitAfterProbe = $gdbOutput.Contains("AFTER_TASK_RESUME_TIMER_CLEAR")
...
Write-Output "BAREMETAL_QEMU_TASK_RESUME_TIMER_CLEAR_PROBE=pass"
