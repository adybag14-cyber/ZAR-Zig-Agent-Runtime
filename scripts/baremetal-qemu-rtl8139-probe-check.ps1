...
if ($null -eq $qemu) {
    ...
    Write-Output 'BAREMETAL_QEMU_RTL8139_PROBE=skipped'
    return
}
...
$optionsPath = Join-Path $releaseDir 'qemu-rtl8139-probe-options.zig'
$mainObj = Join-Path $releaseDir 'openclaw-zig-baremetal-main-rtl8139-probe.o'
$bootObj = Join-Path $releaseDir 'openclaw-zig-pvh-boot-rtl8139-probe.o'
$artifact = Join-Path $releaseDir 'openclaw-zig-baremetal-pvh-rtl8139-probe.elf'
$bootSource = Join-Path $repo 'scripts\baremetal\pvh_boot.S'
$linkerScript = Join-Path $repo 'scripts\baremetal\pvh_lld.ld'
$stdoutPath = Join-Path $releaseDir 'qemu-rtl8139-probe.stdout.log'
$stderrPath = Join-Path $releaseDir 'qemu-rtl8139-probe.stderr.log'
...
pub const rtl8139_probe: bool = true;
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj `
        ...
        --name 'openclaw-zig-baremetal-main-rtl8139-probe' `
        "-femit-bin=$mainObj"
    if ($LASTEXITCODE -ne 0) {
        throw "zig build-obj for RTL8139 probe failed with exit code $LASTEXITCODE"
    }
...
if (-not (Test-Path $artifact)) {
    throw "RTL8139 probe artifact is missing: $artifact"
}
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "QEMU RTL8139 probe timed out after $TimeoutSeconds seconds."
}
...
if ($exitCode -ne $expectedExitCode) {
    ...
    throw ("QEMU RTL8139 probe failed with exit code {0} (probe code 0x{1:X2})." -f $exitCode, $probeCode)
}

Write-Output 'BAREMETAL_QEMU_AVAILABLE=True'
Write-Output "BAREMETAL_QEMU_BINARY=$qemu"
Write-Output 'BAREMETAL_QEMU_RTL8139_PROBE=pass'
Write-Output ("BAREMETAL_QEMU_RTL8139_PROBE_CODE=0x{0:X2}" -f $expectedProbeCode)
Write-Output "BAREMETAL_QEMU_RTL8139_STDOUT=$stdoutPath"
Write-Output "BAREMETAL_QEMU_RTL8139_STDERR=$stderrPath"
