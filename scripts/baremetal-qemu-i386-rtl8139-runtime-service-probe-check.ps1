...
if ($null -eq $qemu) {
    ...
    Write-Output 'BAREMETAL_I386_QEMU_RTL8139_RUNTIME_SERVICE_PROBE=skipped'
    return
}

if (-not $SkipBuild) {
    & $zig build baremetal-i386 -Doptimize=Debug -Dbaremetal-rtl8139-runtime-service-probe=true --summary all
    if ($LASTEXITCODE -ne 0) { throw "zig build baremetal-i386 rtl8139 runtime-service probe failed with exit code $LASTEXITCODE" }
}
...
if ($null -eq $artifact) { throw 'i386 RTL8139 runtime-service artifact not found after build.' }

$diskImage = Join-Path $releaseDir 'qemu-i386-rtl8139-runtime-service-probe.img'
$stdoutPath = Join-Path $releaseDir 'qemu-i386-rtl8139-runtime-service-probe.stdout.log'
$stderrPath = Join-Path $releaseDir 'qemu-i386-rtl8139-runtime-service-probe.stderr.log'
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "QEMU i386 RTL8139 runtime-service probe timed out after $TimeoutSeconds seconds."
}
$proc.WaitForExit()
...
if ($exitCode -ne $expectedExitCode) {
    ...
    throw ("QEMU i386 RTL8139 runtime-service probe failed with exit code {0} (probe code 0x{1:X2})." -f $exitCode, $probeCode)
}

Write-Output 'BAREMETAL_I386_QEMU_AVAILABLE=True'
Write-Output "BAREMETAL_I386_QEMU_BINARY=$qemu"
Write-Output 'BAREMETAL_I386_QEMU_RTL8139_RUNTIME_SERVICE_PROBE=pass'
Write-Output ("BAREMETAL_I386_QEMU_RTL8139_RUNTIME_SERVICE_PROBE_CODE=0x{0:X2}" -f $expectedProbeCode)
Write-Output "BAREMETAL_I386_QEMU_RTL8139_RUNTIME_SERVICE_STDOUT=$stdoutPath"
Write-Output "BAREMETAL_I386_QEMU_RTL8139_RUNTIME_SERVICE_STDERR=$stderrPath"
