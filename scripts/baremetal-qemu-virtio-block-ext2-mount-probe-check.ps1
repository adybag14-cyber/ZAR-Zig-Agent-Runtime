...
$expectedProbeCode = 0x5B
$expectedExitCode = ($expectedProbeCode * 2) + 1
$expectedPayloadMarker = 'hello-from-ext2'
$ext2MagicOffset = 1024 + 56
...
$zigGlobalCacheDir = if ($env:ZIG_GLOBAL_CACHE_DIR -and $env:ZIG_GLOBAL_CACHE_DIR.Trim().Length -gt 0) { $env:ZIG_GLOBAL_CACHE_DIR } else { Join-Path $repo '.zig-global-cache-virtio-block-ext2-mount-probe' }
$zigLocalCacheDir = if ($env:ZIG_LOCAL_CACHE_DIR -and $env:ZIG_LOCAL_CACHE_DIR.Trim().Length -gt 0) { $env:ZIG_LOCAL_CACHE_DIR } else { Join-Path $repo '.zig-cache-virtio-block-ext2-mount-probe' }

if ($null -eq $qemu) {
    Write-Output 'BAREMETAL_QEMU_AVAILABLE=False'
    Write-Output 'BAREMETAL_QEMU_VIRTIO_BLOCK_EXT2_MOUNT_PROBE=skipped'
    return
}
...
$optionsPath = Join-Path $releaseDir 'qemu-virtio-block-ext2-mount-probe-options.zig'
$rootModulePath = (Join-Path $repo "src/baremetal_main.zig").Replace('\\', '/')
$optionsModulePath = $optionsPath.Replace('\\', '/')
$mainObj = Join-Path $releaseDir 'openclaw-zig-baremetal-main-virtio-block-ext2-mount-probe.o'
$bootObj = Join-Path $releaseDir 'openclaw-zig-pvh-boot-virtio-block-ext2-mount-probe.o'
$artifact = Join-Path $releaseDir 'openclaw-zig-baremetal-pvh-virtio-block-ext2-mount-probe.elf'
$diskImage = Join-Path $releaseDir 'qemu-virtio-block-ext2-mount-probe.img'
$bootSource = Join-Path $repo 'scripts\baremetal\pvh_boot.S'
$linkerScript = Join-Path $repo 'scripts\baremetal\pvh_lld.ld'
$stdoutPath = Join-Path $releaseDir 'qemu-virtio-block-ext2-mount-probe.stdout.log'
$stderrPath = Join-Path $releaseDir 'qemu-virtio-block-ext2-mount-probe.stderr.log'
...
pub const virtio_block_ext2_mount_probe: bool = true;
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj -fno-strip -fsingle-threaded -OReleaseSafe -target x86_64-freestanding-none -mcpu baseline --dep build_options "-Mroot=$rootModulePath" "-Mbuild_options=$optionsModulePath" --cache-dir "$zigLocalCacheDir" --global-cache-dir "$zigGlobalCacheDir" --name 'openclaw-zig-baremetal-main-virtio-block-ext2-mount-probe' "-femit-bin=$mainObj"
    if ($LASTEXITCODE -ne 0) { throw "zig build-obj for virtio-block ext2 mount probe failed with exit code $LASTEXITCODE" }

    & $clang -c -target x86_64-unknown-elf $bootSource -o $bootObj
    if ($LASTEXITCODE -ne 0) { throw "clang assemble for virtio-block ext2 mount PVH shim failed with exit code $LASTEXITCODE" }
    ...
    if ($LASTEXITCODE -ne 0) { throw "zig final link for virtio-block ext2 mount PVH artifact failed with exit code $LASTEXITCODE" }
}

if (-not (Test-Path $artifact)) {
    throw "Virtio-block ext2 mount probe artifact is missing: $artifact"
}
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "QEMU virtio-block ext2 mount probe timed out after $TimeoutSeconds seconds."
}
...
if ($proc.ExitCode -ne $expectedExitCode) {
    ...
    throw ("QEMU virtio-block ext2 mount probe failed with exit code {0} (probe code 0x{1:X2})." -f $proc.ExitCode, $probeCode)
}

$imageBytes = [System.IO.File]::ReadAllBytes($diskImage)
$ext2Magic = [System.BitConverter]::ToUInt16($imageBytes, $ext2MagicOffset)
$imageText = [System.Text.Encoding]::ASCII.GetString($imageBytes)
if ($ext2Magic -ne 0xEF53) {
    throw ("Virtio-block ext2 mount probe magic mismatch. Expected 0xEF53, got 0x{0:X4}." -f $ext2Magic)
}
if (-not $imageText.Contains($expectedPayloadMarker)) {
    throw "Virtio-block ext2 mount image does not contain expected payload marker '$expectedPayloadMarker'."
}

Write-Output 'BAREMETAL_QEMU_AVAILABLE=True'
Write-Output "BAREMETAL_QEMU_BINARY=$qemu"
Write-Output 'BAREMETAL_QEMU_VIRTIO_BLOCK_EXT2_MOUNT_PROBE=pass'
Write-Output "BAREMETAL_QEMU_VIRTIO_BLOCK_EXT2_MOUNT_IMAGE=$diskImage"
Write-Output ('BAREMETAL_VIRTIO_BLOCK_EXT2_MAGIC=0x{0:X4}' -f $ext2Magic)
Write-Output "BAREMETAL_VIRTIO_BLOCK_EXT2_PAYLOAD_MARKER=$expectedPayloadMarker"
