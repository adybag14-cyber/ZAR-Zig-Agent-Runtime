...
$zigLocalCacheDir = if ($env:ZIG_LOCAL_CACHE_DIR -and $env:ZIG_LOCAL_CACHE_DIR.Trim().Length -gt 0) { $env:ZIG_LOCAL_CACHE_DIR } else { Join-Path $repo '.zig-cache-e1000-https-post-probe' }

if ($null -eq $qemu) {
    Write-Output 'BAREMETAL_QEMU_AVAILABLE=False'
    Write-Output 'BAREMETAL_QEMU_E1000_HTTPS_POST_PROBE=skipped'
    return
}
...
$optionsPath = Join-Path $releaseDir 'qemu-e1000-https-post-probe-options.zig'
$mainObj = Join-Path $releaseDir 'openclaw-zig-baremetal-main-e1000-https-post-probe.o'
$bootObj = Join-Path $releaseDir 'openclaw-zig-pvh-boot-e1000-https-post-probe.o'
$artifact = Join-Path $releaseDir 'openclaw-zig-baremetal-pvh-e1000-https-post-probe.elf'
$diskImage = Join-Path $releaseDir 'qemu-e1000-https-post-probe.img'
...
$pcapPath = Join-Path $releaseDir 'qemu-e1000-https-post-probe.pcap'
$debugLogPath = Join-Path $releaseDir 'qemu-e1000-https-post-probe.debug.log'
$stdoutPath = Join-Path $releaseDir 'qemu-e1000-https-post-probe.stdout.log'
$stderrPath = Join-Path $releaseDir 'qemu-e1000-https-post-probe.stderr.log'
...
pub const e1000_https_post_probe: bool = true;
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj `
        ...
        --name 'openclaw-zig-baremetal-main-e1000-https-post-probe' `
        "-femit-bin=$mainObj"
    if ($LASTEXITCODE -ne 0) {
        throw "zig build-obj for E1000 HTTPS probe failed with exit code $LASTEXITCODE"
    }
...
if (-not (Test-Path $artifact)) {
    throw "E1000 HTTPS probe artifact is missing: $artifact"
}
if (-not (Test-Path $serverScript)) {
    throw "E1000 HTTPS probe server script is missing: $serverScript"
}
...
try {
    ...
    while (-not (Test-Path $serverReadyPath)) {
        if ($serverProc.HasExited) {
            ...
            throw "E1000 HTTPS probe server exited before ready: $serverStatusText"
        }
        if ([DateTime]::UtcNow -ge $readyDeadline) {
            throw 'Timed out waiting for E1000 HTTPS probe server readiness.'
        }
    ...
    if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
        ...
        throw "QEMU E1000 HTTPS POST probe timed out after $TimeoutSeconds seconds."
    }
    ...
    if ($exitCode -ne $expectedExitCode) {
        ...
        throw ("QEMU E1000 HTTPS POST probe failed with exit code {0} (probe code 0x{1:X2})." -f $exitCode, $probeCode)
    }
    ...
    if ($serverStatusText -ne 'ok') {
        throw "E1000 HTTPS probe server did not finish cleanly: $serverStatusText"
    }
    if (-not (Test-Path $serverRequestLogPath)) {
        throw 'E1000 HTTPS probe server did not capture the request log.'
    }

    Write-Output 'BAREMETAL_QEMU_AVAILABLE=True'
    Write-Output "BAREMETAL_QEMU_BINARY=$qemu"
    Write-Output 'BAREMETAL_QEMU_E1000_HTTPS_POST_PROBE=pass'
    Write-Output ("BAREMETAL_QEMU_E1000_HTTPS_POST_PROBE_CODE=0x{0:X2}" -f $expectedProbeCode)
    Write-Output "BAREMETAL_QEMU_E1000_HTTPS_POST_STDOUT=$stdoutPath"
    Write-Output "BAREMETAL_QEMU_E1000_HTTPS_POST_STDERR=$stderrPath"
    Write-Output "BAREMETAL_QEMU_E1000_HTTPS_POST_SERVER_STATUS=$serverStatusPath"
    Write-Output "BAREMETAL_QEMU_E1000_HTTPS_POST_REQUEST_LOG=$serverRequestLogPath"
    Write-Output "BAREMETAL_QEMU_E1000_HTTPS_POST_PCAP=$pcapPath"
    Write-Output "BAREMETAL_QEMU_E1000_HTTPS_POST_DEBUG_LOG=$debugLogPath"
}
finally {
