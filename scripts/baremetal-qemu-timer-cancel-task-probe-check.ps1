...
if ($null -eq $qemu -or $null -eq $gdb) {
    ...
    Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE=skipped"
    return
}
...
$optionsPath = Join-Path $releaseDir "qemu-timer-cancel-task-probe-options.zig"
$mainObj = Join-Path $releaseDir "openclaw-zig-baremetal-main-timer-cancel-task-probe.o"
$bootObj = Join-Path $releaseDir "openclaw-zig-pvh-boot-timer-cancel-task-probe.o"
$artifact = Join-Path $releaseDir "openclaw-zig-baremetal-pvh-timer-cancel-task-probe.elf"
$bootSource = Join-Path $repo "scripts\baremetal\pvh_boot.S"
$linkerScript = Join-Path $repo "scripts\baremetal\pvh_lld.ld"
$gdbScript = Join-Path $releaseDir "qemu-timer-cancel-task-probe.gdb"
$gdbStdout = Join-Path $releaseDir "qemu-timer-cancel-task-probe.gdb.stdout.log"
$gdbStderr = Join-Path $releaseDir "qemu-timer-cancel-task-probe.gdb.stderr.log"
$qemuStdout = Join-Path $releaseDir "qemu-timer-cancel-task-probe.qemu.stdout.log"
$qemuStderr = Join-Path $releaseDir "qemu-timer-cancel-task-probe.qemu.stderr.log"
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj `
        ...
        --name "openclaw-zig-baremetal-main-timer-cancel-task-probe" `
        "-femit-bin=$mainObj"
    if ($LASTEXITCODE -ne 0) {
        throw "zig build-obj for timer-cancel-task probe runtime failed with exit code $LASTEXITCODE"
    }

    & $clang -c -target x86_64-unknown-elf $bootSource -o $bootObj
    if ($LASTEXITCODE -ne 0) {
        throw "clang assemble for timer-cancel-task probe PVH shim failed with exit code $LASTEXITCODE"
    }

    & $lld -flavor gnu -m elf_x86_64 -o $artifact -T $linkerScript $mainObj $bootObj $compilerRt
    if ($LASTEXITCODE -ne 0) {
        throw "lld link for timer-cancel-task probe PVH artifact failed with exit code $LASTEXITCODE"
    }
}
...
if `$stage == 7
  if *(unsigned int*)(0x$statusAddress+$statusCommandSeqAckOffset) == 7 && *(unsigned char*)(0x$timerStateAddress+$timerEntryCountOffset) == 1 && *(unsigned char*)(0x$timerEntriesAddress+$timerEntryStateOffset) == $timerEntryStateArmed && *(unsigned int*)(0x$timerEntriesAddress+$timerEntryTaskIdOffset) == 1
...
if `$stage == 8
  if *(unsigned int*)(0x$statusAddress+$statusCommandSeqAckOffset) == 8 && *(unsigned char*)(0x$timerStateAddress+$timerEntryCountOffset) == 0 && *(unsigned char*)(0x$timerEntriesAddress+$timerEntryStateOffset) == $timerEntryStateCanceled
...
if `$stage == 9
  if *(unsigned int*)(0x$statusAddress+$statusCommandSeqAckOffset) == 9
    printf "AFTER_TIMER_CANCEL_TASK\n"
...
if (Test-Path $gdbStdout) {
    ...
    $hitAfterTimerCancelTask = $out -match "AFTER_TIMER_CANCEL_TASK"
...
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_START_ADDR=0x$startAddress"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_SPINPAUSE_ADDR=0x$spinPauseAddress"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_STATUS_ADDR=0x$statusAddress"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_MAILBOX_ADDR=0x$commandMailboxAddress"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_SCHEDULER_STATE_ADDR=0x$schedulerStateAddress"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_TASKS_ADDR=0x$schedulerTasksAddress"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_TIMER_STATE_ADDR=0x$timerStateAddress"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_TIMER_ENTRIES_ADDR=0x$timerEntriesAddress"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_WAKE_QUEUE_ADDR=0x$wakeQueueAddress"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_WAKE_QUEUE_COUNT_ADDR=0x$wakeQueueCountAddress"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_HIT_START=$hitStart"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_HIT_AFTER_TIMER_CANCEL_TASK=$hitAfterTimerCancelTask"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_ACK=$ack"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_LAST_OPCODE=$lastOpcode"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_LAST_RESULT=$lastResult"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_TICKS=$ticks"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_MAILBOX_OPCODE=$mailboxOpcode"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_MAILBOX_SEQ=$mailboxSeq"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_SCHED_TASK_COUNT=$schedulerTaskCount"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_TASK0_ID=$task0Id"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_TASK0_STATE=$task0State"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_TASK0_PRIORITY=$task0Priority"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_TASK0_RUN_COUNT=$task0RunCount"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_TASK0_BUDGET=$task0Budget"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_TASK0_BUDGET_REMAINING=$task0BudgetRemaining"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_ARMED_TICKS=$armedTicks"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_PRE_CANCEL_ENTRY_COUNT=$preCancelEntryCount"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_PRE_CANCEL_TIMER0_STATE=$preCancelTimer0State"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_PRE_CANCEL_TIMER0_TASK_ID=$preCancelTimer0TaskId"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_PRE_CANCEL_TIMER0_NEXT_FIRE_TICK=$preCancelTimer0NextFireTick"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_CANCEL_TICKS=$cancelTicks"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_CANCEL_ENTRY_COUNT=$cancelEntryCount"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_CANCEL_TIMER0_STATE=$cancelTimer0State"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_CANCEL_TASK0_STATE=$cancelTask0State"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_TIMER_ENABLED=$timerEnabled"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_TIMER_ENTRY_COUNT=$timerEntryCount"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_PENDING_WAKE_COUNT=$pendingWakeCount"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_TIMER_DISPATCH_COUNT=$timerDispatchCount"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_TIMER_LAST_WAKE_TICK=$timerLastWakeTick"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_WAKE_QUEUE_COUNT=$wakeQueueCount"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_TIMER0_ID=$timer0Id"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_TIMER0_TASK_ID=$timer0TaskId"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_TIMER0_STATE=$timer0State"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_TIMER0_NEXT_FIRE_TICK=$timer0NextFireTick"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_TIMER0_FIRE_COUNT=$timer0FireCount"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_TIMER0_LAST_FIRE_TICK=$timer0LastFireTick"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_GDB_STDOUT=$gdbStdout"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_GDB_STDERR=$gdbStderr"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_QEMU_STDOUT=$qemuStdout"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_QEMU_STDERR=$qemuStderr"
Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE_TIMED_OUT=$timedOut"

$pass = (
    $hitStart -and
    $hitAfterTimerCancelTask -and
    (-not $timedOut) -and
    $ack -eq 9 -and
...
if ($pass) {
    Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE=pass"
    exit 0
}

Write-Output "BAREMETAL_QEMU_TIMER_CANCEL_TASK_PROBE=fail"
if (Test-Path $gdbStdout) {
