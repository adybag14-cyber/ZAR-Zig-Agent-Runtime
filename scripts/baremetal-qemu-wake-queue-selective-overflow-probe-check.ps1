...
$gdbScript = Join-Path $releaseDir "qemu-wake-queue-selective-overflow-probe.gdb"
$gdbStdout = Join-Path $releaseDir "qemu-wake-queue-selective-overflow-probe.gdb.stdout.log"
$gdbStderr = Join-Path $releaseDir "qemu-wake-queue-selective-overflow-probe.gdb.stderr.log"
$qemuStdout = Join-Path $releaseDir "qemu-wake-queue-selective-overflow-probe.qemu.stdout.log"
$qemuStderr = Join-Path $releaseDir "qemu-wake-queue-selective-overflow-probe.qemu.stderr.log"
...
function Invoke-BatchPopArtifactBuildIfNeeded {
    if ($SkipBuild -and -not (Test-Path $artifact)) { throw "Wake-queue selective-overflow prerequisite artifact not found at $artifact and -SkipBuild was supplied." }
    if ($SkipBuild) { return }
...
if ($null -eq $qemu) {
    ...
    Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_OVERFLOW_PROBE=skipped"
    exit 0
}
if ($null -eq $gdb) {
...
set `$_pre_first_vector = 0
set `$_pre_last_seq = 0
set `$_pre_last_vector = 0
set `$_post_vector_count = 0
set `$_post_vector_head = 0
set `$_post_vector_tail = 0
set `$_post_vector_overflow = 0
set `$_post_vector_first_seq = 0
set `$_post_vector_first_vector = 0
set `$_post_vector_remaining_seq = 0
set `$_post_vector_remaining_vector = 0
set `$_post_vector_last_seq = 0
set `$_post_vector_last_vector = 0
set `$_post_reason_vector_count = 0
set `$_post_reason_vector_head = 0
set `$_post_reason_vector_tail = 0
set `$_post_reason_vector_overflow = 0
set `$_post_reason_vector_first_seq = 0
set `$_post_reason_vector_first_vector = 0
set `$_post_reason_vector_last_seq = 0
set `$_post_reason_vector_last_vector = 0
file $artifactForGdb
...
if `$_stage == 7
  if *(unsigned int*)(0x$statusAddress+$statusCommandSeqAckOffset) == `$_expected_seq && *(unsigned char*)(0x$schedulerTasksAddress+$taskStateOffset) == $taskStateReady
    ...
    if `$_wake_cycles == $overflowCycles && *(unsigned int*)0x$wakeQueueCountAddress == $wakeQueueCapacity && *(unsigned int*)0x$wakeQueueOverflowAddress == $expectedOverflow
      ...
      set `$_pre_first_vector = *(unsigned char*)(0x$wakeQueueAddress + (2 * $wakeEventStride) + $wakeEventVectorOffset)
      set `$_pre_last_seq = *(unsigned int*)(0x$wakeQueueAddress + (1 * $wakeEventStride) + $wakeEventSeqOffset)
      set `$_pre_last_vector = *(unsigned char*)(0x$wakeQueueAddress + (1 * $wakeEventStride) + $wakeEventVectorOffset)
      set *(unsigned short*)(0x$commandMailboxAddress+$commandOpcodeOffset) = $wakeQueuePopVectorOpcode
      set *(unsigned int*)(0x$commandMailboxAddress+$commandSeqOffset) = (`$_expected_seq + 1)
      set *(unsigned long long*)(0x$commandMailboxAddress+$commandArg0Offset) = $vectorA
      set *(unsigned long long*)(0x$commandMailboxAddress+$commandArg1Offset) = $batchVectorPopCount
      set `$_expected_seq = (`$_expected_seq + 1)
      set `$_stage = 8
    else
      set *(unsigned short*)(0x$commandMailboxAddress+$commandOpcodeOffset) = $taskWaitInterruptOpcode
...
if `$_stage == 8
  if *(unsigned int*)(0x$statusAddress+$statusCommandSeqAckOffset) == `$_expected_seq && *(unsigned int*)0x$wakeQueueCountAddress == 33 && *(unsigned int*)0x$wakeQueueHeadAddress == 33 && *(unsigned int*)0x$wakeQueueTailAddress == 0
    set `$_post_vector_count = *(unsigned int*)0x$wakeQueueCountAddress
    set `$_post_vector_head = *(unsigned int*)0x$wakeQueueHeadAddress
    set `$_post_vector_tail = *(unsigned int*)0x$wakeQueueTailAddress
    set `$_post_vector_overflow = *(unsigned int*)0x$wakeQueueOverflowAddress
    set `$_post_vector_first_seq = *(unsigned int*)(0x$wakeQueueAddress + (0 * $wakeEventStride) + $wakeEventSeqOffset)
    set `$_post_vector_first_vector = *(unsigned char*)(0x$wakeQueueAddress + (0 * $wakeEventStride) + $wakeEventVectorOffset)
    set `$_post_vector_remaining_seq = *(unsigned int*)(0x$wakeQueueAddress + (31 * $wakeEventStride) + $wakeEventSeqOffset)
    set `$_post_vector_remaining_vector = *(unsigned char*)(0x$wakeQueueAddress + (31 * $wakeEventStride) + $wakeEventVectorOffset)
    set `$_post_vector_last_seq = *(unsigned int*)(0x$wakeQueueAddress + (32 * $wakeEventStride) + $wakeEventSeqOffset)
    set `$_post_vector_last_vector = *(unsigned char*)(0x$wakeQueueAddress + (32 * $wakeEventStride) + $wakeEventVectorOffset)
...
if `$_stage == 9
  if *(unsigned int*)(0x$statusAddress+$statusCommandSeqAckOffset) == `$_expected_seq && *(unsigned int*)0x$wakeQueueCountAddress == 32 && *(unsigned int*)0x$wakeQueueHeadAddress == 32 && *(unsigned int*)0x$wakeQueueTailAddress == 0
    set `$_post_reason_vector_count = *(unsigned int*)0x$wakeQueueCountAddress
    set `$_post_reason_vector_head = *(unsigned int*)0x$wakeQueueHeadAddress
    set `$_post_reason_vector_tail = *(unsigned int*)0x$wakeQueueTailAddress
    set `$_post_reason_vector_overflow = *(unsigned int*)0x$wakeQueueOverflowAddress
    set `$_post_reason_vector_first_seq = *(unsigned int*)(0x$wakeQueueAddress + (0 * $wakeEventStride) + $wakeEventSeqOffset)
    set `$_post_reason_vector_first_vector = *(unsigned char*)(0x$wakeQueueAddress + (0 * $wakeEventStride) + $wakeEventVectorOffset)
    set `$_post_reason_vector_last_seq = *(unsigned int*)(0x$wakeQueueAddress + (31 * $wakeEventStride) + $wakeEventSeqOffset)
    set `$_post_reason_vector_last_vector = *(unsigned char*)(0x$wakeQueueAddress + (31 * $wakeEventStride) + $wakeEventVectorOffset)
    printf "AFTER_WAKE_QUEUE_SELECTIVE_OVERFLOW\n"
    ...
    printf "PRE_FIRST_VECTOR=%u\n", `$_pre_first_vector
    printf "PRE_LAST_SEQ=%u\n", `$_pre_last_seq
    printf "PRE_LAST_VECTOR=%u\n", `$_pre_last_vector
    printf "POST_VECTOR_COUNT=%u\n", `$_post_vector_count
    printf "POST_VECTOR_HEAD=%u\n", `$_post_vector_head
    printf "POST_VECTOR_TAIL=%u\n", `$_post_vector_tail
    printf "POST_VECTOR_OVERFLOW=%u\n", `$_post_vector_overflow
    printf "POST_VECTOR_FIRST_SEQ=%u\n", `$_post_vector_first_seq
    printf "POST_VECTOR_FIRST_VECTOR=%u\n", `$_post_vector_first_vector
    printf "POST_VECTOR_REMAINING_SEQ=%u\n", `$_post_vector_remaining_seq
    printf "POST_VECTOR_REMAINING_VECTOR=%u\n", `$_post_vector_remaining_vector
    printf "POST_VECTOR_LAST_SEQ=%u\n", `$_post_vector_last_seq
    printf "POST_VECTOR_LAST_VECTOR=%u\n", `$_post_vector_last_vector
    printf "POST_REASON_VECTOR_COUNT=%u\n", `$_post_reason_vector_count
    printf "POST_REASON_VECTOR_HEAD=%u\n", `$_post_reason_vector_head
    printf "POST_REASON_VECTOR_TAIL=%u\n", `$_post_reason_vector_tail
    printf "POST_REASON_VECTOR_OVERFLOW=%u\n", `$_post_reason_vector_overflow
    printf "POST_REASON_VECTOR_FIRST_SEQ=%u\n", `$_post_reason_vector_first_seq
    printf "POST_REASON_VECTOR_FIRST_VECTOR=%u\n", `$_post_reason_vector_first_vector
    printf "POST_REASON_VECTOR_LAST_SEQ=%u\n", `$_post_reason_vector_last_seq
    printf "POST_REASON_VECTOR_LAST_VECTOR=%u\n", `$_post_reason_vector_last_vector
    quit
  end
  continue
end
...
if (Test-Path $gdbStdout) {
    ...
    $hitAfter = $gdbOutput.Contains("AFTER_WAKE_QUEUE_SELECTIVE_OVERFLOW")
}
...
if ($timedOut) { throw "QEMU Wake-queue selective-overflow probe timed out after $TimeoutSeconds seconds.`nSTDOUT:`n$gdbOutput`nSTDERR:`n$gdbError" }
...
if (-not $hitStart -or -not $hitAfter) { throw "Wake-queue selective-overflow probe did not reach expected checkpoints.`nSTDOUT:`n$gdbOutput`nSTDERR:`n$gdbError" }
...
$preFirstVector = Extract-IntValue -Text $gdbOutput -Name "PRE_FIRST_VECTOR"
$preLastSeq = Extract-IntValue -Text $gdbOutput -Name "PRE_LAST_SEQ"
$preLastVector = Extract-IntValue -Text $gdbOutput -Name "PRE_LAST_VECTOR"
$postVectorCount = Extract-IntValue -Text $gdbOutput -Name "POST_VECTOR_COUNT"
$postVectorHead = Extract-IntValue -Text $gdbOutput -Name "POST_VECTOR_HEAD"
$postVectorTail = Extract-IntValue -Text $gdbOutput -Name "POST_VECTOR_TAIL"
$postVectorOverflow = Extract-IntValue -Text $gdbOutput -Name "POST_VECTOR_OVERFLOW"
$postVectorFirstSeq = Extract-IntValue -Text $gdbOutput -Name "POST_VECTOR_FIRST_SEQ"
$postVectorFirstVector = Extract-IntValue -Text $gdbOutput -Name "POST_VECTOR_FIRST_VECTOR"
$postVectorRemainingSeq = Extract-IntValue -Text $gdbOutput -Name "POST_VECTOR_REMAINING_SEQ"
$postVectorRemainingVector = Extract-IntValue -Text $gdbOutput -Name "POST_VECTOR_REMAINING_VECTOR"
$postVectorLastSeq = Extract-IntValue -Text $gdbOutput -Name "POST_VECTOR_LAST_SEQ"
$postVectorLastVector = Extract-IntValue -Text $gdbOutput -Name "POST_VECTOR_LAST_VECTOR"
$postReasonVectorCount = Extract-IntValue -Text $gdbOutput -Name "POST_REASON_VECTOR_COUNT"
$postReasonVectorHead = Extract-IntValue -Text $gdbOutput -Name "POST_REASON_VECTOR_HEAD"
$postReasonVectorTail = Extract-IntValue -Text $gdbOutput -Name "POST_REASON_VECTOR_TAIL"
$postReasonVectorOverflow = Extract-IntValue -Text $gdbOutput -Name "POST_REASON_VECTOR_OVERFLOW"
$postReasonVectorFirstSeq = Extract-IntValue -Text $gdbOutput -Name "POST_REASON_VECTOR_FIRST_SEQ"
$postReasonVectorFirstVector = Extract-IntValue -Text $gdbOutput -Name "POST_REASON_VECTOR_FIRST_VECTOR"
$postReasonVectorLastSeq = Extract-IntValue -Text $gdbOutput -Name "POST_REASON_VECTOR_LAST_SEQ"
$postReasonVectorLastVector = Extract-IntValue -Text $gdbOutput -Name "POST_REASON_VECTOR_LAST_VECTOR"
...
if ($preFirstSeq -ne 3 -or $preFirstVector -ne $vectorA -or $preLastSeq -ne 66 -or $preLastVector -ne $vectorB) { throw "Unexpected PRE seq/vector summary: $preFirstSeq/$preFirstVector/$preLastSeq/$preLastVector" }
if ($postVectorCount -ne 33 -or $postVectorHead -ne 33 -or $postVectorTail -ne 0 -or $postVectorOverflow -ne $expectedOverflow) { throw "Unexpected POST_VECTOR summary: $postVectorCount/$postVectorHead/$postVectorTail/$postVectorOverflow" }
if ($postVectorFirstSeq -ne 4 -or $postVectorFirstVector -ne $vectorB -or $postVectorRemainingSeq -ne 65 -or $postVectorRemainingVector -ne $vectorA -or $postVectorLastSeq -ne 66 -or $postVectorLastVector -ne $vectorB) { throw "Unexpected POST_VECTOR seq/vector summary" }
if ($postReasonVectorCount -ne 32 -or $postReasonVectorHead -ne 32 -or $postReasonVectorTail -ne 0 -or $postReasonVectorOverflow -ne $expectedOverflow) { throw "Unexpected POST_REASON_VECTOR summary: $postReasonVectorCount/$postReasonVectorHead/$postReasonVectorTail/$postReasonVectorOverflow" }
if ($postReasonVectorFirstSeq -ne 4 -or $postReasonVectorFirstVector -ne $vectorB -or $postReasonVectorLastSeq -ne 66 -or $postReasonVectorLastVector -ne $vectorB) { throw "Unexpected POST_REASON_VECTOR seq/vector summary" }

Write-Output "BAREMETAL_QEMU_AVAILABLE=True"
Write-Output "BAREMETAL_QEMU_BINARY=$qemu"
Write-Output "BAREMETAL_GDB_BINARY=$gdb"
Write-Output "BAREMETAL_NM_BINARY=$nm"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_OVERFLOW_PROBE=pass"
...
Write-Output "POST_VECTOR_COUNT=$postVectorCount"
Write-Output "POST_VECTOR_HEAD=$postVectorHead"
Write-Output "POST_VECTOR_TAIL=$postVectorTail"
Write-Output "POST_VECTOR_OVERFLOW=$postVectorOverflow"
Write-Output "POST_VECTOR_FIRST_SEQ=$postVectorFirstSeq"
Write-Output "POST_VECTOR_FIRST_VECTOR=$postVectorFirstVector"
Write-Output "POST_VECTOR_REMAINING_SEQ=$postVectorRemainingSeq"
Write-Output "POST_VECTOR_REMAINING_VECTOR=$postVectorRemainingVector"
Write-Output "POST_VECTOR_LAST_SEQ=$postVectorLastSeq"
Write-Output "POST_VECTOR_LAST_VECTOR=$postVectorLastVector"
Write-Output "POST_REASON_VECTOR_COUNT=$postReasonVectorCount"
Write-Output "POST_REASON_VECTOR_HEAD=$postReasonVectorHead"
Write-Output "POST_REASON_VECTOR_TAIL=$postReasonVectorTail"
Write-Output "POST_REASON_VECTOR_OVERFLOW=$postReasonVectorOverflow"
Write-Output "POST_REASON_VECTOR_FIRST_SEQ=$postReasonVectorFirstSeq"
Write-Output "POST_REASON_VECTOR_FIRST_VECTOR=$postReasonVectorFirstVector"
Write-Output "POST_REASON_VECTOR_LAST_SEQ=$postReasonVectorLastSeq"
Write-Output "POST_REASON_VECTOR_LAST_VECTOR=$postReasonVectorLastVector"
