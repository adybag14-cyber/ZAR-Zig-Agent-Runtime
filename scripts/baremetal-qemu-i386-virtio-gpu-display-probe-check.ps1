...
if (-not $SkipBuild) {
    & $zig build baremetal-i386 -Doptimize=Debug -Dbaremetal-virtio-gpu-display-probe=true --prefix $buildPrefix --summary all
...
if (-not (Test-Path $artifact)) { throw "i386 virtio-gpu display artifact not found at expected path: $artifact" }
