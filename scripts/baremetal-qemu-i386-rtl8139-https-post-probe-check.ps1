...
if ($null -eq $qemu) {
    ...
    Write-Output 'BAREMETAL_I386_QEMU_RTL8139_HTTPS_POST_PROBE=skipped'
    return
}
if (-not (Test-Path $serverScript)) { throw "HTTPS probe server script is missing: $serverScript" }

if (-not $SkipBuild) {
    & $zig build baremetal-i386 -Doptimize=ReleaseFast -Dbaremetal-rtl8139-https-post-probe=true --summary all
...
if ($null -eq $artifact) { throw 'i386 RTL8139 HTTPS POST artifact not found after build.' }

$diskImage = Join-Path $releaseDir 'qemu-i386-rtl8139-https-post-probe.img'
$serverReadyPath = Join-Path $releaseDir 'qemu-i386-rtl8139-https-post-server.ready'
$serverStatusPath = Join-Path $releaseDir 'qemu-i386-rtl8139-https-post-server.status.txt'
$serverRequestLogPath = Join-Path $releaseDir 'qemu-i386-rtl8139-https-post-server.request.log'
$serverStdoutPath = Join-Path $releaseDir 'qemu-i386-rtl8139-https-post-server.stdout.log'
$serverStderrPath = Join-Path $releaseDir 'qemu-i386-rtl8139-https-post-server.stderr.log'
$pcapPath = Join-Path $releaseDir 'qemu-i386-rtl8139-https-post-probe.pcap'
$stdoutPath = Join-Path $releaseDir 'qemu-i386-rtl8139-https-post-probe.stdout.log'
$stderrPath = Join-Path $releaseDir 'qemu-i386-rtl8139-https-post-probe.stderr.log'
...
try {
    ...
    if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
        ...
        throw "QEMU i386 RTL8139 HTTPS POST probe timed out after $TimeoutSeconds seconds."
    }
    $proc.WaitForExit()
    ...
    if ($exitCode -ne $expectedExitCode) {
        ...
        throw ("QEMU i386 RTL8139 HTTPS POST probe failed with exit code {0} (probe code 0x{1:X2})." -f $exitCode, $probeCode)
    }
...
Write-Output 'BAREMETAL_I386_QEMU_RTL8139_HTTPS_POST_PROBE=pass'
Write-Output ("BAREMETAL_I386_QEMU_RTL8139_HTTPS_POST_PROBE_CODE=0x{0:X2}" -f $expectedProbeCode)
Write-Output "BAREMETAL_I386_QEMU_RTL8139_HTTPS_POST_STDOUT=$stdoutPath"
Write-Output "BAREMETAL_I386_QEMU_RTL8139_HTTPS_POST_STDERR=$stderrPath"
Write-Output "BAREMETAL_I386_QEMU_RTL8139_HTTPS_POST_SERVER_STATUS=$serverStatusPath"
Write-Output "BAREMETAL_I386_QEMU_RTL8139_HTTPS_POST_REQUEST_LOG=$serverRequestLogPath"
Write-Output "BAREMETAL_I386_QEMU_RTL8139_HTTPS_POST_PCAP=$pcapPath"
