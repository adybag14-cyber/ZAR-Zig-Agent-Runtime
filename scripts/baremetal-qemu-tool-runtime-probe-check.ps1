...
if ($null -eq $qemu) {
    ...
    Write-Output 'BAREMETAL_QEMU_TOOL_RUNTIME_PROBE=skipped'
    return
}
...
$diskImage = Join-Path $releaseDir 'qemu-tool-runtime-probe.img'
$bootSource = Join-Path $repo 'scripts\baremetal\pvh_boot.S'
$linkerScript = Join-Path $repo 'scripts\baremetal\pvh_lld.ld'
$stdoutPath = Join-Path $releaseDir 'qemu-tool-runtime-probe.stdout.log'
$stderrPath = Join-Path $releaseDir 'qemu-tool-runtime-probe.stderr.log'
...
pub const tool_runtime_probe: bool = true;
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "zig build-obj for tool runtime probe failed with exit code $LASTEXITCODE"
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "lld link for tool runtime PVH artifact failed with exit code $LASTEXITCODE"
    }
}

if (-not (Test-Path $artifact)) {
    throw "Tool runtime probe artifact is missing: $artifact"
}
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "QEMU tool runtime probe timed out after $TimeoutSeconds seconds."
}
...
if ($proc.ExitCode -ne $expectedExitCode) {
    ...
    throw "Tool runtime probe exit code mismatch. Expected $expectedExitCode, got $($proc.ExitCode)."
}

Write-Output 'BAREMETAL_QEMU_TOOL_RUNTIME_PROBE=pass'
