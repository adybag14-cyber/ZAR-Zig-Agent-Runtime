...
if ($null -eq $qemu) {
    ...
    Write-Output 'BAREMETAL_I386_QEMU_VIRTIO_BLOCK_MOUNT_CONTROL_PROBE=skipped'
    return
}
...
if (-not $SkipBuild) {
    & $zig build baremetal-i386 -Doptimize=ReleaseSafe -Dbaremetal-virtio-block-mount-control-probe=true --summary all
    if ($LASTEXITCODE -ne 0) { throw "zig build baremetal-i386 virtio-block mount control probe failed with exit code $LASTEXITCODE" }
}
...
if ($null -eq $artifact) { throw 'i386 virtio-block mount control probe artifact not found after build.' }
...
Write-Output 'BAREMETAL_I386_QEMU_AVAILABLE=True'
Write-Output "BAREMETAL_I386_QEMU_BINARY=$qemu"
Write-Output 'BAREMETAL_I386_QEMU_VIRTIO_BLOCK_MOUNT_CONTROL_PROBE=pass'
