...
$zigLocalCacheDir = if ($env:ZIG_LOCAL_CACHE_DIR -and $env:ZIG_LOCAL_CACHE_DIR.Trim().Length -gt 0) { $env:ZIG_LOCAL_CACHE_DIR } else { Join-Path $repo '.zig-cache-virtio-net-ipv4-probe' }
...
if ($null -eq $qemu) {
    ...
    Write-Output 'BAREMETAL_QEMU_VIRTIO_NET_IPV4_PROBE=skipped'
    return
}
...
$stdoutPath = Join-Path $releaseDir 'qemu-virtio-net-ipv4-probe.stdout.log'
$stderrPath = Join-Path $releaseDir 'qemu-virtio-net-ipv4-probe.stderr.log'
...
pub const virtio_net_ipv4_probe: bool = true;
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "zig build-obj for VIRTIO-NET IPV4 probe failed with exit code $LASTEXITCODE"
    }
...
if (-not (Test-Path $artifact)) {
    throw "VIRTIO-NET IPV4 probe artifact is missing: $artifact"
}
...
Write-Output 'BAREMETAL_QEMU_VIRTIO_NET_IPV4_PROBE=passed'
