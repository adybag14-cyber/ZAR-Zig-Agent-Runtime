...
if ($null -eq $qemu) {
    ...
    Write-Output "BAREMETAL_I386_FIRMWARE_AP_STARTUP_PROBE=skipped"
    return
}
...
if ($null -eq $artifact) {
    throw "i386 bare-metal firmware AP startup artifact not found after build."
}

$firmwareImage = Join-Path $releaseDir "qemu-i386-firmware-ap-startup-probe.img"
$firmwareMetadata = Join-Path $releaseDir "qemu-i386-firmware-ap-startup-probe.meta.txt"
$stdoutPath = Join-Path $releaseDir "qemu-i386-firmware-ap-startup-probe-stdout.log"
$stderrPath = Join-Path $releaseDir "qemu-i386-firmware-ap-startup-probe-stderr.log"
$debugLogPath = Join-Path $releaseDir "qemu-i386-firmware-ap-startup-probe-debug.log"
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    if (Test-Path $debugLogPath) {
        $debugTail = (Get-Content -Path $debugLogPath -Tail 60 -ErrorAction SilentlyContinue) -join "`n"
    }
    if ([string]::IsNullOrWhiteSpace($debugTail)) {
        throw "QEMU i386 firmware AP startup probe timed out after $TimeoutSeconds seconds."
    }
    throw "QEMU i386 firmware AP startup probe timed out after $TimeoutSeconds seconds.`n$debugTail"
}
...
if (-not $expectedExitCodes.ContainsKey($exitCode)) {
    ...
    throw "QEMU i386 firmware AP startup probe failed: exit=$exitCode expected one of [$expectedText]`n$stderrTail`n$debugTail"
}
...
Write-Output ("BAREMETAL_I386_QEMU_FIRMWARE_AP_STARTUP_PROBE_CODE=0x{0:X2}" -f $observedProbeCode)
Write-Output ("BAREMETAL_I386_QEMU_FIRMWARE_AP_EXECUTION_OBSERVED={0}" -f ($(if ($executionObserved) { "True" } else { "False" })))
Write-Output "BAREMETAL_I386_QEMU_FIRMWARE_AP_STARTUP_PROBE_DEBUG=$debugLogPath"
Write-Output "BAREMETAL_I386_FIRMWARE_AP_STARTUP_PROBE=pass"
