...
$gdbScript = Join-Path $releaseDir "qemu-scheduler-timeslice-update-probe.gdb"
$gdbStdout = Join-Path $releaseDir "qemu-scheduler-timeslice-update-probe.gdb.stdout.log"
$gdbStderr = Join-Path $releaseDir "qemu-scheduler-timeslice-update-probe.gdb.stderr.log"
$qemuStdout = Join-Path $releaseDir "qemu-scheduler-timeslice-update-probe.qemu.stdout.log"
$qemuStderr = Join-Path $releaseDir "qemu-scheduler-timeslice-update-probe.qemu.stderr.log"
...
function Invoke-SchedulerArtifactBuildIfNeeded {
    if ($SkipBuild -and -not (Test-Path $artifact)) { throw "Scheduler timeslice prerequisite artifact not found at $artifact and -SkipBuild was supplied." }
    if ($SkipBuild) { return }
...
if ($null -eq $qemu) {
    ...
    Write-Output "BAREMETAL_QEMU_SCHEDULER_TIMESLICE_UPDATE_PROBE=skipped"
    exit 0
}
if ($null -eq $gdb) {
...
if $stage == 3
  if *(unsigned int*)(0x__STATUS__+__STATUS_ACK_OFFSET__) == 3 && *(unsigned char*)(0x__SCHED_STATE__+__SCHED_TASK_COUNT_OFFSET__) == 1 && *(unsigned int*)(0x__TASKS__+__TASK_RUN_COUNT_OFFSET__) == 1 && *(unsigned int*)(0x__TASKS__+__TASK_BUDGET_REMAINING_OFFSET__) == 9 && *(unsigned int*)(0x__SCHED_STATE__+__SCHED_TIMESLICE_OFFSET__) == __TIMESLICE_INITIAL__
    printf "PRE_TIMESLICE=%u\n", *(unsigned int*)(0x__SCHED_STATE__+__SCHED_TIMESLICE_OFFSET__)
    printf "PRE_RUN_COUNT=%u\n", *(unsigned int*)(0x__TASKS__+__TASK_RUN_COUNT_OFFSET__)
    printf "PRE_BUDGET_REMAINING=%u\n", *(unsigned int*)(0x__TASKS__+__TASK_BUDGET_REMAINING_OFFSET__)
    set *(unsigned short*)(0x__COMMAND_MAILBOX__+__COMMAND_OPCODE_OFFSET__) = __SCHEDULER_SET_TIMESLICE_OPCODE__
    set *(unsigned int*)(0x__COMMAND_MAILBOX__+__COMMAND_SEQ_OFFSET__) = 4
    set *(unsigned long long*)(0x__COMMAND_MAILBOX__+__COMMAND_ARG0_OFFSET__) = __TIMESLICE_UPDATED1__
    set *(unsigned long long*)(0x__COMMAND_MAILBOX__+__COMMAND_ARG1_OFFSET__) = 0
    set $stage = 4
  end
  continue
...
if $stage == 4
  if *(unsigned int*)(0x__STATUS__+__STATUS_ACK_OFFSET__) == 4 && *(unsigned int*)(0x__SCHED_STATE__+__SCHED_TIMESLICE_OFFSET__) == __TIMESLICE_UPDATED1__ && *(unsigned int*)(0x__TASKS__+__TASK_RUN_COUNT_OFFSET__) == 2 && *(unsigned int*)(0x__TASKS__+__TASK_BUDGET_REMAINING_OFFSET__) == 5
    printf "MID_TIMESLICE_4=%u\n", *(unsigned int*)(0x__SCHED_STATE__+__SCHED_TIMESLICE_OFFSET__)
    printf "MID_RUN_COUNT_4=%u\n", *(unsigned int*)(0x__TASKS__+__TASK_RUN_COUNT_OFFSET__)
    printf "MID_BUDGET_REMAINING_4=%u\n", *(unsigned int*)(0x__TASKS__+__TASK_BUDGET_REMAINING_OFFSET__)
    set *(unsigned short*)(0x__COMMAND_MAILBOX__+__COMMAND_OPCODE_OFFSET__) = __SCHEDULER_SET_TIMESLICE_OPCODE__
    set *(unsigned int*)(0x__COMMAND_MAILBOX__+__COMMAND_SEQ_OFFSET__) = 5
    set *(unsigned long long*)(0x__COMMAND_MAILBOX__+__COMMAND_ARG0_OFFSET__) = __TIMESLICE_UPDATED2__
    set *(unsigned long long*)(0x__COMMAND_MAILBOX__+__COMMAND_ARG1_OFFSET__) = 0
    set $stage = 5
  end
  continue
...
if $stage == 5
  if *(unsigned int*)(0x__STATUS__+__STATUS_ACK_OFFSET__) == 5 && *(unsigned int*)(0x__SCHED_STATE__+__SCHED_TIMESLICE_OFFSET__) == __TIMESLICE_UPDATED2__ && *(unsigned int*)(0x__TASKS__+__TASK_RUN_COUNT_OFFSET__) == 3 && *(unsigned int*)(0x__TASKS__+__TASK_BUDGET_REMAINING_OFFSET__) == 3
    printf "MID_TIMESLICE_2=%u\n", *(unsigned int*)(0x__SCHED_STATE__+__SCHED_TIMESLICE_OFFSET__)
    printf "MID_RUN_COUNT_2=%u\n", *(unsigned int*)(0x__TASKS__+__TASK_RUN_COUNT_OFFSET__)
    printf "MID_BUDGET_REMAINING_2=%u\n", *(unsigned int*)(0x__TASKS__+__TASK_BUDGET_REMAINING_OFFSET__)
    set *(unsigned short*)(0x__COMMAND_MAILBOX__+__COMMAND_OPCODE_OFFSET__) = __SCHEDULER_SET_TIMESLICE_OPCODE__
...
if $stage == 6
  if *(unsigned int*)(0x__STATUS__+__STATUS_ACK_OFFSET__) == 6 && *(short*)(0x__STATUS__+__STATUS_LAST_RESULT_OFFSET__) == -22 && *(unsigned int*)(0x__SCHED_STATE__+__SCHED_TIMESLICE_OFFSET__) == __TIMESLICE_UPDATED2__ && *(unsigned int*)(0x__TASKS__+__TASK_RUN_COUNT_OFFSET__) == 4 && *(unsigned int*)(0x__TASKS__+__TASK_BUDGET_REMAINING_OFFSET__) == 1
    printf "HIT_AFTER_SCHEDULER_TIMESLICE_UPDATE_PROBE\n"
...
$gdbContent = $gdbTemplate.
    ...
    Replace('__SCHEDULER_SET_TIMESLICE_OPCODE__', [string]$schedulerSetTimesliceOpcode).
    Replace('__TASK_BUDGET__', [string]$taskBudget).
    Replace('__TASK_PRIORITY__', [string]$taskPriority).
    Replace('__TIMESLICE_INITIAL__', [string]$timesliceInitial).
    Replace('__TIMESLICE_UPDATED1__', [string]$timesliceUpdated1).
    Replace('__TIMESLICE_UPDATED2__', [string]$timesliceUpdated2).
...
try {
    ...
    if (-not $gdbProcess.WaitForExit($TimeoutSeconds * 1000)) {
        ...
        throw "Timed out waiting for GDB scheduler timeslice update probe"
    }
    ...
    if ($gdbExitCode -ne 0) {
        throw "GDB scheduler timeslice update probe failed with exit code $gdbExitCode. stdout: $gdbOutput stderr: $gdbError"
    }

    foreach ($requiredMarker in @("HIT_START", "HIT_AFTER_SCHEDULER_TIMESLICE_UPDATE_PROBE")) {
    ...
    $expectations = @{
        "PRE_TIMESLICE" = $timesliceInitial
        "PRE_RUN_COUNT" = 1
        "PRE_BUDGET_REMAINING" = 9
        "MID_TIMESLICE_4" = $timesliceUpdated1
        "MID_RUN_COUNT_4" = 2
        "MID_BUDGET_REMAINING_4" = 5
        "MID_TIMESLICE_2" = $timesliceUpdated2
        "MID_RUN_COUNT_2" = 3
        "MID_BUDGET_REMAINING_2" = 3
        "ACK" = 6
        "LAST_OPCODE" = $schedulerSetTimesliceOpcode
        "LAST_RESULT" = -22
        "ENABLED" = 1
        "TASK_COUNT" = 1
        "RUNNING_SLOT" = 0
        "TIMESLICE" = $timesliceUpdated2
        "TASK0_ID" = 1
        "TASK0_STATE" = 1
        "TASK0_PRIORITY" = $taskPriority
        "TASK0_RUN_COUNT" = 4
        "TASK0_BUDGET" = $taskBudget
        "TASK0_BUDGET_REMAINING" = 1
    }
    ...
    if ($ticks -lt 5) {
        throw "Unexpected TICKS value. Expected at least 5, got $ticks. stdout: $gdbOutput stderr: $gdbError"
    }

    $dispatchCount = Extract-IntValue -Text $gdbOutput -Name "DISPATCH_COUNT"
    if ($null -eq $dispatchCount -or $dispatchCount -lt 4) {
        throw "Unexpected DISPATCH_COUNT value. Expected at least 4, got $dispatchCount. stdout: $gdbOutput stderr: $gdbError"
    }
    ...
    Write-Output "BAREMETAL_GDB_BINARY=$gdb"
    Write-Output "BAREMETAL_NM_BINARY=$nm"
    Write-Output "BAREMETAL_QEMU_SCHEDULER_TIMESLICE_UPDATE_PROBE=pass"
    $gdbOutput.TrimEnd()
} finally {
