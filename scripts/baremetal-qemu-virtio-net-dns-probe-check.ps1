...
if ($null -eq $qemu) {
    ...
    Write-Output 'BAREMETAL_QEMU_VIRTIO_NET_DNS_PROBE=skipped'
    return
}
...
$optionsPath = Join-Path $releaseDir 'qemu-virtio-net-dns-probe-options.zig'
$rootModulePath = (Join-Path $repo "src/baremetal_main.zig").Replace('\\', '/')
$optionsModulePath = $optionsPath.Replace('\\', '/')
$mainObj = Join-Path $releaseDir 'openclaw-zig-baremetal-main-virtio-net-dns-probe.o'
$bootObj = Join-Path $releaseDir 'openclaw-zig-pvh-boot-virtio-net-dns-probe.o'
$artifact = Join-Path $releaseDir 'openclaw-zig-baremetal-pvh-virtio-net-dns-probe.elf'
$bootSource = Join-Path $repo 'scripts\baremetal\pvh_boot.S'
$linkerScript = Join-Path $repo 'scripts\baremetal\pvh_lld.ld'
$stdoutPath = Join-Path $releaseDir 'qemu-virtio-net-dns-probe.stdout.log'
$stderrPath = Join-Path $releaseDir 'qemu-virtio-net-dns-probe.stderr.log'
...
pub const virtio_net_dns_probe: bool = true;
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj `
        ...
        --name 'openclaw-zig-baremetal-main-virtio-net-dns-probe' `
        "-femit-bin=$mainObj"
    if ($LASTEXITCODE -ne 0) {
        throw "zig build-obj for VIRTIO-NET DNS probe failed with exit code $LASTEXITCODE"
    }
...
if (-not (Test-Path $artifact)) {
    throw "VIRTIO-NET DNS probe artifact is missing: $artifact"
}
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "QEMU VIRTIO-NET DNS probe timed out after $TimeoutSeconds seconds."
}
...
if ($exitCode -ne $expectedExitCode) {
    ...
    throw ("QEMU VIRTIO-NET DNS probe failed with exit code {0} (probe code 0x{1:X2})." -f $exitCode, $probeCode)
}

Write-Output 'BAREMETAL_QEMU_AVAILABLE=True'
Write-Output "BAREMETAL_QEMU_BINARY=$qemu"
Write-Output 'BAREMETAL_QEMU_VIRTIO_NET_DNS_PROBE=pass'
Write-Output ("BAREMETAL_QEMU_VIRTIO_NET_DNS_PROBE_CODE=0x{0:X2}" -f $expectedProbeCode)
Write-Output "BAREMETAL_QEMU_VIRTIO_NET_DNS_STDOUT=$stdoutPath"
Write-Output "BAREMETAL_QEMU_VIRTIO_NET_DNS_STDERR=$stderrPath"
