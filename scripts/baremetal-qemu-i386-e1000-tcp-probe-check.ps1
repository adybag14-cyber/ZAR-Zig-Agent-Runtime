...
if ($null -eq $qemu) {
    ...
    Write-Output 'BAREMETAL_I386_QEMU_E1000_TCP_PROBE=skipped'
    return
}

if (-not $SkipBuild) {
    & $zig build baremetal-i386 -Doptimize=ReleaseFast -Dbaremetal-e1000-tcp-probe=true --summary all
    if ($LASTEXITCODE -ne 0) { throw "zig build baremetal-i386 e1000 tcp probe failed with exit code $LASTEXITCODE" }
}
...
$stdoutPath = Join-Path $releaseDir 'qemu-i386-e1000-tcp-probe.stdout.log'
$stderrPath = Join-Path $releaseDir 'qemu-i386-e1000-tcp-probe.stderr.log'
$echoStdoutPath = Join-Path $releaseDir 'qemu-i386-e1000-tcp-echo.stdout.log'
$echoStderrPath = Join-Path $releaseDir 'qemu-i386-e1000-tcp-echo.stderr.log'
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "QEMU i386 E1000 TCP probe timed out after $TimeoutSeconds seconds."
}
$proc.WaitForExit()
...
if ($exitCode -ne $expectedExitCode) { throw "QEMU i386 E1000 TCP probe failed: expected exit $expectedExitCode but saw $exitCode" }
if ($echoProc.ExitCode -gt 0) { throw "QEMU i386 E1000 TCP echo helper failed with exit code $($echoProc.ExitCode)" }

Write-Output 'BAREMETAL_I386_QEMU_AVAILABLE=True'
Write-Output "BAREMETAL_I386_QEMU_BINARY=$qemu"
Write-Output 'BAREMETAL_I386_QEMU_E1000_TCP_PROBE=pass'
Write-Output ("BAREMETAL_I386_QEMU_E1000_TCP_PROBE_CODE=0x{0:X2}" -f $expectedProbeCode)
Write-Output "BAREMETAL_I386_QEMU_E1000_TCP_STDOUT=$stdoutPath"
Write-Output "BAREMETAL_I386_QEMU_E1000_TCP_STDERR=$stderrPath"
Write-Output "BAREMETAL_I386_QEMU_E1000_TCP_ECHO_STDOUT=$echoStdoutPath"
Write-Output "BAREMETAL_I386_QEMU_E1000_TCP_ECHO_STDERR=$echoStderrPath"
