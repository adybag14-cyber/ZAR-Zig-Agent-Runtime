...
if ($null -eq $qemu) {
    ...
    Write-Output "BAREMETAL_I386_FIRMWARE_SMP_OWNED_PROBE=skipped"
    return
}
...
if (-not $SkipBuild) {
    & $zig build baremetal-i386 -Doptimize=ReleaseFast -Dbaremetal-i386-smp-owned-probe=true --prefix $buildPrefix --summary all
    if ($LASTEXITCODE -ne 0) {
        throw "zig build baremetal-i386 -Dbaremetal-i386-smp-owned-probe=true failed with exit code $LASTEXITCODE"
    }
}
...
if ($null -eq $artifact) {
    throw "i386 bare-metal firmware SMP owned artifact not found after build."
}

$firmwareImage = Join-Path $releaseDir "qemu-i386-firmware-smp-owned-probe.img"
$firmwareMetadata = Join-Path $releaseDir "qemu-i386-firmware-smp-owned-probe.meta.txt"
$stdoutPath = Join-Path $releaseDir "qemu-i386-firmware-smp-owned-probe-stdout.log"
$stderrPath = Join-Path $releaseDir "qemu-i386-firmware-smp-owned-probe-stderr.log"
$debugLogPath = Join-Path $releaseDir "qemu-i386-firmware-smp-owned-probe-debug.log"
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    if ([string]::IsNullOrWhiteSpace($debugTail)) {
        throw "QEMU i386 firmware SMP owned probe timed out after $TimeoutSeconds seconds."
    }
    throw "QEMU i386 firmware SMP owned probe timed out after $TimeoutSeconds seconds.`n$debugTail"
}
...
if (-not $expectedExitCodes.ContainsKey($exitCode)) {
    ...
    throw "QEMU i386 firmware SMP owned probe failed: exit=$exitCode expected one of [$expectedText]`n$stderrTail`n$debugTail"
}
...
Write-Output ("BAREMETAL_I386_QEMU_FIRMWARE_SMP_OWNED_PROBE_CODE=0x{0:X2}" -f $observedProbeCode)
Write-Output "BAREMETAL_I386_QEMU_FIRMWARE_SMP_OWNED_PROBE_DEBUG=$debugLogPath"
Write-Output "BAREMETAL_I386_FIRMWARE_SMP_OWNED_PROBE=pass"
