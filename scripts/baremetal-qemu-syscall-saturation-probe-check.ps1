...
$syscallTokenBase = 0x1000
$overflowSyscallId = $syscallEntryCapacity + 1
$overflowToken = 0xDEAD
$reuseSlotIndex = 5
$reusePreviousId = $reuseSlotIndex + 1
$reuseNewId = $syscallEntryCapacity + 42
$reuseToken = 0xA55A
$reuseInvokeArg = 0x66
$expectedInvokeResult = ($reuseToken -bxor $reuseInvokeArg -bxor $reuseNewId)
...
if ($null -eq $qemu -or $null -eq $gdb) {
    ...
    Write-Output "BAREMETAL_QEMU_SYSCALL_SATURATION_PROBE=skipped"
    return
}
if ($null -eq $nm) {
...
$optionsPath = Join-Path $releaseDir "qemu-syscall-saturation-probe-options.zig"
$mainObj = Join-Path $releaseDir "openclaw-zig-baremetal-main-syscall-saturation-probe.o"
$bootObj = Join-Path $releaseDir "openclaw-zig-pvh-boot-syscall-saturation-probe.o"
$artifact = Join-Path $releaseDir "openclaw-zig-baremetal-pvh-syscall-saturation-probe.elf"
$bootSource = Join-Path $repo "scripts\baremetal\pvh_boot.S"
$linkerScript = Join-Path $repo "scripts\baremetal\pvh_lld.ld"
$gdbScript = Join-Path $releaseDir "qemu-syscall-saturation-probe-$runStamp.gdb"
$gdbStdout = Join-Path $releaseDir "qemu-syscall-saturation-probe-$runStamp.gdb.stdout.log"
$gdbStderr = Join-Path $releaseDir "qemu-syscall-saturation-probe-$runStamp.gdb.stderr.log"
$qemuStdout = Join-Path $releaseDir "qemu-syscall-saturation-probe-$runStamp.qemu.stdout.log"
$qemuStderr = Join-Path $releaseDir "qemu-syscall-saturation-probe-$runStamp.qemu.stderr.log"
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    if ($LASTEXITCODE -ne 0) { throw "clang assemble for syscall-saturation probe PVH shim failed with exit code $LASTEXITCODE" }
    & $lld -flavor gnu -m elf_x86_64 -o $artifact -T $linkerScript $mainObj $bootObj $compilerRt
    if ($LASTEXITCODE -ne 0) { throw "lld link for syscall-saturation probe PVH artifact failed with exit code $LASTEXITCODE" }
}
...
$reuseSlotAddressExpr = "(0x$syscallEntriesAddress + ($syscallEntryStride * $reuseSlotIndex))"
$lastSlotAddressExpr = "(0x$syscallEntriesAddress + ($syscallEntryStride * ($syscallEntryCapacity - 1)))"
...
set $_registered = 0
set $_full_count = 0
set $_last_registered_id = 0
set $_overflow_result = 0
set $_reuse_old_id = 0
set $_reuse_new_id = 0
set $_invoke_result = 0
set $_invoke_tick = 0
file __ARTIFACT__
...
if $stage == 2
  if *(unsigned int*)(0x__STATUS__+__STATUS_ACK_OFFSET__) == (1 + $_registered) && *(unsigned char*)(0x__SYSCALL_STATE__+__SYSCALL_ENTRY_COUNT_OFFSET__) == $_registered
    if $_registered < __SYSCALL_ENTRY_CAPACITY__
      ...
      set $_full_count = *(unsigned char*)(0x__SYSCALL_STATE__+__SYSCALL_ENTRY_COUNT_OFFSET__)
      set $_last_registered_id = *(unsigned int*)(__LAST_SLOT__+__SYSCALL_ENTRY_ID_OFFSET__)
      set $_reuse_old_id = *(unsigned int*)(__REUSE_SLOT__+__SYSCALL_ENTRY_ID_OFFSET__)
      set *(unsigned short*)(0x__COMMAND_MAILBOX__+__COMMAND_OPCODE_OFFSET__) = __SYSCALL_REGISTER_OPCODE__
      set *(unsigned int*)(0x__COMMAND_MAILBOX__+__COMMAND_SEQ_OFFSET__) = (__SYSCALL_ENTRY_CAPACITY__ + 2)
      set *(unsigned long long*)(0x__COMMAND_MAILBOX__+__COMMAND_ARG0_OFFSET__) = __OVERFLOW_ID__
      set *(unsigned long long*)(0x__COMMAND_MAILBOX__+__COMMAND_ARG1_OFFSET__) = __OVERFLOW_TOKEN__
      set $stage = 3
    end
  end
  continue
...
if $stage == 3
  if *(unsigned int*)(0x__STATUS__+__STATUS_ACK_OFFSET__) == (__SYSCALL_ENTRY_CAPACITY__ + 2) && *(short*)(0x__STATUS__+__STATUS_LAST_RESULT_OFFSET__) == __RESULT_NO_SPACE__ && *(unsigned char*)(0x__SYSCALL_STATE__+__SYSCALL_ENTRY_COUNT_OFFSET__) == __SYSCALL_ENTRY_CAPACITY__
    set $_overflow_result = *(short*)(0x__STATUS__+__STATUS_LAST_RESULT_OFFSET__)
    ...
    set *(unsigned long long*)(0x__COMMAND_MAILBOX__+__COMMAND_ARG0_OFFSET__) = __REUSE_PREVIOUS_ID__
    set *(unsigned long long*)(0x__COMMAND_MAILBOX__+__COMMAND_ARG1_OFFSET__) = 0
    set $stage = 4
  end
  continue
...
if $stage == 4
  if *(unsigned int*)(0x__STATUS__+__STATUS_ACK_OFFSET__) == (__SYSCALL_ENTRY_CAPACITY__ + 3) && *(short*)(0x__STATUS__+__STATUS_LAST_RESULT_OFFSET__) == 0 && *(unsigned char*)(0x__SYSCALL_STATE__+__SYSCALL_ENTRY_COUNT_OFFSET__) == (__SYSCALL_ENTRY_CAPACITY__ - 1) && *(unsigned char*)(__REUSE_SLOT__+__SYSCALL_ENTRY_STATE_OFFSET__) == __SYSCALL_ENTRY_UNUSED__
    set *(unsigned short*)(0x__COMMAND_MAILBOX__+__COMMAND_OPCODE_OFFSET__) = __SYSCALL_REGISTER_OPCODE__
    set *(unsigned int*)(0x__COMMAND_MAILBOX__+__COMMAND_SEQ_OFFSET__) = (__SYSCALL_ENTRY_CAPACITY__ + 4)
    set *(unsigned long long*)(0x__COMMAND_MAILBOX__+__COMMAND_ARG0_OFFSET__) = __REUSE_NEW_ID__
    set *(unsigned long long*)(0x__COMMAND_MAILBOX__+__COMMAND_ARG1_OFFSET__) = __REUSE_TOKEN__
    set $stage = 5
  end
  continue
end
if $stage == 5
  if *(unsigned int*)(0x__STATUS__+__STATUS_ACK_OFFSET__) == (__SYSCALL_ENTRY_CAPACITY__ + 4) && *(short*)(0x__STATUS__+__STATUS_LAST_RESULT_OFFSET__) == 0 && *(unsigned char*)(0x__SYSCALL_STATE__+__SYSCALL_ENTRY_COUNT_OFFSET__) == __SYSCALL_ENTRY_CAPACITY__ && *(unsigned char*)(__REUSE_SLOT__+__SYSCALL_ENTRY_STATE_OFFSET__) == __SYSCALL_ENTRY_REGISTERED__ && *(unsigned int*)(__REUSE_SLOT__+__SYSCALL_ENTRY_ID_OFFSET__) == __REUSE_NEW_ID__ && *(unsigned long long*)(__REUSE_SLOT__+__SYSCALL_ENTRY_TOKEN_OFFSET__) == __REUSE_TOKEN__
    set $_reuse_new_id = *(unsigned int*)(__REUSE_SLOT__+__SYSCALL_ENTRY_ID_OFFSET__)
    ...
    set *(unsigned long long*)(0x__COMMAND_MAILBOX__+__COMMAND_ARG0_OFFSET__) = __REUSE_NEW_ID__
    set *(unsigned long long*)(0x__COMMAND_MAILBOX__+__COMMAND_ARG1_OFFSET__) = __REUSE_INVOKE_ARG__
    set $stage = 6
  end
  continue
end
if $stage == 6
  if *(unsigned int*)(0x__STATUS__+__STATUS_ACK_OFFSET__) == (__SYSCALL_ENTRY_CAPACITY__ + 5) && *(unsigned short*)(0x__STATUS__+__STATUS_LAST_OPCODE_OFFSET__) == __SYSCALL_INVOKE_OPCODE__ && *(short*)(0x__STATUS__+__STATUS_LAST_RESULT_OFFSET__) == __RESULT_OK__ && *(unsigned char*)(0x__STATUS__+__STATUS_MODE_OFFSET__) == __MODE_RUNNING__ && *(unsigned char*)(0x__SYSCALL_STATE__+__SYSCALL_ENTRY_COUNT_OFFSET__) == __SYSCALL_ENTRY_CAPACITY__ && *(unsigned long long*)(0x__SYSCALL_STATE__+__SYSCALL_DISPATCH_COUNT_OFFSET__) == 1 && *(unsigned int*)(0x__SYSCALL_STATE__+__SYSCALL_LAST_ID_OFFSET__) == __REUSE_NEW_ID__ && *(long long*)(0x__SYSCALL_STATE__+__SYSCALL_LAST_RESULT_OFFSET__) == __EXPECTED_INVOKE_RESULT__ && *(unsigned char*)(__REUSE_SLOT__+__SYSCALL_ENTRY_STATE_OFFSET__) == __SYSCALL_ENTRY_REGISTERED__ && *(unsigned int*)(__REUSE_SLOT__+__SYSCALL_ENTRY_ID_OFFSET__) == __REUSE_NEW_ID__ && *(unsigned long long*)(__REUSE_SLOT__+__SYSCALL_ENTRY_INVOKE_COUNT_OFFSET__) == 1
    set $_invoke_result = *(long long*)(0x__SYSCALL_STATE__+__SYSCALL_LAST_RESULT_OFFSET__)
    set $_invoke_tick = *(unsigned long long*)(0x__SYSCALL_STATE__+__SYSCALL_LAST_INVOKE_TICK_OFFSET__)
    printf "HIT_AFTER_SYSCALL_SATURATION_PROBE\n"
    ...
    printf "ENTRY_CAPACITY=%u\n", __SYSCALL_ENTRY_CAPACITY__
    printf "ENTRY_COUNT=%u\n", *(unsigned char*)(0x__SYSCALL_STATE__+__SYSCALL_ENTRY_COUNT_OFFSET__)
    printf "FULL_COUNT=%u\n", $_full_count
    printf "LAST_REGISTERED_ID=%u\n", $_last_registered_id
    printf "OVERFLOW_RESULT=%d\n", $_overflow_result
    printf "REUSE_PREVIOUS_ID=%u\n", $_reuse_old_id
    printf "REUSE_NEW_ID=%u\n", $_reuse_new_id
    printf "REUSE_TOKEN=%llu\n", *(unsigned long long*)(__REUSE_SLOT__+__SYSCALL_ENTRY_TOKEN_OFFSET__)
    printf "REUSE_INVOKE_COUNT=%llu\n", *(unsigned long long*)(__REUSE_SLOT__+__SYSCALL_ENTRY_INVOKE_COUNT_OFFSET__)
    printf "REUSE_LAST_ARG=%llu\n", *(unsigned long long*)(__REUSE_SLOT__+__SYSCALL_ENTRY_LAST_ARG_OFFSET__)
    printf "REUSE_LAST_RESULT=%lld\n", *(long long*)(__REUSE_SLOT__+__SYSCALL_ENTRY_LAST_RESULT_OFFSET__)
...
$gdbContent = $gdbTemplate.
    ...
    Replace('__TOKEN_BASE__', [string]$syscallTokenBase).
    Replace('__OVERFLOW_ID__', [string]$overflowSyscallId).
    Replace('__OVERFLOW_TOKEN__', [string]$overflowToken).
    Replace('__REUSE_PREVIOUS_ID__', [string]$reusePreviousId).
    Replace('__REUSE_NEW_ID__', [string]$reuseNewId).
    Replace('__REUSE_TOKEN__', [string]$reuseToken).
    Replace('__REUSE_INVOKE_ARG__', [string]$reuseInvokeArg).
    ...
    Replace('__REUSE_SLOT__', $reuseSlotAddressExpr).
    Replace('__LAST_SLOT__', $lastSlotAddressExpr)
...
try {
    ...
    if (-not $gdbProcess.WaitForExit($TimeoutSeconds * 1000)) {
        ...
        throw "Timed out waiting for GDB syscall-saturation probe"
    }
    ...
    if ($gdbExitCode -ne 0) {
        throw "GDB syscall-saturation probe failed with exit code $gdbExitCode. stdout: $gdbOutput stderr: $gdbError"
    }

    foreach ($requiredMarker in @("HIT_START", "HIT_AFTER_SYSCALL_SATURATION_PROBE")) {
    ...
    $expectations = @{
        ...
        "ENTRY_CAPACITY" = $syscallEntryCapacity
        "ENTRY_COUNT" = $syscallEntryCapacity
        "FULL_COUNT" = $syscallEntryCapacity
        "LAST_REGISTERED_ID" = $syscallEntryCapacity
        "OVERFLOW_RESULT" = $resultNoSpace
        "REUSE_PREVIOUS_ID" = $reusePreviousId
        "REUSE_NEW_ID" = $reuseNewId
        "REUSE_TOKEN" = $reuseToken
        "REUSE_INVOKE_COUNT" = 1
        "REUSE_LAST_ARG" = $reuseInvokeArg
        "REUSE_LAST_RESULT" = $expectedInvokeResult
        "DISPATCH_COUNT" = 1
        "LAST_ID" = $reuseNewId
    ...
    Write-Output "BAREMETAL_GDB_BINARY=$gdb"
    Write-Output "BAREMETAL_NM_BINARY=$nm"
    Write-Output "BAREMETAL_QEMU_SYSCALL_SATURATION_PROBE=pass"
    $gdbOutput.TrimEnd()
} finally {
