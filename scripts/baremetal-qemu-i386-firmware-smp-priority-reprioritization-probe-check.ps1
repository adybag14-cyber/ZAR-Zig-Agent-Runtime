...
if ($null -eq $qemu) {
    ...
    Write-Output "BAREMETAL_I386_FIRMWARE_SMP_PRIORITY_REPRIORITIZATION_PROBE=skipped"
    return
}
...
if (-not $SkipBuild) {
    & $zig build baremetal-i386 -Doptimize=ReleaseFast -Dbaremetal-i386-smp-priority-reprioritization-probe=true --prefix $buildPrefix --summary all
    if ($LASTEXITCODE -ne 0) {
        throw "zig build baremetal-i386 -Dbaremetal-i386-smp-priority-reprioritization-probe=true failed with exit code $LASTEXITCODE"
    }
}
...
if ($null -eq $artifact) {
    throw "i386 bare-metal firmware SMP priority reprioritization artifact not found after build."
}

$firmwareImage = Join-Path $releaseDir "qemu-i386-firmware-smp-priority-reprioritization-probe.img"
$firmwareMetadata = Join-Path $releaseDir "qemu-i386-firmware-smp-priority-reprioritization-probe.meta.txt"
$stdoutPath = Join-Path $releaseDir "qemu-i386-firmware-smp-priority-reprioritization-probe-stdout.log"
$stderrPath = Join-Path $releaseDir "qemu-i386-firmware-smp-priority-reprioritization-probe-stderr.log"
$debugLogPath = Join-Path $releaseDir "qemu-i386-firmware-smp-priority-reprioritization-probe-debug.log"
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "QEMU i386 firmware SMP priority reprioritization probe timed out after $TimeoutSeconds seconds."
    }
    throw "QEMU i386 firmware SMP priority reprioritization probe timed out after $TimeoutSeconds seconds.`n$debugTail"
}
...
if (-not $expectedExitCodes.ContainsKey($exitCode)) {
    ...
    throw "QEMU i386 firmware SMP priority reprioritization probe failed: exit=$exitCode expected one of [$expectedText]`n$stderrTail`n$debugTail"
}
...
Write-Output ("BAREMETAL_I386_QEMU_FIRMWARE_SMP_PRIORITY_REPRIORITIZATION_PROBE_CODE=0x{0:X2}" -f $observedProbeCode)
Write-Output "BAREMETAL_I386_QEMU_FIRMWARE_SMP_PRIORITY_REPRIORITIZATION_PROBE_DEBUG=$debugLogPath"
Write-Output "BAREMETAL_I386_FIRMWARE_SMP_PRIORITY_REPRIORITIZATION_PROBE=pass"
