...
if ($null -eq $qemu) {
    ...
    Write-Output 'BAREMETAL_QEMU_E1000_HTTP_POST_PROBE=skipped'
    return
}
...
$optionsPath = Join-Path $releaseDir 'qemu-e1000-http-post-probe-options.zig'
$rootModulePath = (Join-Path $repo "src/baremetal_main.zig").Replace('\\', '/')
$optionsModulePath = $optionsPath.Replace('\\', '/')
$mainObj = Join-Path $releaseDir 'openclaw-zig-baremetal-main-e1000-http-post-probe.o'
$bootObj = Join-Path $releaseDir 'openclaw-zig-pvh-boot-e1000-http-post-probe.o'
$artifact = Join-Path $releaseDir 'openclaw-zig-baremetal-pvh-e1000-http-post-probe.elf'
$diskImage = Join-Path $releaseDir 'qemu-e1000-http-post-probe.img'
$bootSource = Join-Path $repo 'scripts\baremetal\pvh_boot.S'
$linkerScript = Join-Path $repo 'scripts\baremetal\pvh_lld.ld'
$stdoutPath = Join-Path $releaseDir 'qemu-e1000-http-post-probe.stdout.log'
$stderrPath = Join-Path $releaseDir 'qemu-e1000-http-post-probe.stderr.log'
...
pub const e1000_http_post_probe: bool = true;
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj `
        ...
        --name 'openclaw-zig-baremetal-main-e1000-http-post-probe' `
        "-femit-bin=$mainObj"
    if ($LASTEXITCODE -ne 0) {
        throw "zig build-obj for E1000 HTTP probe failed with exit code $LASTEXITCODE"
    }
...
if (-not (Test-Path $artifact)) {
    throw "E1000 HTTP probe artifact is missing: $artifact"
}
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "QEMU E1000 HTTP POST probe timed out after $TimeoutSeconds seconds."
}
...
if ($exitCode -ne $expectedExitCode) {
    ...
    throw ("QEMU E1000 HTTP POST probe failed with exit code {0} (probe code 0x{1:X2})." -f $exitCode, $probeCode)
}

Write-Output 'BAREMETAL_QEMU_AVAILABLE=True'
Write-Output "BAREMETAL_QEMU_BINARY=$qemu"
Write-Output 'BAREMETAL_QEMU_E1000_HTTP_POST_PROBE=pass'
Write-Output ("BAREMETAL_QEMU_E1000_HTTP_POST_PROBE_CODE=0x{0:X2}" -f $expectedProbeCode)
Write-Output "BAREMETAL_QEMU_E1000_HTTP_POST_STDOUT=$stdoutPath"
Write-Output "BAREMETAL_QEMU_E1000_HTTP_POST_STDERR=$stderrPath"
