...
$gdbScript = Join-Path $releaseDir "qemu-mode-boot-phase-history-probe.gdb"
$gdbStdout = Join-Path $releaseDir "qemu-mode-boot-phase-history-probe.gdb.stdout.log"
$gdbStderr = Join-Path $releaseDir "qemu-mode-boot-phase-history-probe.gdb.stderr.log"
$qemuStdout = Join-Path $releaseDir "qemu-mode-boot-phase-history-probe.qemu.stdout.log"
$qemuStderr = Join-Path $releaseDir "qemu-mode-boot-phase-history-probe.qemu.stderr.log"
...
$modeHistoryCapacity = 64
$bootPhaseHistoryCapacity = 64
$overflowCycleCount = 33
$overflowCommandCount = $overflowCycleCount * 2
$expectedHistoryOverflow = $overflowCommandCount - $modeHistoryCapacity
$expectedFinalAck = 5 + $overflowCommandCount
$expectedFinalTickFloor = $overflowCommandCount
$expectedHistoryHead = 2
$eventStride = 24
...
function Invoke-DescriptorArtifactBuildIfNeeded {
    if ($SkipBuild -and -not (Test-Path $artifact)) { throw "Mode/boot-phase history prerequisite artifact not found at $artifact and -SkipBuild was supplied." }
    if ($SkipBuild) { return }
...
if ($null -eq $qemu) {
    ...
    Write-Output "BAREMETAL_QEMU_MODE_BOOT_PHASE_HISTORY_PROBE=skipped"
    exit 0
}
if ($null -eq $gdb) {
...
if $stage == 0
  if *(unsigned int*)(0x__STATUS__+__STATUS_ACK_OFFSET__) == 0
    ...
    set *(unsigned int*)(0x__BOOTDIAG__+__BOOTDIAG_LAST_COMMAND_SEQ_OFFSET__) = 0
    set *(unsigned long long*)(0x__BOOTDIAG__+__BOOTDIAG_LAST_COMMAND_TICK_OFFSET__) = 0
    set *(unsigned long long*)(0x__BOOTDIAG__+__BOOTDIAG_LAST_TICK_OBSERVED_OFFSET__) = 0
...
if $stage == 2
  if *(unsigned int*)(0x__STATUS__+__STATUS_ACK_OFFSET__) == 4 && *(unsigned int*)0x__MODE_HISTORY_COUNT__ == 0
...
if $stage == 3
  if *(unsigned int*)(0x__STATUS__+__STATUS_ACK_OFFSET__) == 5 && *(unsigned int*)0x__MODE_HISTORY_COUNT__ == 0 && *(unsigned int*)0x__BOOT_HISTORY_COUNT__ == 0
    set *(unsigned char*)(0x__STATUS__+__STATUS_MODE_OFFSET__) = __MODE_RUNNING__
    set *(unsigned long long*)(0x__STATUS__+__STATUS_TICKS_OFFSET__) = 0
    set *(unsigned short*)(0x__STATUS__+__STATUS_HEALTH_OFFSET__) = 200
    set *(unsigned int*)(0x__STATUS__+__STATUS_PANIC_OFFSET__) = 0
...
if $stage == 4
  if *(unsigned int*)(0x__STATUS__+__STATUS_ACK_OFFSET__) == $expected_seq && $overflow_emitted < __OVERFLOW_COMMAND_COUNT__
    if ($overflow_emitted % 2) == 0
      set *(unsigned short*)(0x__COMMAND_MAILBOX__+__COMMAND_OPCODE_OFFSET__) = __SET_BOOT_PHASE_OPCODE__
      set *(unsigned int*)(0x__COMMAND_MAILBOX__+__COMMAND_SEQ_OFFSET__) = $expected_seq + 1
      set *(unsigned long long*)(0x__COMMAND_MAILBOX__+__COMMAND_ARG0_OFFSET__) = __BOOT_PHASE_INIT__
      set *(unsigned long long*)(0x__COMMAND_MAILBOX__+__COMMAND_ARG1_OFFSET__) = 0
    else
      set *(unsigned short*)(0x__COMMAND_MAILBOX__+__COMMAND_OPCODE_OFFSET__) = __SET_MODE_OPCODE__
    ...
    set $overflow_emitted = $overflow_emitted + 1
    if $overflow_emitted == __OVERFLOW_COMMAND_COUNT__
      set $stage = 5
    end
  end
  continue
...
if $stage == 5
  if *(unsigned int*)(0x__STATUS__+__STATUS_ACK_OFFSET__) == $expected_seq && *(unsigned int*)0x__MODE_HISTORY_COUNT__ == __MODE_HISTORY_CAPACITY__ && *(unsigned int*)0x__MODE_HISTORY_OVERFLOW__ == __EXPECTED_HISTORY_OVERFLOW__ && *(unsigned int*)0x__BOOT_HISTORY_COUNT__ == __BOOT_HISTORY_CAPACITY__ && *(unsigned int*)0x__BOOT_HISTORY_OVERFLOW__ == __EXPECTED_HISTORY_OVERFLOW__
    set $mode_head = *(unsigned int*)0x__MODE_HISTORY_HEAD__
    set $mode_oldest = $mode_head
    set $mode_newest = ($mode_head + __MODE_HISTORY_CAPACITY__ - 1) % __MODE_HISTORY_CAPACITY__
    set $boot_head = *(unsigned int*)0x__BOOT_HISTORY_HEAD__
    set $boot_oldest = $boot_head
    set $boot_newest = ($boot_head + __BOOT_HISTORY_CAPACITY__ - 1) % __BOOT_HISTORY_CAPACITY__
    printf "HIT_AFTER_MODE_BOOT_PHASE_HISTORY_PROBE\n"
    ...
    printf "MODE_HISTORY_OVERFLOW=%u\n", *(unsigned int*)0x__MODE_HISTORY_OVERFLOW__
    printf "MODE_HISTORY_HEAD=%u\n", *(unsigned int*)0x__MODE_HISTORY_HEAD__
    printf "MODE_HISTORY_FIRST_SEQ=%u\n", *(unsigned int*)(0x__MODE_HISTORY__+($mode_oldest*__EVENT_STRIDE__)+__EVENT_SEQ_OFFSET__)
    printf "MODE_HISTORY_FIRST_PREV=%u\n", *(unsigned char*)(0x__MODE_HISTORY__+($mode_oldest*__EVENT_STRIDE__)+__EVENT_PREV_OFFSET__)
    printf "MODE_HISTORY_FIRST_NEW=%u\n", *(unsigned char*)(0x__MODE_HISTORY__+($mode_oldest*__EVENT_STRIDE__)+__EVENT_NEW_OFFSET__)
    printf "MODE_HISTORY_FIRST_REASON=%u\n", *(unsigned char*)(0x__MODE_HISTORY__+($mode_oldest*__EVENT_STRIDE__)+__EVENT_REASON_OFFSET__)
    printf "MODE_HISTORY_LAST_SEQ=%u\n", *(unsigned int*)(0x__MODE_HISTORY__+($mode_newest*__EVENT_STRIDE__)+__EVENT_SEQ_OFFSET__)
    printf "MODE_HISTORY_LAST_PREV=%u\n", *(unsigned char*)(0x__MODE_HISTORY__+($mode_newest*__EVENT_STRIDE__)+__EVENT_PREV_OFFSET__)
    printf "MODE_HISTORY_LAST_NEW=%u\n", *(unsigned char*)(0x__MODE_HISTORY__+($mode_newest*__EVENT_STRIDE__)+__EVENT_NEW_OFFSET__)
    printf "MODE_HISTORY_LAST_REASON=%u\n", *(unsigned char*)(0x__MODE_HISTORY__+($mode_newest*__EVENT_STRIDE__)+__EVENT_REASON_OFFSET__)
    ...
    printf "BOOT_HISTORY_FIRST_SEQ=%u\n", *(unsigned int*)(0x__BOOT_HISTORY__+($boot_oldest*__EVENT_STRIDE__)+__EVENT_SEQ_OFFSET__)
    printf "BOOT_HISTORY_FIRST_PREV=%u\n", *(unsigned char*)(0x__BOOT_HISTORY__+($boot_oldest*__EVENT_STRIDE__)+__EVENT_PREV_OFFSET__)
    printf "BOOT_HISTORY_FIRST_NEW=%u\n", *(unsigned char*)(0x__BOOT_HISTORY__+($boot_oldest*__EVENT_STRIDE__)+__EVENT_NEW_OFFSET__)
    printf "BOOT_HISTORY_FIRST_REASON=%u\n", *(unsigned char*)(0x__BOOT_HISTORY__+($boot_oldest*__EVENT_STRIDE__)+__EVENT_REASON_OFFSET__)
    printf "BOOT_HISTORY_LAST_SEQ=%u\n", *(unsigned int*)(0x__BOOT_HISTORY__+($boot_newest*__EVENT_STRIDE__)+__EVENT_SEQ_OFFSET__)
    printf "BOOT_HISTORY_LAST_PREV=%u\n", *(unsigned char*)(0x__BOOT_HISTORY__+($boot_newest*__EVENT_STRIDE__)+__EVENT_PREV_OFFSET__)
    printf "BOOT_HISTORY_LAST_NEW=%u\n", *(unsigned char*)(0x__BOOT_HISTORY__+($boot_newest*__EVENT_STRIDE__)+__EVENT_NEW_OFFSET__)
    printf "BOOT_HISTORY_LAST_REASON=%u\n", *(unsigned char*)(0x__BOOT_HISTORY__+($boot_newest*__EVENT_STRIDE__)+__EVENT_REASON_OFFSET__)
    quit
  end
  continue
end
...
$gdbContent = $gdbTemplate.
    ...
    Replace('__BOOTDIAG_LAST_COMMAND_SEQ_OFFSET__', [string]$bootDiagLastCommandSeqOffset).
    Replace('__BOOTDIAG_LAST_COMMAND_TICK_OFFSET__', [string]$bootDiagLastCommandTickOffset).
    Replace('__BOOTDIAG_LAST_TICK_OBSERVED_OFFSET__', [string]$bootDiagLastTickObservedOffset).
    ...
    Replace('__OVERFLOW_COMMAND_COUNT__', [string]$overflowCommandCount).
    Replace('__MODE_HISTORY_CAPACITY__', [string]$modeHistoryCapacity).
    Replace('__BOOT_HISTORY_CAPACITY__', [string]$bootPhaseHistoryCapacity).
    Replace('__EXPECTED_HISTORY_OVERFLOW__', [string]$expectedHistoryOverflow).
    Replace('__EVENT_STRIDE__', [string]$eventStride).
    Replace('__EVENT_SEQ_OFFSET__', [string]$historySeqOffset).
    Replace('__EVENT_PREV_OFFSET__', [string]$historyPrevOffset).
    Replace('__EVENT_NEW_OFFSET__', [string]$historyNewOffset).
    Replace('__EVENT_REASON_OFFSET__', [string]$historyReasonOffset)
...
try {
    ...
    if (-not $gdbProcess.WaitForExit($TimeoutSeconds * 1000)) {
        ...
        throw "Timed out waiting for GDB mode/boot-phase history probe"
    }
    ...
    if ($gdbExitCode -ne 0) {
        throw "GDB mode/boot-phase history probe failed with exit code $gdbExitCode. stdout: $gdbOutput stderr: $gdbError"
    }

    foreach ($requiredMarker in @("HIT_START", "HIT_AFTER_SEMANTIC_SAMPLE", "HIT_AFTER_MODE_BOOT_PHASE_HISTORY_PROBE")) {
    ...
    $expectations = @{
        "MODE_SEMANTIC_LEN" = 3
        "MODE_SEM0_PREV" = $modeRunning
        "MODE_SEM0_NEW" = $modeBooting
        "MODE_SEM0_REASON" = $modeReasonCommand
        "MODE_SEM1_PREV" = $modeBooting
        "MODE_SEM1_NEW" = $modeRunning
        "MODE_SEM1_REASON" = $modeReasonRuntimeTick
        "MODE_SEM2_PREV" = $modeRunning
        "MODE_SEM2_NEW" = $modePanicked
        "MODE_SEM2_REASON" = $modeReasonPanic
        "BOOT_SEMANTIC_LEN" = 3
        "BOOT_SEM0_PREV" = $bootPhaseRuntime
        "BOOT_SEM0_NEW" = $bootPhaseInit
        "BOOT_SEM0_REASON" = $bootReasonCommand
        "BOOT_SEM1_PREV" = $bootPhaseInit
        "BOOT_SEM1_NEW" = $bootPhaseRuntime
        "BOOT_SEM1_REASON" = $bootReasonRuntimeTick
        "BOOT_SEM2_PREV" = $bootPhaseRuntime
        "BOOT_SEM2_NEW" = $bootPhasePanicked
        "BOOT_SEM2_REASON" = $bootReasonPanic
        "ACK" = $expectedFinalAck
        "LAST_OPCODE" = $setModeOpcode
        "LAST_RESULT" = 0
        "MODE_HISTORY_LEN" = $modeHistoryCapacity
        "MODE_HISTORY_OVERFLOW" = $expectedHistoryOverflow
        "MODE_HISTORY_HEAD" = $expectedHistoryHead
        "MODE_HISTORY_FIRST_SEQ" = 3
        "MODE_HISTORY_FIRST_PREV" = $modeRunning
        "MODE_HISTORY_FIRST_NEW" = $modeBooting
        "MODE_HISTORY_FIRST_REASON" = $modeReasonCommand
        "MODE_HISTORY_LAST_SEQ" = $overflowCommandCount
        "MODE_HISTORY_LAST_PREV" = $modeBooting
        "MODE_HISTORY_LAST_NEW" = $modeRunning
        "MODE_HISTORY_LAST_REASON" = $modeReasonRuntimeTick
        "BOOT_HISTORY_LEN" = $bootPhaseHistoryCapacity
        "BOOT_HISTORY_OVERFLOW" = $expectedHistoryOverflow
        "BOOT_HISTORY_HEAD" = $expectedHistoryHead
        "BOOT_HISTORY_FIRST_SEQ" = 3
        "BOOT_HISTORY_FIRST_PREV" = $bootPhaseRuntime
        "BOOT_HISTORY_FIRST_NEW" = $bootPhaseInit
        "BOOT_HISTORY_FIRST_REASON" = $bootReasonCommand
        "BOOT_HISTORY_LAST_SEQ" = $overflowCommandCount
        "BOOT_HISTORY_LAST_PREV" = $bootPhaseInit
        "BOOT_HISTORY_LAST_NEW" = $bootPhaseRuntime
        "BOOT_HISTORY_LAST_REASON" = $bootReasonRuntimeTick
    }
    ...
    if ($ticks -lt $expectedFinalTickFloor) {
        throw "Unexpected TICKS value. Expected at least $expectedFinalTickFloor, got $ticks. stdout: $gdbOutput stderr: $gdbError"
    }
    ...
    Write-Output "BAREMETAL_GDB_BINARY=$gdb"
    Write-Output "BAREMETAL_NM_BINARY=$nm"
    Write-Output "BAREMETAL_QEMU_MODE_BOOT_PHASE_HISTORY_PROBE=pass"
    $gdbOutput.TrimEnd()
} finally {
