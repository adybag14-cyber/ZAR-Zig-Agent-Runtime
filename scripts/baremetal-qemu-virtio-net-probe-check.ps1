...
$zigLocalCacheDir = if ($env:ZIG_LOCAL_CACHE_DIR -and $env:ZIG_LOCAL_CACHE_DIR.Trim().Length -gt 0) { $env:ZIG_LOCAL_CACHE_DIR } else { Join-Path $repo '.zig-cache-virtio-net-probe' }
...
if ($null -eq $qemu) {
    ...
    Write-Output 'BAREMETAL_QEMU_VIRTIO_NET_PROBE=skipped'
    return
}
...
$optionsPath = Join-Path $releaseDir 'qemu-virtio-net-probe-options.zig'
$mainObj = Join-Path $releaseDir 'openclaw-zig-baremetal-main-virtio-net-probe.o'
$bootObj = Join-Path $releaseDir 'openclaw-zig-pvh-boot-virtio-net-probe.o'
$artifact = Join-Path $releaseDir 'openclaw-zig-baremetal-pvh-virtio-net-probe.elf'
...
$stdoutPath = Join-Path $releaseDir 'qemu-virtio-net-probe.stdout.log'
$stderrPath = Join-Path $releaseDir 'qemu-virtio-net-probe.stderr.log'
...
pub const virtio_net_probe: bool = true;
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj `
        ...
        --name 'openclaw-zig-baremetal-main-virtio-net-probe' `
        "-femit-bin=$mainObj"
    if ($LASTEXITCODE -ne 0) {
        throw "zig build-obj for VIRTIO-NET probe failed with exit code $LASTEXITCODE"
    }
...
if (-not (Test-Path $artifact)) {
    throw "VIRTIO-NET probe artifact is missing: $artifact"
}
...
Write-Output 'BAREMETAL_QEMU_VIRTIO_NET_PROBE=passed'
