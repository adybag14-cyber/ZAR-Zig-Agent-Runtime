...
$optionsPath = Join-Path $releaseDir 'qemu-rtl8139-http-post-probe-options.zig'
$mainObj = Join-Path $releaseDir 'openclaw-zig-baremetal-main-rtl8139-http-post-probe.o'
$bootObj = Join-Path $releaseDir 'openclaw-zig-pvh-boot-rtl8139-tcp-probe.o'
$artifact = Join-Path $releaseDir 'openclaw-zig-baremetal-pvh-rtl8139-http-post-probe.elf'
$diskImage = Join-Path $releaseDir 'qemu-rtl8139-http-post-probe.img'
$bootSource = Join-Path $repo 'scripts\baremetal\pvh_boot.S'
$linkerScript = Join-Path $repo 'scripts\baremetal\pvh_lld.ld'
$stdoutPath = Join-Path $releaseDir 'qemu-rtl8139-http-post-probe.stdout.log'
$stderrPath = Join-Path $releaseDir 'qemu-rtl8139-http-post-probe.stderr.log'
...
pub const rtl8139_http_post_probe: bool = true;
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj `
        ...
        --name 'openclaw-zig-baremetal-main-rtl8139-http-post-probe' `
        "-femit-bin=$mainObj"
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "QEMU RTL8139 HTTP POST probe timed out after $TimeoutSeconds seconds."
}
...
if ($exitCode -ne $expectedExitCode) {
    ...
    throw ("QEMU RTL8139 HTTP POST probe failed with exit code {0} (probe code 0x{1:X2})." -f $exitCode, $probeCode)
}

Write-Output 'BAREMETAL_QEMU_AVAILABLE=True'
Write-Output "BAREMETAL_QEMU_BINARY=$qemu"
Write-Output 'BAREMETAL_QEMU_RTL8139_HTTP_POST_PROBE=pass'
Write-Output ("BAREMETAL_QEMU_RTL8139_HTTP_POST_PROBE_CODE=0x{0:X2}" -f $expectedProbeCode)
Write-Output "BAREMETAL_QEMU_RTL8139_HTTP_POST_STDOUT=$stdoutPath"
Write-Output "BAREMETAL_QEMU_RTL8139_HTTP_POST_STDERR=$stderrPath"
