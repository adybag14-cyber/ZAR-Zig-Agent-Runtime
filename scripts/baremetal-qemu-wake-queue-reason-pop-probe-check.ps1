...
if ($null -eq $qemu) {
    ...
    Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_PROBE=skipped"
    return
}
if ($null -eq $gdb) {
...
$optionsPath = Join-Path $releaseDir "qemu-wake-queue-reason-pop-options.zig"
$mainObj = Join-Path $releaseDir "openclaw-zig-baremetal-main-wake-queue-reason-pop.o"
$bootObj = Join-Path $releaseDir "openclaw-zig-pvh-boot-wake-queue-reason-pop.o"
$artifact = Join-Path $releaseDir "openclaw-zig-baremetal-pvh-wake-queue-reason-pop.elf"
$bootSource = Join-Path $repo "scripts\baremetal\pvh_boot.S"
$linkerScript = Join-Path $repo "scripts\baremetal\pvh_lld.ld"
$gdbScript = Join-Path $releaseDir "qemu-wake-queue-reason-pop-$runStamp.gdb"
$gdbStdout = Join-Path $releaseDir "qemu-wake-queue-reason-pop-$runStamp.gdb.stdout.log"
$gdbStderr = Join-Path $releaseDir "qemu-wake-queue-reason-pop-$runStamp.gdb.stderr.log"
$qemuStdout = Join-Path $releaseDir "qemu-wake-queue-reason-pop-$runStamp.qemu.stdout.log"
$qemuStderr = Join-Path $releaseDir "qemu-wake-queue-reason-pop-$runStamp.qemu.stderr.log"
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj `
        ...
        --name "openclaw-zig-baremetal-main-wake-queue-reason-pop" `
        "-femit-bin=$mainObj"
    if ($LASTEXITCODE -ne 0) {
        throw "zig build-obj for wake-queue reason-pop runtime failed with exit code $LASTEXITCODE"
    }

    & $clang -c -target x86_64-unknown-elf $bootSource -o $bootObj
    if ($LASTEXITCODE -ne 0) {
        throw "clang assemble for wake-queue reason-pop PVH shim failed with exit code $LASTEXITCODE"
    }

    & $lld -flavor gnu -m elf_x86_64 -o $artifact -T $linkerScript $mainObj $bootObj $compilerRt
    if ($LASTEXITCODE -ne 0) {
        throw "lld link for wake-queue reason-pop PVH artifact failed with exit code $LASTEXITCODE"
    }
}
...
if `$_stage == 17
  if *(unsigned int*)(0x$statusAddress+$statusCommandSeqAckOffset) == `$_expected_seq && *(unsigned int*)0x$wakeQueueCountAddress == 3 && *(unsigned int*)(0x$wakeQueueAddress + (1 * $wakeEventStride) + $wakeEventTaskIdOffset) == `$_task3_id && *(unsigned char*)(0x$wakeQueueAddress + (1 * $wakeEventStride) + $wakeEventReasonOffset) == $wakeReasonInterrupt
    ...
    set *(unsigned int*)(0x$commandMailboxAddress+$commandSeqOffset) = (`$_expected_seq + 1)
    set *(unsigned long long*)(0x$commandMailboxAddress+$commandArg0Offset) = $wakeReasonInterrupt
    set *(unsigned long long*)(0x$commandMailboxAddress+$commandArg1Offset) = 8
    set `$_expected_seq = (`$_expected_seq + 1)
    set `$_stage = 18
  end
  continue
...
if `$_stage == 19
  if *(unsigned int*)(0x$statusAddress+$statusCommandSeqAckOffset) == `$_expected_seq && *(short*)(0x$statusAddress+$statusLastCommandResultOffset) == $resultInvalidArgument && *(unsigned int*)0x$wakeQueueCountAddress == 1
    ...
    set `$_final_vector1 = 0
    printf "AFTER_WAKE_QUEUE_REASON_POP\n"
...
if (Test-Path $gdbStdout) {
    ...
    $hitAfter = $gdbOutput.Contains("AFTER_WAKE_QUEUE_REASON_POP")
}
...
if ($timedOut) {
    throw "QEMU Wake-queue reason-pop probe timed out after $TimeoutSeconds seconds.`nSTDOUT:`n$gdbOutput`nSTDERR:`n$gdbError"
}
...
if (-not $hitStart -or -not $hitAfter) {
    throw "Wake-queue reason-pop probe did not reach expected checkpoints.`nSTDOUT:`n$gdbOutput`nSTDERR:`n$gdbError"
}
...
if ($finalTask1 -ne 0) { throw "Expected FINAL_TASK1=0, got $finalTask1" }
if ($finalVector0 -ne 0) { throw "Expected FINAL_VECTOR0=0, got $finalVector0" }
if ($finalVector1 -ne 0) { throw "Expected FINAL_VECTOR1=0, got $finalVector1" }
...
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_ARTIFACT=$artifact"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_START_ADDR=0x$startAddress"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_SPINPAUSE_ADDR=0x$spinPauseAddress"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_STATUS_ADDR=0x$statusAddress"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_MAILBOX_ADDR=0x$commandMailboxAddress"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_TIMED_OUT=$timedOut"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_GDB_STDOUT=$gdbStdout"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_GDB_STDERR=$gdbStderr"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_QEMU_STDOUT=$qemuStdout"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_QEMU_STDERR=$qemuStderr"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_ACK=$ack"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_LAST_OPCODE=$lastOpcode"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_LAST_RESULT=$lastResult"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_TICKS=$ticks"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_TASK1_ID=$task1Id"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_TASK2_ID=$task2Id"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_TASK3_ID=$task3Id"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_TASK4_ID=$task4Id"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_PRE_COUNT=$preCount"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_PRE_TASK0=$preTask0"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_PRE_TASK1=$preTask1"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_PRE_TASK2=$preTask2"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_PRE_TASK3=$preTask3"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_PRE_VECTOR0=$preVector0"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_PRE_VECTOR1=$preVector1"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_PRE_VECTOR2=$preVector2"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_PRE_VECTOR3=$preVector3"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_MID_COUNT=$midCount"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_MID_TASK0=$midTask0"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_MID_TASK1=$midTask1"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_MID_TASK2=$midTask2"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_MID_VECTOR0=$midVector0"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_MID_VECTOR1=$midVector1"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_MID_VECTOR2=$midVector2"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_POST_COUNT=$postCount"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_POST_TASK0=$postTask0"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_POST_TASK1=$postTask1"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_POST_VECTOR0=$postVector0"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_POST_VECTOR1=$postVector1"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_FINAL_COUNT=$finalCount"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_FINAL_TASK0=$finalTask0"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_FINAL_TASK1=$finalTask1"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_FINAL_VECTOR0=$finalVector0"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_FINAL_VECTOR1=$finalVector1"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_REASON_POP_PROBE=pass"
