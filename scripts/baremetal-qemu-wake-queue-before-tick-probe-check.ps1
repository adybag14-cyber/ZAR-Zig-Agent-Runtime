...
if ($null -eq $qemu) {
    ...
    Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_PROBE=skipped"
    return
}
if ($null -eq $gdb) {
...
$optionsPath = Join-Path $releaseDir "qemu-wake-queue-before-tick-options.zig"
$mainObj = Join-Path $releaseDir "openclaw-zig-baremetal-main-wake-queue-before-tick.o"
$bootObj = Join-Path $releaseDir "openclaw-zig-pvh-boot-wake-queue-before-tick.o"
$artifact = Join-Path $releaseDir "openclaw-zig-baremetal-pvh-wake-queue-before-tick.elf"
$bootSource = Join-Path $repo "scripts\baremetal\pvh_boot.S"
$linkerScript = Join-Path $repo "scripts\baremetal\pvh_lld.ld"
$gdbScript = Join-Path $releaseDir "qemu-wake-queue-before-tick-$runStamp.gdb"
$gdbStdout = Join-Path $releaseDir "qemu-wake-queue-before-tick-$runStamp.gdb.stdout.log"
$gdbStderr = Join-Path $releaseDir "qemu-wake-queue-before-tick-$runStamp.gdb.stderr.log"
$qemuStdout = Join-Path $releaseDir "qemu-wake-queue-before-tick-$runStamp.qemu.stdout.log"
$qemuStderr = Join-Path $releaseDir "qemu-wake-queue-before-tick-$runStamp.qemu.stderr.log"
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj `
        ...
        --name "openclaw-zig-baremetal-main-wake-queue-before-tick" `
        "-femit-bin=$mainObj"
    if ($LASTEXITCODE -ne 0) {
        throw "zig build-obj for wake-queue before-tick runtime failed with exit code $LASTEXITCODE"
    }

    & $clang -c -target x86_64-unknown-elf $bootSource -o $bootObj
    if ($LASTEXITCODE -ne 0) {
        throw "clang assemble for wake-queue before-tick PVH shim failed with exit code $LASTEXITCODE"
    }

    & $lld -flavor gnu -m elf_x86_64 -o $artifact -T $linkerScript $mainObj $bootObj $compilerRt
    if ($LASTEXITCODE -ne 0) {
        throw "lld link for wake-queue before-tick PVH artifact failed with exit code $LASTEXITCODE"
    }
}
...
if `$_stage == 16
  if *(unsigned int*)(0x$statusAddress+$statusCommandSeqAckOffset) == `$_expected_seq && *(unsigned int*)0x$wakeQueueCountAddress == 4 && *(unsigned int*)(0x$wakeQueueAddress + (3 * $wakeEventStride) + $wakeEventTaskIdOffset) == `$_task4_id && *(unsigned char*)(0x$wakeQueueAddress + (3 * $wakeEventStride) + $wakeEventReasonOffset) == $wakeReasonInterrupt && *(unsigned char*)(0x$wakeQueueAddress + (3 * $wakeEventStride) + $wakeEventVectorOffset) == $interruptVectorB
    ...
    set `$_pre_tick0 = *(unsigned long long*)(0x$wakeQueueAddress + (0 * $wakeEventStride) + $wakeEventTickOffset)
    set `$_pre_tick1 = *(unsigned long long*)(0x$wakeQueueAddress + (1 * $wakeEventStride) + $wakeEventTickOffset)
    set `$_pre_tick2 = *(unsigned long long*)(0x$wakeQueueAddress + (2 * $wakeEventStride) + $wakeEventTickOffset)
    set `$_pre_tick3 = *(unsigned long long*)(0x$wakeQueueAddress + (3 * $wakeEventStride) + $wakeEventTickOffset)
    set *(unsigned short*)(0x$commandMailboxAddress+$commandOpcodeOffset) = $wakeQueuePopBeforeTickOpcode
    set *(unsigned int*)(0x$commandMailboxAddress+$commandSeqOffset) = (`$_expected_seq + 1)
    set *(unsigned long long*)(0x$commandMailboxAddress+$commandArg0Offset) = `$_pre_tick0
    set *(unsigned long long*)(0x$commandMailboxAddress+$commandArg1Offset) = 0
    set `$_expected_seq = (`$_expected_seq + 1)
    set `$_stage = 17
  end
...
if `$_stage == 17
  if *(unsigned int*)(0x$statusAddress+$statusCommandSeqAckOffset) == `$_expected_seq && *(unsigned int*)0x$wakeQueueCountAddress == 3 && *(unsigned int*)(0x$wakeQueueAddress + (0 * $wakeEventStride) + $wakeEventTaskIdOffset) == `$_task2_id && *(unsigned char*)(0x$wakeQueueAddress + (0 * $wakeEventStride) + $wakeEventVectorOffset) == $interruptVectorA
    set `$_mid_count = *(unsigned int*)0x$wakeQueueCountAddress
    set `$_mid_task1 = *(unsigned int*)(0x$wakeQueueAddress + (0 * $wakeEventStride) + $wakeEventTaskIdOffset)
    set `$_mid_vector1 = *(unsigned char*)(0x$wakeQueueAddress + (0 * $wakeEventStride) + $wakeEventVectorOffset)
    set `$_mid_tick0 = *(unsigned long long*)(0x$wakeQueueAddress + (0 * $wakeEventStride) + $wakeEventTickOffset)
    set *(unsigned short*)(0x$commandMailboxAddress+$commandOpcodeOffset) = $wakeQueuePopBeforeTickOpcode
    set *(unsigned int*)(0x$commandMailboxAddress+$commandSeqOffset) = (`$_expected_seq + 1)
    set *(unsigned long long*)(0x$commandMailboxAddress+$commandArg0Offset) = `$_pre_tick2
...
if `$_stage == 18
  if *(unsigned int*)(0x$statusAddress+$statusCommandSeqAckOffset) == `$_expected_seq && *(unsigned int*)0x$wakeQueueCountAddress == 1 && *(unsigned int*)(0x$wakeQueueAddress + (0 * $wakeEventStride) + $wakeEventTaskIdOffset) == `$_task4_id
    ...
    set `$_post_vector1 = 0
    set `$_post_tick0 = *(unsigned long long*)(0x$wakeQueueAddress + (0 * $wakeEventStride) + $wakeEventTickOffset)
    set *(unsigned short*)(0x$commandMailboxAddress+$commandOpcodeOffset) = $wakeQueuePopBeforeTickOpcode
    set *(unsigned int*)(0x$commandMailboxAddress+$commandSeqOffset) = (`$_expected_seq + 1)
    set *(unsigned long long*)(0x$commandMailboxAddress+$commandArg0Offset) = `$_pre_tick2
...
if (Test-Path $gdbStdout) {
    ...
    $hitAfter = $gdbOutput.Contains("AFTER_WAKE_QUEUE_BEFORE_TICK")
}
...
if ($timedOut) {
    throw "QEMU Wake-queue before-tick probe timed out after $TimeoutSeconds seconds.`nSTDOUT:`n$gdbOutput`nSTDERR:`n$gdbError"
}
...
if (-not $hitStart -or -not $hitAfter) {
    throw "Wake-queue before-tick probe did not reach expected checkpoints.`nSTDOUT:`n$gdbOutput`nSTDERR:`n$gdbError"
}
...
$preTick0 = Extract-IntValue -Text $gdbOutput -Name "PRE_TICK0"
$preTick1 = Extract-IntValue -Text $gdbOutput -Name "PRE_TICK1"
$preTick2 = Extract-IntValue -Text $gdbOutput -Name "PRE_TICK2"
$preTick3 = Extract-IntValue -Text $gdbOutput -Name "PRE_TICK3"
...
$midTick0 = Extract-IntValue -Text $gdbOutput -Name "MID_TICK0"
...
if (-not ($preTick0 -lt $preTick1 -and $preTick1 -lt $preTick2 -and $preTick2 -lt $preTick3)) { throw "Expected monotonic PRE_TICK sequence, got $preTick0,$preTick1,$preTick2,$preTick3" }
...
if ($postTask0 -ne $task4Id) { throw "Expected POST_TASK0=$task4Id, got $postTask0" }
if ($postTask1 -ne 0) { throw "Expected POST_TASK1=0, got $postTask1" }
...
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_ARTIFACT=$artifact"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_START_ADDR=0x$startAddress"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_SPINPAUSE_ADDR=0x$spinPauseAddress"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_STATUS_ADDR=0x$statusAddress"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_MAILBOX_ADDR=0x$commandMailboxAddress"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_TIMED_OUT=$timedOut"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_GDB_STDOUT=$gdbStdout"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_GDB_STDERR=$gdbStderr"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_QEMU_STDOUT=$qemuStdout"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_QEMU_STDERR=$qemuStderr"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_ACK=$ack"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_LAST_OPCODE=$lastOpcode"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_LAST_RESULT=$lastResult"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_TICKS=$ticks"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_TASK1_ID=$task1Id"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_TASK2_ID=$task2Id"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_TASK3_ID=$task3Id"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_TASK4_ID=$task4Id"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_PRE_COUNT=$preCount"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_PRE_TASK0=$preTask0"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_PRE_TASK1=$preTask1"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_PRE_TASK2=$preTask2"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_PRE_TASK3=$preTask3"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_PRE_VECTOR0=$preVector0"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_PRE_VECTOR1=$preVector1"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_PRE_VECTOR2=$preVector2"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_PRE_VECTOR3=$preVector3"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_PRE_TICK0=$preTick0"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_PRE_TICK1=$preTick1"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_PRE_TICK2=$preTick2"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_PRE_TICK3=$preTick3"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_MID_COUNT=$midCount"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_MID_TASK1=$midTask1"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_MID_VECTOR1=$midVector1"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_MID_TICK0=$midTick0"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_POST_COUNT=$postCount"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_POST_TASK0=$postTask0"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_BEFORE_TICK_POST_TASK1=$postTask1"
