...
param(
    ...
    [int] $MemoryMiB = 128,
    [Parameter(Mandatory = $true)][string] $ScriptStem,
    [Parameter(Mandatory = $true)][string] $ProbeSlug,
...
if ($null -eq $qemu) {
    ...
    Write-Output ("{0}=skipped" -f $ReceiptKey)
    return
}
...
if (-not $SkipBuild) {
    & $zig build baremetal-i386 -Doptimize=ReleaseFast ("-D{0}=true" -f $BuildFlag) --prefix $buildPrefix --summary all
    if ($LASTEXITCODE -ne 0) {
        throw "zig build baremetal-i386 -D$BuildFlag=true failed with exit code $LASTEXITCODE"
    }
}
...
$stdoutPath = Join-Path $releaseDir ("qemu-i386-firmware-{0}-{1}m-stdout.log" -f $ProbeSlug, $MemoryMiB)
$stderrPath = Join-Path $releaseDir ("qemu-i386-firmware-{0}-{1}m-stderr.log" -f $ProbeSlug, $MemoryMiB)
$debugLogPath = Join-Path $releaseDir ("qemu-i386-firmware-{0}-{1}m-debug.log" -f $ProbeSlug, $MemoryMiB)
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    if ([string]::IsNullOrWhiteSpace($debugTail)) {
        throw "QEMU i386 firmware $ProbeLabel timed out after $TimeoutSeconds seconds."
    }
    throw "QEMU i386 firmware $ProbeLabel timed out after $TimeoutSeconds seconds.`n$debugTail"
}
...
if (-not $expectedExitCodes.ContainsKey($exitCode)) {
    ...
    throw "QEMU i386 firmware $ProbeLabel failed: exit=$exitCode expected one of [$expectedText]`n$stderrTail`n$debugTail"
}
...
Write-Output ("{0}={1}" -f $QemuDebugReceiptKey, $debugLogPath)
Write-Output ("{0}=pass" -f $ReceiptKey)
