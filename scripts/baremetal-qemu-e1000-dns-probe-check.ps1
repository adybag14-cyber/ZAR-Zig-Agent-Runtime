...
if ($null -eq $qemu) {
    ...
    Write-Output 'BAREMETAL_QEMU_E1000_DNS_PROBE=skipped'
    return
}
...
$optionsPath = Join-Path $releaseDir 'qemu-e1000-dns-probe-options.zig'
$mainObj = Join-Path $releaseDir 'openclaw-zig-baremetal-main-e1000-dns-probe.o'
$bootObj = Join-Path $releaseDir 'openclaw-zig-pvh-boot-e1000-dns-probe.o'
$artifact = Join-Path $releaseDir 'openclaw-zig-baremetal-pvh-e1000-dns-probe.elf'
$bootSource = Join-Path $repo 'scripts\baremetal\pvh_boot.S'
$linkerScript = Join-Path $repo 'scripts\baremetal\pvh_lld.ld'
$stdoutPath = Join-Path $releaseDir 'qemu-e1000-dns-probe.stdout.log'
$stderrPath = Join-Path $releaseDir 'qemu-e1000-dns-probe.stderr.log'
...
pub const e1000_dns_probe: bool = true;
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj `
        ...
        --name 'openclaw-zig-baremetal-main-e1000-dns-probe' `
        "-femit-bin=$mainObj"
    if ($LASTEXITCODE -ne 0) {
        throw "zig build-obj for E1000 DNS probe failed with exit code $LASTEXITCODE"
    }
...
if (-not (Test-Path $artifact)) {
    throw "E1000 DNS probe artifact is missing: $artifact"
}
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "QEMU E1000 DNS probe timed out after $TimeoutSeconds seconds."
}
...
if ($exitCode -ne $expectedExitCode) {
    ...
    throw ("QEMU E1000 DNS probe failed with exit code {0} (probe code 0x{1:X2})." -f $exitCode, $probeCode)
}

Write-Output 'BAREMETAL_QEMU_AVAILABLE=True'
Write-Output "BAREMETAL_QEMU_BINARY=$qemu"
Write-Output 'BAREMETAL_QEMU_E1000_DNS_PROBE=pass'
Write-Output ("BAREMETAL_QEMU_E1000_DNS_PROBE_CODE=0x{0:X2}" -f $expectedProbeCode)
Write-Output "BAREMETAL_QEMU_E1000_DNS_STDOUT=$stdoutPath"
Write-Output "BAREMETAL_QEMU_E1000_DNS_STDERR=$stderrPath"
