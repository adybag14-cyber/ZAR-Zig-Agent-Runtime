...
if ($null -eq $qemu) {
    ...
    Write-Output 'BAREMETAL_QEMU_VIRTIO_BLOCK_INSTALLER_PROBE=skipped'
    return
}
...
pub const virtio_block_installer_probe: bool = true;
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj `
        ...
        --name 'openclaw-zig-baremetal-main-virtio-block-installer-probe' `
        "-femit-bin=$mainObj"
    if ($LASTEXITCODE -ne 0) {
        throw "zig build-obj for virtio-block installer probe failed with exit code $LASTEXITCODE"
    }

    & $clang -c -target x86_64-unknown-elf $bootSource -o $bootObj
    if ($LASTEXITCODE -ne 0) {
        throw "clang assemble for virtio-block installer PVH shim failed with exit code $LASTEXITCODE"
    }
    ...
    & $zig build-exe `
        ...
        "-femit-bin=$artifact"
    if ($LASTEXITCODE -ne 0) {
        throw "zig final link for virtio-block installer PVH artifact failed with exit code $LASTEXITCODE"
    }
}

if (-not (Test-Path $artifact)) {
    throw "Virtio-block installer probe artifact is missing: $artifact"
}
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "QEMU virtio-block installer probe timed out after $TimeoutSeconds seconds."
}
...
if ($exitCode -ne $expectedExitCode) {
    ...
    throw ("QEMU virtio-block installer probe failed with exit code {0} (probe code 0x{1:X2})." -f $exitCode, $probeCode)
}
...
if ($toolMagic -ne $toolLayoutMagic) {
    throw ("Virtio-block installer tool-layout superblock magic mismatch. Expected 0x{0:X8}, got 0x{1:X8}." -f $toolLayoutMagic, $toolMagic)
}
if ($fsMagic -ne $filesystemMagic) {
    throw ("Virtio-block installer filesystem superblock magic mismatch. Expected 0x{0:X8}, got 0x{1:X8}." -f $filesystemMagic, $fsMagic)
}
if (-not $imageText.Contains($expectedLoaderMarker)) {
    throw "Virtio-block installer image does not contain expected loader marker '$expectedLoaderMarker'."
}
if (-not $imageText.Contains($expectedBootstrapMarker)) {
    throw "Virtio-block installer image does not contain expected bootstrap marker '$expectedBootstrapMarker'."
}

Write-Output 'BAREMETAL_QEMU_AVAILABLE=True'
Write-Output "BAREMETAL_QEMU_BINARY=$qemu"
Write-Output 'BAREMETAL_QEMU_VIRTIO_BLOCK_INSTALLER_PROBE=pass'
Write-Output "BAREMETAL_QEMU_VIRTIO_BLOCK_INSTALLER_IMAGE=$diskImage"
