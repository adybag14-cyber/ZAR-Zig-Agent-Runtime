...
$zigLocalCacheDir = if ($env:ZIG_LOCAL_CACHE_DIR -and $env:ZIG_LOCAL_CACHE_DIR.Trim().Length -gt 0) { $env:ZIG_LOCAL_CACHE_DIR } else { Join-Path $repo '.zig-cache-e1000-tool-service-probe' }

if ($null -eq $qemu) {
    Write-Output 'BAREMETAL_QEMU_AVAILABLE=False'
    Write-Output 'BAREMETAL_QEMU_E1000_TOOL_SERVICE_PROBE=skipped'
    return
}
...
$optionsPath = Join-Path $releaseDir 'qemu-e1000-tool-service-probe-options.zig'
$mainObj = Join-Path $releaseDir 'openclaw-zig-baremetal-main-e1000-tool-service-probe.o'
$bootObj = Join-Path $releaseDir 'openclaw-zig-pvh-boot-e1000-tool-service-probe.o'
$artifact = Join-Path $releaseDir 'openclaw-zig-baremetal-pvh-e1000-tool-service-probe.elf'
$diskImage = Join-Path $releaseDir 'qemu-e1000-tool-service-probe.img'
$bootSource = Join-Path $repo 'scripts\baremetal\pvh_boot.S'
$linkerScript = Join-Path $repo 'scripts\baremetal\pvh_lld.ld'
$stdoutPath = Join-Path $releaseDir 'qemu-e1000-tool-service-probe.stdout.log'
$stderrPath = Join-Path $releaseDir 'qemu-e1000-tool-service-probe.stderr.log'
$debugLogPath = Join-Path $releaseDir 'qemu-e1000-tool-service-probe.debug.log'
...
pub const e1000_tool_service_probe: bool = true;
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj `
        ...
        --name 'openclaw-zig-baremetal-main-e1000-tool-service-probe' `
        "-femit-bin=$mainObj"
    if ($LASTEXITCODE -ne 0) {
        throw "zig build-obj for E1000 tool-service probe failed with exit code $LASTEXITCODE"
    }

    & $clang -c -target x86_64-unknown-elf $bootSource -o $bootObj
    if ($LASTEXITCODE -ne 0) {
        throw "clang assemble for E1000 tool-service PVH shim failed with exit code $LASTEXITCODE"
    }

    & $lld -flavor gnu -m elf_x86_64 -o $artifact -T $linkerScript $mainObj $bootObj $compilerRt
    if ($LASTEXITCODE -ne 0) {
        throw "lld link for E1000 tool-service PVH artifact failed with exit code $LASTEXITCODE"
    }
}

if (-not (Test-Path $artifact)) {
    throw "E1000 tool-service probe artifact is missing: $artifact"
}
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    if ([string]::IsNullOrEmpty($debugTail)) {
        throw "QEMU E1000 tool-service probe timed out after $TimeoutSeconds seconds."
    }
    $debugTail = $debugTail.Trim()
    throw "QEMU E1000 tool-service probe timed out after $TimeoutSeconds seconds. Last debug stages: $debugTail"
}
...
if ($exitCode -ne $expectedExitCode) {
    ...
    throw ("QEMU E1000 tool-service probe failed with exit code {0} (probe code 0x{1:X2})." -f $exitCode, $probeCode)
}

Write-Output 'BAREMETAL_QEMU_AVAILABLE=True'
Write-Output "BAREMETAL_QEMU_BINARY=$qemu"
Write-Output 'BAREMETAL_QEMU_E1000_TOOL_SERVICE_PROBE=pass'
Write-Output ("BAREMETAL_QEMU_E1000_TOOL_SERVICE_PROBE_CODE=0x{0:X2}" -f $expectedProbeCode)
Write-Output "BAREMETAL_QEMU_E1000_TOOL_SERVICE_STDOUT=$stdoutPath"
Write-Output "BAREMETAL_QEMU_E1000_TOOL_SERVICE_STDERR=$stderrPath"
Write-Output "BAREMETAL_QEMU_E1000_TOOL_SERVICE_DEBUG=$debugLogPath"
