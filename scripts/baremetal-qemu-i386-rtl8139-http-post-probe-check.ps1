...
if ($null -eq $qemu) {
    ...
    Write-Output 'BAREMETAL_I386_QEMU_RTL8139_HTTP_POST_PROBE=skipped'
    return
}

if (-not $SkipBuild) {
    & $zig build baremetal-i386 -Doptimize=ReleaseFast -Dbaremetal-rtl8139-http-post-probe=true --summary all
...
if ($null -eq $artifact) { throw 'i386 RTL8139 HTTP POST artifact not found after build.' }

$diskImage = Join-Path $releaseDir 'qemu-i386-rtl8139-http-post-probe.img'
$stdoutPath = Join-Path $releaseDir 'qemu-i386-rtl8139-http-post-probe.stdout.log'
$stderrPath = Join-Path $releaseDir 'qemu-i386-rtl8139-http-post-probe.stderr.log'
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "QEMU i386 RTL8139 HTTP POST probe timed out after $TimeoutSeconds seconds."
}
$proc.WaitForExit()
...
if ($exitCode -ne $expectedExitCode) {
    ...
    throw ("QEMU i386 RTL8139 HTTP POST probe failed with exit code {0} (probe code 0x{1:X2})." -f $exitCode, $probeCode)
}

Write-Output 'BAREMETAL_I386_QEMU_AVAILABLE=True'
Write-Output "BAREMETAL_I386_QEMU_BINARY=$qemu"
Write-Output 'BAREMETAL_I386_QEMU_RTL8139_HTTP_POST_PROBE=pass'
Write-Output ("BAREMETAL_I386_QEMU_RTL8139_HTTP_POST_PROBE_CODE=0x{0:X2}" -f $expectedProbeCode)
Write-Output "BAREMETAL_I386_QEMU_RTL8139_HTTP_POST_STDOUT=$stdoutPath"
Write-Output "BAREMETAL_I386_QEMU_RTL8139_HTTP_POST_STDERR=$stderrPath"
