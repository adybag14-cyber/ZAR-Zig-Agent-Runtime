...
if ($null -eq $qemu -or $null -eq $gdb) {
    ...
    Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE=skipped"
    return
}
...
$optionsPath = Join-Path $releaseDir "qemu-wake-queue-selective-probe-options.zig"
$rootModulePath = (Join-Path $repo "src/baremetal_main.zig").Replace('\\', '/')
$optionsModulePath = $optionsPath.Replace('\\', '/')
$mainObj = Join-Path $releaseDir "openclaw-zig-baremetal-main-wake-queue-selective-probe.o"
$bootObj = Join-Path $releaseDir "openclaw-zig-pvh-boot-wake-queue-selective-probe.o"
...
$gdbScript = Join-Path $releaseDir "qemu-wake-queue-selective-probe.gdb"
$gdbStdout = Join-Path $releaseDir "qemu-wake-queue-selective-probe.gdb.stdout.log"
$gdbStderr = Join-Path $releaseDir "qemu-wake-queue-selective-probe.gdb.stderr.log"
$qemuStdout = Join-Path $releaseDir "qemu-wake-queue-selective-probe.qemu.stdout.log"
$qemuStderr = Join-Path $releaseDir "qemu-wake-queue-selective-probe.qemu.stderr.log"
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj `
        ...
        --name "openclaw-zig-baremetal-main-wake-queue-selective-probe" `
        "-femit-bin=$mainObj"
    if ($LASTEXITCODE -ne 0) {
        throw "zig build-obj for wake-queue-selective probe runtime failed with exit code $LASTEXITCODE"
    }

    & $clang -c -target x86_64-unknown-elf $bootSource -o $bootObj
    if ($LASTEXITCODE -ne 0) {
        throw "clang assemble for wake-queue-selective probe PVH shim failed with exit code $LASTEXITCODE"
    }

    & $lld -flavor gnu -m elf_x86_64 -o $artifact -T $linkerScript $mainObj $bootObj $compilerRt
    if ($LASTEXITCODE -ne 0) {
        throw "lld link for wake-queue-selective probe PVH artifact failed with exit code $LASTEXITCODE"
    }
}
...
if `$stage == 19
  if *(unsigned int*)(0x$wakeQueueCountAddress) == 5 && *(unsigned int*)(0x$wakeQueueAddress+$wakeEventTaskIdOffset) == $taskIdTimer && *(unsigned int*)(0x$wakeQueueAddress+($wakeEventStride*1)+$wakeEventTaskIdOffset) == $taskIdInterruptA1 && *(unsigned int*)(0x$wakeQueueAddress+($wakeEventStride*2)+$wakeEventTaskIdOffset) == $taskIdInterruptA2 && *(unsigned int*)(0x$wakeQueueAddress+($wakeEventStride*3)+$wakeEventTaskIdOffset) == $taskIdInterruptB && *(unsigned int*)(0x$wakeQueueAddress+($wakeEventStride*4)+$wakeEventTaskIdOffset) == $taskIdManual && *(unsigned char*)(0x$wakeQueueAddress+($wakeEventStride*4)+$wakeEventReasonOffset) == $wakeReasonManual
...
if `$stage == 20
  if *(unsigned int*)(0x$statusAddress+$statusCommandSeqAckOffset) == 20 && *(unsigned int*)(0x$wakeQueueCountAddress) == 4 && *(unsigned int*)(0x$wakeQueueAddress+($wakeEventStride*1)+$wakeEventTaskIdOffset) == $taskIdInterruptA2
    set `$post_reason_len = *(unsigned int*)(0x$wakeQueueCountAddress)
    set `$post_reason_task1 = *(unsigned int*)(0x$wakeQueueAddress+($wakeEventStride*1)+$wakeEventTaskIdOffset)
...
if `$stage == 22
  if *(unsigned int*)(0x$statusAddress+$statusCommandSeqAckOffset) == 22 && *(unsigned int*)(0x$wakeQueueCountAddress) == 2 && *(unsigned int*)(0x$wakeQueueAddress+$wakeEventTaskIdOffset) == $taskIdTimer && *(unsigned int*)(0x$wakeQueueAddress+($wakeEventStride*1)+$wakeEventTaskIdOffset) == $taskIdManual
...
printf "AFTER_WAKE_QUEUE_SELECTIVE\n"
...
$hitStart = $false
$hitAfterWakeQueueSelective = $false
$ack = $null
$lastOpcode = $null
$lastResult = $null
$ticks = $null
...
if (Test-Path $gdbStdout) {
    ...
    $hitAfterWakeQueueSelective = $gdbOutput.Contains("AFTER_WAKE_QUEUE_SELECTIVE")
...
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_START_ADDR=0x$startAddress"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_SPINPAUSE_ADDR=0x$spinPauseAddress"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_STATUS_ADDR=0x$statusAddress"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_MAILBOX_ADDR=0x$commandMailboxAddress"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_WAKE_QUEUE_ADDR=0x$wakeQueueAddress"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_WAKE_QUEUE_COUNT_ADDR=0x$wakeQueueCountAddress"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_TIMER_STATE_ADDR=0x$timerStateAddress"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_HIT_START=$hitStart"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_HIT_AFTER_SELECTIVE=$hitAfterWakeQueueSelective"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_ACK=$ack"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_LAST_OPCODE=$lastOpcode"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_LAST_RESULT=$lastResult"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_TICKS=$ticks"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_MAILBOX_OPCODE=$mailboxOpcode"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_MAILBOX_SEQ=$mailboxSeq"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_PRE_LEN=$preLen"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_PRE_TASK0=$preTask0"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_PRE_TASK1=$preTask1"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_PRE_TASK2=$preTask2"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_PRE_TASK3=$preTask3"
...
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_WAKE0_TASK_ID=$wake0TaskId"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_WAKE0_TIMER_ID=$wake0TimerId"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_WAKE0_REASON=$wake0Reason"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_WAKE0_VECTOR=$wake0Vector"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_WAKE0_TICK=$wake0Tick"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_GDB_STDOUT=$gdbStdout"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_GDB_STDERR=$gdbStderr"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_QEMU_STDOUT=$qemuStdout"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_QEMU_STDERR=$qemuStderr"
Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE_TIMED_OUT=$timedOut"
...
$probePassed = $hitStart -and
    ...
    ($wake0Tick -gt $beforeTickCutoff) -and
    ($ticks -ge ($wake0Tick + $postDrainSlackTicks)) -and
    ($wake0Seq -ge 5)

Write-Output "BAREMETAL_QEMU_WAKE_QUEUE_SELECTIVE_PROBE=$($(if ($probePassed) { 'pass' } else { 'fail' }))"
