...
if ($null -eq $qemu -or $null -eq $gdb) {
    ...
    Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE=skipped"
    return
}
...
$optionsPath = Join-Path $releaseDir "qemu-task-resume-interrupt-timeout-probe-options.zig"
$mainObj = Join-Path $releaseDir "openclaw-zig-baremetal-main-task-resume-interrupt-timeout-probe.o"
$bootObj = Join-Path $releaseDir "openclaw-zig-pvh-boot-task-resume-interrupt-timeout-probe.o"
$artifact = Join-Path $releaseDir "openclaw-zig-baremetal-pvh-task-resume-interrupt-timeout-probe.elf"
$bootSource = Join-Path $repo "scripts\baremetal\pvh_boot.S"
$linkerScript = Join-Path $repo "scripts\baremetal\pvh_lld.ld"
$gdbScript = Join-Path $releaseDir "qemu-task-resume-interrupt-timeout-probe.gdb"
$gdbStdout = Join-Path $releaseDir "qemu-task-resume-interrupt-timeout-probe.gdb.stdout.log"
$gdbStderr = Join-Path $releaseDir "qemu-task-resume-interrupt-timeout-probe.gdb.stderr.log"
$qemuStdout = Join-Path $releaseDir "qemu-task-resume-interrupt-timeout-probe.qemu.stdout.log"
$qemuStderr = Join-Path $releaseDir "qemu-task-resume-interrupt-timeout-probe.qemu.stderr.log"
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj `
        ...
        --name "openclaw-zig-baremetal-main-task-resume-interrupt-timeout-probe" `
        "-femit-bin=$mainObj"
...
printf "AFTER_TASK_RESUME_INTERRUPT_TIMEOUT\n"
...
if (Test-Path $gdbStdout) {
    ...
    $hitAfterTaskResumeInterruptTimeout = $gdbOutput.Contains("AFTER_TASK_RESUME_INTERRUPT_TIMEOUT")
...
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_START_ADDR=0x$startAddress"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_SPINPAUSE_ADDR=0x$spinPauseAddress"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_STATUS_ADDR=0x$statusAddress"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_MAILBOX_ADDR=0x$commandMailboxAddress"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_SCHEDULER_STATE_ADDR=0x$schedulerStateAddress"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_SCHEDULER_TASKS_ADDR=0x$schedulerTasksAddress"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_SCHEDULER_WAIT_KIND_ADDR=0x$schedulerWaitKindAddress"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_SCHEDULER_WAIT_INTERRUPT_VECTOR_ADDR=0x$schedulerWaitInterruptVectorAddress"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_SCHEDULER_WAIT_TIMEOUT_TICK_ADDR=0x$schedulerWaitTimeoutTickAddress"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_TIMER_STATE_ADDR=0x$timerStateAddress"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_WAKE_QUEUE_ADDR=0x$wakeQueueAddress"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_WAKE_QUEUE_COUNT_ADDR=0x$wakeQueueCountAddress"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_INTERRUPT_STATE_ADDR=0x$interruptStateAddress"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_HIT_START=$hitStart"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_HIT_AFTER=$hitAfterTaskResumeInterruptTimeout"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_ACK=$ack"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_LAST_OPCODE=$lastOpcode"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_LAST_RESULT=$lastResult"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_TICKS=$ticks"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_MAILBOX_OPCODE=$mailboxOpcode"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_MAILBOX_SEQ=$mailboxSeq"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_SCHED_TASK_COUNT=$schedTaskCount"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_TASK0_ID=$task0Id"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_TASK0_STATE=$task0State"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_TASK0_PRIORITY=$task0Priority"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_TASK0_RUN_COUNT=$task0RunCount"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_TASK0_BUDGET=$task0Budget"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_TASK0_BUDGET_REMAINING=$task0BudgetRemaining"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_WAIT_KIND0=$waitKind0"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_WAIT_VECTOR0=$waitVector0"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_WAIT_TIMEOUT0=$waitTimeout0"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_TIMER_ENABLED=$timerEnabled"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_TIMER_ENTRY_COUNT=$timerEntryCount"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_TIMER_PENDING_WAKE_COUNT=$timerPendingWakeCount"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_TIMER_NEXT_TIMER_ID=$timerNextTimerId"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_TIMER_DISPATCH_COUNT=$timerDispatchCount"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_TIMER_LAST_INTERRUPT_COUNT=$timerLastInterruptCount"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_TIMER_LAST_WAKE_TICK=$timerLastWakeTick"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_WAKE_QUEUE_COUNT=$wakeQueueCount"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_WAKE0_SEQ=$wake0Seq"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_WAKE0_TASK_ID=$wake0TaskId"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_WAKE0_TIMER_ID=$wake0TimerId"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_WAKE0_REASON=$wake0Reason"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_WAKE0_VECTOR=$wake0Vector"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_WAKE0_TICK=$wake0Tick"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_INTERRUPT_COUNT=$interruptCount"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_LAST_INTERRUPT_VECTOR=$lastInterruptVector"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_GDB_STDOUT=$gdbStdout"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_GDB_STDERR=$gdbStderr"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_QEMU_STDOUT=$qemuStdout"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_QEMU_STDERR=$qemuStderr"
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE_TIMED_OUT=$timedOut"

$probePassed = $hitStart -and
    $hitAfterTaskResumeInterruptTimeout -and
    (-not $timedOut) -and
    ($ack -eq 7) -and
    ($lastOpcode -eq $taskResumeOpcode) -and
    ($lastResult -eq 0) -and
    ($mailboxOpcode -eq $taskResumeOpcode) -and
    ($mailboxSeq -eq 7) -and
...
Write-Output "BAREMETAL_QEMU_TASK_RESUME_INTERRUPT_TIMEOUT_PROBE=$($(if ($probePassed) { 'pass' } else { 'fail' }))"
