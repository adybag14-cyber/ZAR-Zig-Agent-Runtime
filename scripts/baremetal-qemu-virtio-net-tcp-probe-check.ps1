...
$zigLocalCacheDir = if ($env:ZIG_LOCAL_CACHE_DIR -and $env:ZIG_LOCAL_CACHE_DIR.Trim().Length -gt 0) { $env:ZIG_LOCAL_CACHE_DIR } else { Join-Path $repo '.zig-cache-virtio-net-tcp-probe' }
...
if ($null -eq $qemu) {
    ...
    Write-Output 'BAREMETAL_QEMU_VIRTIO_NET_TCP_PROBE=skipped'
    return
}
...
$diskImage = Join-Path $releaseDir 'qemu-virtio-net-tcp-probe.img'
$bootSource = Join-Path $repo 'scripts\baremetal\pvh_boot.S'
$linkerScript = Join-Path $repo 'scripts\baremetal\pvh_lld.ld'
$stdoutPath = Join-Path $releaseDir 'qemu-virtio-net-tcp-probe.stdout.log'
$stderrPath = Join-Path $releaseDir 'qemu-virtio-net-tcp-probe.stderr.log'
$debugLogPath = Join-Path $releaseDir 'qemu-virtio-net-tcp-probe.debug.log'
...
pub const virtio_net_tcp_probe: bool = true;
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "zig build-obj for VIRTIO-NET TCP probe failed with exit code $LASTEXITCODE"
    }
...
if (-not (Test-Path $artifact)) {
    throw "VIRTIO-NET TCP probe artifact is missing: $artifact"
}
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    if ([string]::IsNullOrEmpty($debugTail)) {
        throw "QEMU VIRTIO-NET TCP probe timed out after $TimeoutSeconds seconds."
    }
    $debugTail = $debugTail.Trim()
    throw "QEMU VIRTIO-NET TCP probe timed out after $TimeoutSeconds seconds. Last debug stages: $debugTail"
}
...
if ($exitCode -ne $expectedExitCode) {
    ...
    throw ("QEMU VIRTIO-NET TCP probe failed with exit code {0} (probe code 0x{1:X2})." -f $exitCode, $probeCode)
}

Write-Output 'BAREMETAL_QEMU_AVAILABLE=True'
Write-Output "BAREMETAL_QEMU_BINARY=$qemu"
Write-Output 'BAREMETAL_QEMU_VIRTIO_NET_TCP_PROBE=pass'
Write-Output ("BAREMETAL_QEMU_VIRTIO_NET_TCP_PROBE_CODE=0x{0:X2}" -f $expectedProbeCode)
Write-Output "BAREMETAL_QEMU_VIRTIO_NET_TCP_STDOUT=$stdoutPath"
Write-Output "BAREMETAL_QEMU_VIRTIO_NET_TCP_STDERR=$stderrPath"
Write-Output "BAREMETAL_QEMU_VIRTIO_NET_TCP_DEBUG=$debugLogPath"
