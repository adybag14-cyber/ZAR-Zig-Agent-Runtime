...
if ($null -eq $qemu) {
    ...
    Write-Output "BAREMETAL_I386_FIRMWARE_SMP_PRIORITY_QUOTA_PROBE=skipped"
    return
}
...
if (-not $SkipBuild) {
    & $zig build baremetal-i386 -Doptimize=ReleaseFast -Dbaremetal-i386-smp-priority-quota-probe=true --prefix $buildPrefix --summary all
    if ($LASTEXITCODE -ne 0) {
        throw "zig build baremetal-i386 -Dbaremetal-i386-smp-priority-quota-probe=true failed with exit code $LASTEXITCODE"
    }
}
...
if ($null -eq $artifact) {
    throw "i386 bare-metal firmware SMP priority quota artifact not found after build."
}

$firmwareImage = Join-Path $releaseDir ("qemu-i386-firmware-smp-priority-quota-probe-{0}m.img" -f $MemoryMiB)
$firmwareMetadata = Join-Path $releaseDir ("qemu-i386-firmware-smp-priority-quota-probe-{0}m.meta.txt" -f $MemoryMiB)
$stdoutPath = Join-Path $releaseDir ("qemu-i386-firmware-smp-priority-quota-probe-{0}m-stdout.log" -f $MemoryMiB)
$stderrPath = Join-Path $releaseDir ("qemu-i386-firmware-smp-priority-quota-probe-{0}m-stderr.log" -f $MemoryMiB)
$debugLogPath = Join-Path $releaseDir ("qemu-i386-firmware-smp-priority-quota-probe-{0}m-debug.log" -f $MemoryMiB)
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "QEMU i386 firmware SMP priority quota probe timed out after $TimeoutSeconds seconds."
    }
    throw "QEMU i386 firmware SMP priority quota probe timed out after $TimeoutSeconds seconds.`n$debugTail"
}
...
if (-not $expectedExitCodes.ContainsKey($exitCode)) {
    ...
    throw "QEMU i386 firmware SMP priority quota probe failed: exit=$exitCode expected one of [$expectedText]`n$stderrTail`n$debugTail"
}
...
Write-Output ("BAREMETAL_I386_QEMU_FIRMWARE_SMP_PRIORITY_QUOTA_PROBE_CODE=0x{0:X2}" -f $observedProbeCode)
Write-Output "BAREMETAL_I386_QEMU_FIRMWARE_SMP_PRIORITY_QUOTA_PROBE_DEBUG=$debugLogPath"
Write-Output "BAREMETAL_I386_FIRMWARE_SMP_PRIORITY_QUOTA_PROBE=pass"
