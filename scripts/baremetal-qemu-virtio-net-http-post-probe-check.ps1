...
$zigLocalCacheDir = if ($env:ZIG_LOCAL_CACHE_DIR -and $env:ZIG_LOCAL_CACHE_DIR.Trim().Length -gt 0) { $env:ZIG_LOCAL_CACHE_DIR } else { Join-Path $repo '.zig-cache-virtio-net-http-post-probe' }
...
if ($null -eq $qemu) {
    ...
    Write-Output 'BAREMETAL_QEMU_VIRTIO_NET_HTTP_POST_PROBE=skipped'
    return
}
...
$optionsPath = Join-Path $releaseDir 'qemu-virtio-net-http-post-probe-options.zig'
$mainObj = Join-Path $releaseDir 'openclaw-zig-baremetal-main-virtio-net-http-post-probe.o'
$bootObj = Join-Path $releaseDir 'openclaw-zig-pvh-boot-virtio-net-http-post-probe.o'
$artifact = Join-Path $releaseDir 'openclaw-zig-baremetal-pvh-virtio-net-http-post-probe.elf'
$diskImage = Join-Path $releaseDir 'qemu-virtio-net-http-post-probe.img'
$bootSource = Join-Path $repo 'scripts\baremetal\pvh_boot.S'
$linkerScript = Join-Path $repo 'scripts\baremetal\pvh_lld.ld'
$stdoutPath = Join-Path $releaseDir 'qemu-virtio-net-http-post-probe.stdout.log'
$stderrPath = Join-Path $releaseDir 'qemu-virtio-net-http-post-probe.stderr.log'
...
pub const virtio_net_http_post_probe: bool = true;
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj `
        ...
        --name 'openclaw-zig-baremetal-main-virtio-net-http-post-probe' `
        "-femit-bin=$mainObj"
    if ($LASTEXITCODE -ne 0) {
        throw "zig build-obj for virtio-net HTTP probe failed with exit code $LASTEXITCODE"
    }
...
if (-not (Test-Path $artifact)) {
    throw "virtio-net HTTP probe artifact is missing: $artifact"
}
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "QEMU VIRTIO_NET HTTP POST probe timed out after $TimeoutSeconds seconds."
}
...
if ($exitCode -ne $expectedExitCode) {
    ...
    throw ("QEMU VIRTIO_NET HTTP POST probe failed with exit code {0} (probe code 0x{1:X2})." -f $exitCode, $probeCode)
}

Write-Output 'BAREMETAL_QEMU_AVAILABLE=True'
Write-Output "BAREMETAL_QEMU_BINARY=$qemu"
Write-Output 'BAREMETAL_QEMU_VIRTIO_NET_HTTP_POST_PROBE=pass'
Write-Output ("BAREMETAL_QEMU_VIRTIO_NET_HTTP_POST_PROBE_CODE=0x{0:X2}" -f $expectedProbeCode)
Write-Output "BAREMETAL_QEMU_VIRTIO_NET_HTTP_POST_STDOUT=$stdoutPath"
Write-Output "BAREMETAL_QEMU_VIRTIO_NET_HTTP_POST_STDERR=$stderrPath"
