...
if ($null -eq $qemu) {
    ...
    Write-Output 'BAREMETAL_I386_QEMU_E1000_UDP_PROBE=skipped'
    return
}

if (-not $SkipBuild) {
    & $zig build baremetal-i386 -Doptimize=ReleaseFast -Dbaremetal-e1000-udp-probe=true --summary all
    if ($LASTEXITCODE -ne 0) { throw "zig build baremetal-i386 e1000 udp probe failed with exit code $LASTEXITCODE" }
}
...
$stdoutPath = Join-Path $releaseDir 'qemu-i386-e1000-udp-probe.stdout.log'
$stderrPath = Join-Path $releaseDir 'qemu-i386-e1000-udp-probe.stderr.log'
$echoStdoutPath = Join-Path $releaseDir 'qemu-i386-e1000-udp-echo.stdout.log'
$echoStderrPath = Join-Path $releaseDir 'qemu-i386-e1000-udp-echo.stderr.log'
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "QEMU i386 E1000 UDP probe timed out after $TimeoutSeconds seconds."
}
$proc.WaitForExit()
...
if ($exitCode -ne $expectedExitCode) { throw "QEMU i386 E1000 UDP probe failed: expected exit $expectedExitCode but saw $exitCode" }
if ($echoProc.ExitCode -gt 0) { throw "QEMU i386 E1000 UDP echo helper failed with exit code $($echoProc.ExitCode)" }

Write-Output 'BAREMETAL_I386_QEMU_AVAILABLE=True'
Write-Output "BAREMETAL_I386_QEMU_BINARY=$qemu"
Write-Output 'BAREMETAL_I386_QEMU_E1000_UDP_PROBE=pass'
Write-Output ("BAREMETAL_I386_QEMU_E1000_UDP_PROBE_CODE=0x{0:X2}" -f $expectedProbeCode)
Write-Output "BAREMETAL_I386_QEMU_E1000_UDP_STDOUT=$stdoutPath"
Write-Output "BAREMETAL_I386_QEMU_E1000_UDP_STDERR=$stderrPath"
Write-Output "BAREMETAL_I386_QEMU_E1000_UDP_ECHO_STDOUT=$echoStdoutPath"
Write-Output "BAREMETAL_I386_QEMU_E1000_UDP_ECHO_STDERR=$echoStderrPath"
