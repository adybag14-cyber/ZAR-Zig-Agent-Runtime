...
param(
    ...
    [int] $ReplyDelayMs = 0,
    [string] $RemoteMac = '02:5A:52:10:00:F1'
)
...
try {
    ...
    Write-Output "VIRTIO_NET_ECHO_FROM=$($peer.Address):$($peer.Port)"
    Write-Output "VIRTIO_NET_ECHO_FRAME_LEN=$($frame.Length)"
    ...
    Write-Output "VIRTIO_NET_ECHO_SENT=$bytesSent"
    Write-Output "VIRTIO_NET_ECHO_REPLY_PORT=$ReplyPort"
    Write-Output "VIRTIO_NET_ECHO_REPLY_DELAY_MS=$ReplyDelayMs"
    Write-Output 'VIRTIO_NET_ECHO_STATUS=ok'
}
finally {
    $udp.Close()
}
