...
$zigLocalCacheDir = if ($env:ZIG_LOCAL_CACHE_DIR -and $env:ZIG_LOCAL_CACHE_DIR.Trim().Length -gt 0) { $env:ZIG_LOCAL_CACHE_DIR } else { Join-Path $repo '.zig-cache-virtio-net-udp-probe' }
...
if ($null -eq $qemu) {
    ...
    Write-Output 'BAREMETAL_QEMU_VIRTIO_NET_UDP_PROBE=skipped'
    return
}
...
$optionsPath = Join-Path $releaseDir 'qemu-virtio-net-udp-probe-options.zig'
$mainObj = Join-Path $releaseDir 'openclaw-zig-baremetal-main-virtio-net-udp-probe.o'
$bootObj = Join-Path $releaseDir 'openclaw-zig-pvh-boot-virtio-net-udp-probe.o'
$artifact = Join-Path $releaseDir 'openclaw-zig-baremetal-pvh-virtio-net-udp-probe.elf'
$bootSource = Join-Path $repo 'scripts\baremetal\pvh_boot.S'
$linkerScript = Join-Path $repo 'scripts\baremetal\pvh_lld.ld'
$echoScript = Join-Path $repo 'scripts\qemu-virtio-net-dgram-echo.ps1'
$stdoutPath = Join-Path $releaseDir 'qemu-virtio-net-udp-probe.stdout.log'
$stderrPath = Join-Path $releaseDir 'qemu-virtio-net-udp-probe.stderr.log'
$echoStdoutPath = Join-Path $releaseDir 'qemu-virtio-net-echo.stdout.log'
$echoStderrPath = Join-Path $releaseDir 'qemu-virtio-net-echo.stderr.log'
...
pub const virtio_net_udp_probe: bool = true;
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj `
        ...
        --name 'openclaw-zig-baremetal-main-virtio-net-udp-probe' `
        "-femit-bin=$mainObj"
    if ($LASTEXITCODE -ne 0) {
        throw "zig build-obj for VIRTIO-NET UDP probe failed with exit code $LASTEXITCODE"
    }

    & $clang -c -target x86_64-unknown-elf $bootSource -o $bootObj
    if ($LASTEXITCODE -ne 0) {
        throw "clang assemble for VIRTIO-NET PVH shim failed with exit code $LASTEXITCODE"
    }

    & $lld -flavor gnu -m elf_x86_64 -o $artifact -T $linkerScript $mainObj $bootObj $compilerRt
    if ($LASTEXITCODE -ne 0) {
        throw "lld link for VIRTIO-NET PVH artifact failed with exit code $LASTEXITCODE"
    }
}

if (-not (Test-Path $artifact)) {
    throw "VIRTIO-NET UDP probe artifact is missing: $artifact"
}
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "QEMU virtio-net probe timed out after $TimeoutSeconds seconds."
}
...
if (-not $echoProc.WaitForExit(5000)) {
    ...
    throw 'QEMU virtio-net echo helper did not exit cleanly.'
}
...
Write-Output "BAREMETAL_QEMU_VIRTIO_NET_ECHO_STDOUT=$echoStdoutPath"
Write-Output "BAREMETAL_QEMU_VIRTIO_NET_ECHO_STDERR=$echoStderrPath"
...
if ($actualExitCode -ne $expectedExitCode) {
    ...
    throw "QEMU virtio-net probe failed: expected exit $expectedExitCode but saw $actualExitCode"
}

if ($echoProc.ExitCode -ne 0) {
    if ($echoStdout) { Write-Output $echoStdout.TrimEnd() }
    if ($echoStderr) { Write-Output $echoStderr.TrimEnd() }
    throw "QEMU virtio-net echo helper failed with exit code $($echoProc.ExitCode)"
}

Write-Output 'BAREMETAL_QEMU_VIRTIO_NET_UDP_PROBE=passed'
