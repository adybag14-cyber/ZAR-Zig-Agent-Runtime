...
if ($null -eq $qemu) {
    ...
    Write-Output "BAREMETAL_I386_FIRMWARE_SMP_PRIORITY_ROTATION_PROBE=skipped"
    return
}
...
if (-not $SkipBuild) {
    & $zig build baremetal-i386 -Doptimize=ReleaseFast -Dbaremetal-i386-smp-priority-rotation-probe=true --prefix $buildPrefix --summary all
    if ($LASTEXITCODE -ne 0) {
        throw "zig build baremetal-i386 -Dbaremetal-i386-smp-priority-rotation-probe=true failed with exit code $LASTEXITCODE"
    }
}
...
if ($null -eq $artifact) {
    throw "i386 bare-metal firmware SMP priority rotation artifact not found after build."
}

$firmwareImage = Join-Path $releaseDir "qemu-i386-firmware-smp-priority-rotation-probe.img"
$firmwareMetadata = Join-Path $releaseDir "qemu-i386-firmware-smp-priority-rotation-probe.meta.txt"
$stdoutPath = Join-Path $releaseDir "qemu-i386-firmware-smp-priority-rotation-probe-stdout.log"
$stderrPath = Join-Path $releaseDir "qemu-i386-firmware-smp-priority-rotation-probe-stderr.log"
$debugLogPath = Join-Path $releaseDir "qemu-i386-firmware-smp-priority-rotation-probe-debug.log"
...
if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "QEMU i386 firmware SMP priority rotation probe timed out after $TimeoutSeconds seconds."
    }
    throw "QEMU i386 firmware SMP priority rotation probe timed out after $TimeoutSeconds seconds.`n$debugTail"
}
...
if (-not $expectedExitCodes.ContainsKey($exitCode)) {
    ...
    throw "QEMU i386 firmware SMP priority rotation probe failed: exit=$exitCode expected one of [$expectedText]`n$stderrTail`n$debugTail"
}
...
Write-Output ("BAREMETAL_I386_QEMU_FIRMWARE_SMP_PRIORITY_ROTATION_PROBE_CODE=0x{0:X2}" -f $observedProbeCode)
Write-Output "BAREMETAL_I386_QEMU_FIRMWARE_SMP_PRIORITY_ROTATION_PROBE_DEBUG=$debugLogPath"
Write-Output "BAREMETAL_I386_FIRMWARE_SMP_PRIORITY_ROTATION_PROBE=pass"
