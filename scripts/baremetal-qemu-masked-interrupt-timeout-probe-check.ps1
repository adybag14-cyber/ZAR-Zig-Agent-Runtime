...
if ($null -eq $qemu -or $null -eq $gdb) {
    ...
    Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE=skipped"
    return
}
...
$optionsPath = Join-Path $releaseDir "qemu-masked-interrupt-timeout-probe-options.zig"
$rootModulePath = (Join-Path $repo "src/baremetal_main.zig").Replace('\\', '/')
$optionsModulePath = $optionsPath.Replace('\\', '/')
$mainObj = Join-Path $releaseDir "openclaw-zig-baremetal-main-masked-interrupt-timeout-probe.o"
$bootObj = Join-Path $releaseDir "openclaw-zig-pvh-boot-masked-interrupt-timeout-probe.o"
$artifact = Join-Path $releaseDir "openclaw-zig-baremetal-pvh-masked-interrupt-timeout-probe.elf"
$bootSource = Join-Path $repo "scripts\baremetal\pvh_boot.S"
$linkerScript = Join-Path $repo "scripts\baremetal\pvh_lld.ld"
$gdbScript = Join-Path $releaseDir "qemu-masked-interrupt-timeout-probe.gdb"
$gdbStdout = Join-Path $releaseDir "qemu-masked-interrupt-timeout-probe.gdb.stdout.log"
$gdbStderr = Join-Path $releaseDir "qemu-masked-interrupt-timeout-probe.gdb.stderr.log"
$qemuStdout = Join-Path $releaseDir "qemu-masked-interrupt-timeout-probe.qemu.stdout.log"
$qemuStderr = Join-Path $releaseDir "qemu-masked-interrupt-timeout-probe.qemu.stderr.log"
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj `
        ...
        --name "openclaw-zig-baremetal-main-masked-interrupt-timeout-probe" `
        "-femit-bin=$mainObj"
    if ($LASTEXITCODE -ne 0) {
        throw "zig build-obj for masked-interrupt-timeout probe runtime failed with exit code $LASTEXITCODE"
    }

    & $clang -c -target x86_64-unknown-elf $bootSource -o $bootObj
    if ($LASTEXITCODE -ne 0) {
        throw "clang assemble for masked-interrupt-timeout probe PVH shim failed with exit code $LASTEXITCODE"
    }

    & $lld -flavor gnu -m elf_x86_64 -o $artifact -T $linkerScript $mainObj $bootObj $compilerRt
    if ($LASTEXITCODE -ne 0) {
        throw "lld link for masked-interrupt-timeout probe PVH artifact failed with exit code $LASTEXITCODE"
    }
}
...
if `$stage == 8
  if *(unsigned int*)(0x$statusAddress+$statusCommandSeqAckOffset) == 8 && *(unsigned int*)(0x$schedulerTasksAddress+$taskIdOffset) != 0
...
if `$stage == 9
  if *(unsigned int*)(0x$statusAddress+$statusCommandSeqAckOffset) == 9 && *(unsigned char*)(0x$schedulerTasksAddress+$taskStateOffset) == $taskStateWaiting && *(unsigned char*)(0x$schedulerWaitKindAddress) == $waitConditionInterruptAny && *(unsigned long long*)(0x$schedulerWaitTimeoutTickAddress) > *(unsigned long long*)(0x$statusAddress+$statusTicksOffset) && *(unsigned int*)(0x$wakeQueueCountAddress) == 0 && *(unsigned long long*)(0x$interruptStateAddress+$interruptStateInterruptCountOffset) == 0 && *(unsigned long long*)(0x$maskedInterruptIgnoredCountAddress) == 0 && *(unsigned char*)(0x$interruptMaskProfileAddress) == $interruptMaskProfileExternalAll
...
if `$stage == 10
  if *(unsigned int*)(0x$statusAddress+$statusCommandSeqAckOffset) == 10 && *(unsigned char*)(0x$schedulerTasksAddress+$taskStateOffset) == $taskStateWaiting && *(unsigned char*)(0x$schedulerWaitKindAddress) == $waitConditionInterruptAny && *(unsigned int*)(0x$wakeQueueCountAddress) == 0 && *(unsigned long long*)(0x$interruptStateAddress+$interruptStateInterruptCountOffset) == 0 && *(unsigned long long*)(0x$maskedInterruptIgnoredCountAddress) == 1 && *(unsigned char*)(0x$lastMaskedInterruptVectorAddress) == $maskedVector && *(unsigned char*)(0x$interruptMaskProfileAddress) == $interruptMaskProfileExternalAll
    printf "TASK0_STATE_AFTER_INTERRUPT=%u\n", *(unsigned char*)(0x$schedulerTasksAddress+$taskStateOffset)
    printf "WAKE_QUEUE_COUNT_AFTER_INTERRUPT=%u\n", *(unsigned int*)(0x$wakeQueueCountAddress)
    printf "MASKED_IGNORED_AFTER_INTERRUPT=%llu\n", *(unsigned long long*)(0x$maskedInterruptIgnoredCountAddress)
    printf "PROFILE_AFTER_INTERRUPT=%u\n", *(unsigned char*)(0x$interruptMaskProfileAddress)
    printf "LAST_MASKED_VECTOR_AFTER_INTERRUPT=%u\n", *(unsigned char*)(0x$lastMaskedInterruptVectorAddress)
    printf "INTERRUPT_COUNT_AFTER_INTERRUPT=%llu\n", *(unsigned long long*)(0x$interruptStateAddress+$interruptStateInterruptCountOffset)
...
if `$stage == 11
  if *(unsigned int*)(0x$wakeQueueCountAddress) == 1 && *(unsigned char*)(0x$schedulerTasksAddress+$taskStateOffset) == $taskStateReady && *(unsigned char*)(0x$schedulerWaitKindAddress) == $waitConditionNone && *(unsigned long long*)(0x$schedulerWaitTimeoutTickAddress) == 0 && *(unsigned long long*)(0x$interruptStateAddress+$interruptStateInterruptCountOffset) == 0 && *(unsigned long long*)(0x$maskedInterruptIgnoredCountAddress) == 1 && *(unsigned char*)(0x$interruptMaskProfileAddress) == $interruptMaskProfileExternalAll
...
if `$stage == 12
  if *(unsigned long long*)(0x$statusAddress+$statusTicksOffset) >= `$post_wake_tick + $postWakeSlackTicks && *(unsigned int*)(0x$wakeQueueCountAddress) == 1 && *(unsigned long long*)(0x$interruptStateAddress+$interruptStateInterruptCountOffset) == 0 && *(unsigned long long*)(0x$maskedInterruptIgnoredCountAddress) == 1
    set `$stage = 13
  end
  continue
end
printf "AFTER_MASKED_INTERRUPT_TIMEOUT\n"
...
$hitStart = $false
$hitAfterMaskedInterruptTimeout = $false
$ack = $null
$lastOpcode = $null
$lastResult = $null
$ticks = $null
...
$preWakeTick = $null
$postWakeTick = $null
$task0StateAfterInterrupt = $null
$wakeQueueCountAfterInterrupt = $null
$maskedIgnoredAfterInterrupt = $null
$profileAfterInterrupt = $null
$lastMaskedVectorAfterInterrupt = $null
$interruptCountAfterInterrupt = $null
$waitKind0 = $null
$waitVector0 = $null
$waitTimeout0 = $null
$timerEnabled = $null
...
if (Test-Path $gdbStdout) {
    ...
    $hitAfterMaskedInterruptTimeout = $gdbOutput.Contains("AFTER_MASKED_INTERRUPT_TIMEOUT")
    ...
    $task0StateAfterInterrupt = Extract-IntValue -Text $gdbOutput -Name "TASK0_STATE_AFTER_INTERRUPT"
    $wakeQueueCountAfterInterrupt = Extract-IntValue -Text $gdbOutput -Name "WAKE_QUEUE_COUNT_AFTER_INTERRUPT"
    $maskedIgnoredAfterInterrupt = Extract-IntValue -Text $gdbOutput -Name "MASKED_IGNORED_AFTER_INTERRUPT"
    $profileAfterInterrupt = Extract-IntValue -Text $gdbOutput -Name "PROFILE_AFTER_INTERRUPT"
    $lastMaskedVectorAfterInterrupt = Extract-IntValue -Text $gdbOutput -Name "LAST_MASKED_VECTOR_AFTER_INTERRUPT"
    $interruptCountAfterInterrupt = Extract-IntValue -Text $gdbOutput -Name "INTERRUPT_COUNT_AFTER_INTERRUPT"
    ...
    $interruptMaskProfile = Extract-IntValue -Text $gdbOutput -Name "INTERRUPT_MASK_PROFILE"
    $maskedInterruptIgnoredCount = Extract-IntValue -Text $gdbOutput -Name "MASKED_INTERRUPT_IGNORED_COUNT"
    $lastMaskedInterruptVector = Extract-IntValue -Text $gdbOutput -Name "LAST_MASKED_INTERRUPT_VECTOR"
}
...
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_START_ADDR=0x$startAddress"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_SPINPAUSE_ADDR=0x$spinPauseAddress"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_STATUS_ADDR=0x$statusAddress"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_MAILBOX_ADDR=0x$commandMailboxAddress"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_SCHEDULER_STATE_ADDR=0x$schedulerStateAddress"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_SCHEDULER_TASKS_ADDR=0x$schedulerTasksAddress"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_SCHEDULER_WAIT_KIND_ADDR=0x$schedulerWaitKindAddress"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_SCHEDULER_WAIT_INTERRUPT_VECTOR_ADDR=0x$schedulerWaitInterruptVectorAddress"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_SCHEDULER_WAIT_TIMEOUT_TICK_ADDR=0x$schedulerWaitTimeoutTickAddress"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_TIMER_STATE_ADDR=0x$timerStateAddress"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_WAKE_QUEUE_ADDR=0x$wakeQueueAddress"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_WAKE_QUEUE_COUNT_ADDR=0x$wakeQueueCountAddress"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_INTERRUPT_STATE_ADDR=0x$interruptStateAddress"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_INTERRUPT_MASK_PROFILE_ADDR=0x$interruptMaskProfileAddress"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_MASKED_INTERRUPT_IGNORED_COUNT_ADDR=0x$maskedInterruptIgnoredCountAddress"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_LAST_MASKED_INTERRUPT_VECTOR_ADDR=0x$lastMaskedInterruptVectorAddress"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_HIT_START=$hitStart"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_HIT_AFTER_MASKED_INTERRUPT_TIMEOUT=$hitAfterMaskedInterruptTimeout"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_ACK=$ack"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_LAST_OPCODE=$lastOpcode"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_LAST_RESULT=$lastResult"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_TICKS=$ticks"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_MAILBOX_OPCODE=$mailboxOpcode"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_MAILBOX_SEQ=$mailboxSeq"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_SCHED_TASK_COUNT=$schedTaskCount"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_TASK0_ID=$task0Id"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_TASK0_STATE=$task0State"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_TASK0_PRIORITY=$task0Priority"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_TASK0_RUN_COUNT=$task0RunCount"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_TASK0_BUDGET=$task0Budget"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_TASK0_BUDGET_REMAINING=$task0BudgetRemaining"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_ARMED_TICKS=$armedTicks"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_ARMED_WAIT_TIMEOUT=$armedWaitTimeout"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_PRE_WAKE_TICK=$preWakeTick"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_POST_WAKE_TICK=$postWakeTick"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_TASK0_STATE_AFTER_INTERRUPT=$task0StateAfterInterrupt"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_WAKE_QUEUE_COUNT_AFTER_INTERRUPT=$wakeQueueCountAfterInterrupt"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_MASKED_IGNORED_AFTER_INTERRUPT=$maskedIgnoredAfterInterrupt"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_PROFILE_AFTER_INTERRUPT=$profileAfterInterrupt"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_LAST_MASKED_VECTOR_AFTER_INTERRUPT=$lastMaskedVectorAfterInterrupt"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_INTERRUPT_COUNT_AFTER_INTERRUPT=$interruptCountAfterInterrupt"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_WAIT_KIND0=$waitKind0"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_WAIT_VECTOR0=$waitVector0"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_WAIT_TIMEOUT0=$waitTimeout0"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_TIMER_ENABLED=$timerEnabled"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_TIMER_ENTRY_COUNT=$timerEntryCount"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_TIMER_PENDING_WAKE_COUNT=$timerPendingWakeCount"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_TIMER_DISPATCH_COUNT=$timerDispatchCount"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_TIMER_LAST_INTERRUPT_COUNT=$timerLastInterruptCount"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_TIMER_LAST_WAKE_TICK=$timerLastWakeTick"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_WAKE_QUEUE_COUNT=$wakeQueueCount"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_WAKE0_SEQ=$wake0Seq"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_WAKE0_TASK_ID=$wake0TaskId"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_WAKE0_TIMER_ID=$wake0TimerId"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_WAKE0_REASON=$wake0Reason"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_WAKE0_VECTOR=$wake0Vector"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_WAKE0_TICK=$wake0Tick"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_INTERRUPT_COUNT=$interruptCount"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_LAST_INTERRUPT_VECTOR=$lastInterruptVector"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_INTERRUPT_MASK_PROFILE=$interruptMaskProfile"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_MASKED_INTERRUPT_IGNORED_COUNT=$maskedInterruptIgnoredCount"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_LAST_MASKED_INTERRUPT_VECTOR=$lastMaskedInterruptVector"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_GDB_STDOUT=$gdbStdout"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_GDB_STDERR=$gdbStderr"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_QEMU_STDOUT=$qemuStdout"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_QEMU_STDERR=$qemuStderr"
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE_TIMED_OUT=$timedOut"

$probePassed = $hitStart -and
    $hitAfterMaskedInterruptTimeout -and
    (-not $timedOut) -and
    ($ack -eq 10) -and
    ($lastOpcode -eq $triggerInterruptOpcode) -and
    ($lastResult -eq 0) -and
    ($mailboxOpcode -eq $triggerInterruptOpcode) -and
    ($mailboxSeq -eq 10) -and
    ($schedTaskCount -eq 1) -and
    ($task0Id -eq 1) -and
    ($task0StateAfterInterrupt -eq $taskStateWaiting) -and
    ($wakeQueueCountAfterInterrupt -eq 0) -and
    ($maskedIgnoredAfterInterrupt -eq 1) -and
    ($profileAfterInterrupt -eq $interruptMaskProfileExternalAll) -and
    ($lastMaskedVectorAfterInterrupt -eq $maskedVector) -and
    ($interruptCountAfterInterrupt -eq 0) -and
    ...
    ($armedTicks -ge 1) -and
    ($armedWaitTimeout -eq ($armedTicks + ($timeoutTicks - 1))) -and
    ($preWakeTick -lt $armedWaitTimeout) -and
    ...
    ($wake0Tick -eq $armedWaitTimeout) -and
...
Write-Output "BAREMETAL_QEMU_MASKED_INTERRUPT_TIMEOUT_PROBE=$($(if ($probePassed) { 'pass' } else { 'fail' }))"
