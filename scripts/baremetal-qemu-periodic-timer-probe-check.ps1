...
if ($null -eq $qemu -or $null -eq $gdb) {
    ...
    Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE=skipped"
    return
}
...
$optionsPath = Join-Path $releaseDir "qemu-periodic-timer-probe-options.zig"
$mainObj = Join-Path $releaseDir "openclaw-zig-baremetal-main-periodic-timer-probe.o"
$bootObj = Join-Path $releaseDir "openclaw-zig-pvh-boot-periodic-timer-probe.o"
...
$gdbScript = Join-Path $releaseDir "qemu-periodic-timer-probe.gdb"
$gdbStdout = Join-Path $releaseDir "qemu-periodic-timer-probe.gdb.stdout.log"
$gdbStderr = Join-Path $releaseDir "qemu-periodic-timer-probe.gdb.stderr.log"
$qemuStdout = Join-Path $releaseDir "qemu-periodic-timer-probe.qemu.stdout.log"
$qemuStderr = Join-Path $releaseDir "qemu-periodic-timer-probe.qemu.stderr.log"
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj `
        ...
        --name "openclaw-zig-baremetal-main-periodic-timer-probe" `
        "-femit-bin=$mainObj"
    if ($LASTEXITCODE -ne 0) {
        throw "zig build-obj for periodic-timer probe runtime failed with exit code $LASTEXITCODE"
    }

    & $clang -c -target x86_64-unknown-elf $bootSource -o $bootObj
    if ($LASTEXITCODE -ne 0) {
        throw "clang assemble for periodic-timer probe PVH shim failed with exit code $LASTEXITCODE"
    }

    & $lld -flavor gnu -m elf_x86_64 -o $artifact -T $linkerScript $mainObj $bootObj $compilerRt
    if ($LASTEXITCODE -ne 0) {
        throw "lld link for periodic-timer probe PVH artifact failed with exit code $LASTEXITCODE"
    }
}
...
set `$paused_fire_count = 0
set `$paused_dispatch_count = 0
set `$paused_wake_count = 0
set `$paused_tick = 0
...
if `$stage == 7
  if *(unsigned int*)(0x$statusAddress+$statusCommandSeqAckOffset) == 7 && *(unsigned char*)(0x$timerEntriesAddress+$timerEntryStateOffset) == 1 && (*(unsigned short*)(0x$timerEntriesAddress+$timerEntryFlagsOffset) & 1) == 1 && *(unsigned int*)(0x$timerEntriesAddress+$timerEntryPeriodTicksOffset) == $periodicDelay
    set `$stage = 8
  end
  continue
end
if `$stage == 8
  if *(unsigned long long*)(0x$timerEntriesAddress+$timerEntryFireCountOffset) >= 1 && *(unsigned short*)(0x$timerStateAddress+$timerPendingWakeCountOffset) >= 1 && *(unsigned char*)(0x$timerEntriesAddress+$timerEntryStateOffset) == 1
    ...
    set `$first_wake_count = *(unsigned short*)(0x$timerStateAddress+$timerPendingWakeCountOffset)
...
if `$stage == 9
  if *(unsigned int*)(0x$statusAddress+$statusCommandSeqAckOffset) == 8 && *(unsigned char*)(0x$timerStateAddress+$timerEnabledOffset) == 0
...
if `$stage == 10
  if *(unsigned long long*)(0x$statusAddress+$statusTicksOffset) >= `$pause_target_tick && *(unsigned char*)(0x$timerStateAddress+$timerEnabledOffset) == 0 && *(unsigned long long*)(0x$timerEntriesAddress+$timerEntryFireCountOffset) == `$first_fire_count && *(unsigned short*)(0x$timerStateAddress+$timerPendingWakeCountOffset) == `$first_wake_count && *(unsigned long long*)(0x$timerStateAddress+$timerDispatchCountOffset) == `$first_dispatch_count
    set `$paused_fire_count = *(unsigned long long*)(0x$timerEntriesAddress+$timerEntryFireCountOffset)
    set `$paused_dispatch_count = *(unsigned long long*)(0x$timerStateAddress+$timerDispatchCountOffset)
    set `$paused_wake_count = *(unsigned short*)(0x$timerStateAddress+$timerPendingWakeCountOffset)
...
if `$stage == 11
  if *(unsigned int*)(0x$statusAddress+$statusCommandSeqAckOffset) == 9 && *(unsigned char*)(0x$timerStateAddress+$timerEnabledOffset) == 1
    set `$stage = 12
  end
  continue
end
if `$stage == 12
  if *(unsigned long long*)(0x$timerEntriesAddress+$timerEntryFireCountOffset) == (`$first_fire_count + 1) && *(unsigned short*)(0x$timerStateAddress+$timerPendingWakeCountOffset) == (`$first_wake_count + 1) && *(unsigned long long*)(0x$timerStateAddress+$timerDispatchCountOffset) == (`$first_dispatch_count + 1) && *(unsigned char*)(0x$timerEntriesAddress+$timerEntryStateOffset) == 1
    ...
    set `$second_wake_count = *(unsigned short*)(0x$timerStateAddress+$timerPendingWakeCountOffset)
...
$pausedFireCount = $null
$pausedDispatchCount = $null
$pausedWakeCount = $null
$pausedTick = $null
$wake0Seq = $null
$wake0TaskId = $null
$wake0TimerId = $null
...
if (Test-Path $gdbStdout) {
    ...
    $hitAfterPeriodicTimer = $out -match "AFTER_PERIODIC_TIMER"
    ...
    $pausedFireCount = Extract-IntValue -Text $out -Name "PAUSED_FIRE_COUNT"
    $pausedDispatchCount = Extract-IntValue -Text $out -Name "PAUSED_DISPATCH_COUNT"
    $pausedWakeCount = Extract-IntValue -Text $out -Name "PAUSED_WAKE_COUNT"
    $pausedTick = Extract-IntValue -Text $out -Name "PAUSED_TICK"
...
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_START_ADDR=0x$startAddress"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_SPINPAUSE_ADDR=0x$spinPauseAddress"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_STATUS_ADDR=0x$statusAddress"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_MAILBOX_ADDR=0x$commandMailboxAddress"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_SCHEDULER_STATE_ADDR=0x$schedulerStateAddress"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TASKS_ADDR=0x$schedulerTasksAddress"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TIMER_STATE_ADDR=0x$timerStateAddress"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TIMER_ENTRIES_ADDR=0x$timerEntriesAddress"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_WAKE_QUEUE_ADDR=0x$wakeQueueAddress"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_HIT_START=$hitStart"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_HIT_AFTER_PERIODIC_TIMER=$hitAfterPeriodicTimer"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_ACK=$ack"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_LAST_OPCODE=$lastOpcode"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_LAST_RESULT=$lastResult"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TICKS=$ticks"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_MAILBOX_OPCODE=$mailboxOpcode"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_MAILBOX_SEQ=$mailboxSeq"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_SCHED_TASK_COUNT=$schedulerTaskCount"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TASK0_ID=$task0Id"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TASK0_STATE=$task0State"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TASK0_PRIORITY=$task0Priority"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TASK0_RUN_COUNT=$task0RunCount"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TASK0_BUDGET=$task0Budget"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TASK0_BUDGET_REMAINING=$task0BudgetRemaining"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TIMER_ENABLED=$timerEnabled"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TIMER_ENTRY_COUNT=$timerEntryCount"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_PENDING_WAKE_COUNT=$pendingWakeCount"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TIMER_DISPATCH_COUNT=$timerDispatchCount"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TIMER_LAST_WAKE_TICK=$timerLastWakeTick"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TIMER_QUANTUM=$timerQuantumOut"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TIMER0_ID=$timer0Id"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TIMER0_TASK_ID=$timer0TaskId"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TIMER0_STATE=$timer0State"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TIMER0_REASON=$timer0Reason"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TIMER0_FLAGS=$timer0Flags"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TIMER0_PERIOD_TICKS=$timer0PeriodTicks"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TIMER0_NEXT_FIRE_TICK=$timer0NextFireTick"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TIMER0_FIRE_COUNT=$timer0FireCount"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TIMER0_LAST_FIRE_TICK=$timer0LastFireTick"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_FIRST_FIRE_COUNT=$firstFireCount"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_FIRST_DISPATCH_COUNT=$firstDispatchCount"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_FIRST_WAKE_COUNT=$firstWakeCount"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_FIRST_LAST_FIRE_TICK=$firstLastFireTick"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_PAUSED_FIRE_COUNT=$pausedFireCount"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_PAUSED_DISPATCH_COUNT=$pausedDispatchCount"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_PAUSED_WAKE_COUNT=$pausedWakeCount"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_PAUSED_TICK=$pausedTick"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_WAKE0_SEQ=$wake0Seq"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_WAKE0_TASK_ID=$wake0TaskId"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_WAKE0_TIMER_ID=$wake0TimerId"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_WAKE0_REASON=$wake0Reason"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_WAKE0_VECTOR=$wake0Vector"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_WAKE0_TICK=$wake0Tick"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_WAKE1_SEQ=$wake1Seq"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_WAKE1_TASK_ID=$wake1TaskId"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_WAKE1_TIMER_ID=$wake1TimerId"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_WAKE1_REASON=$wake1Reason"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_WAKE1_VECTOR=$wake1Vector"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_WAKE1_TICK=$wake1Tick"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_GDB_STDOUT=$gdbStdout"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_GDB_STDERR=$gdbStderr"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_QEMU_STDOUT=$qemuStdout"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_QEMU_STDERR=$qemuStderr"
Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE_TIMED_OUT=$timedOut"

$pass = (
    $hitStart -and
    $hitAfterPeriodicTimer -and
    (-not $timedOut) -and
    $ack -eq 9 -and
    ...
    $timerEntryCount -eq 1 -and
    $pendingWakeCount -eq 2 -and
    $timerDispatchCount -eq 2 -and
    $timerLastWakeTick -eq $timer0LastFireTick -and
    $timerQuantumOut -eq $timerQuantum -and
    $timer0Id -eq 1 -and
    $timer0TaskId -eq 1 -and
    $timer0State -eq 1 -and
    $timer0Reason -eq 1 -and
    (($timer0Flags -band 1) -eq 1) -and
    $timer0PeriodTicks -eq $periodicDelay -and
    $timer0FireCount -eq 2 -and
    $timer0LastFireTick -gt $firstLastFireTick -and
    $timer0NextFireTick -gt $timer0LastFireTick -and
    $firstFireCount -eq 1 -and
    $firstDispatchCount -eq 1 -and
    $firstWakeCount -eq 1 -and
    $firstLastFireTick -ge 1 -and
    $pausedFireCount -eq $firstFireCount -and
    $pausedDispatchCount -eq $firstDispatchCount -and
    $pausedWakeCount -eq $firstWakeCount -and
    $pausedTick -gt $firstLastFireTick -and
    $wake0Seq -eq 1 -and
    $wake0TaskId -eq 1 -and
    $wake0TimerId -eq 1 -and
    $wake0Reason -eq 1 -and
    $wake0Vector -eq 0 -and
    $wake0Tick -eq $firstLastFireTick -and
    $wake1Seq -eq 2 -and
    $wake1TaskId -eq 1 -and
    $wake1TimerId -eq 1 -and
    $wake1Reason -eq 1 -and
    $wake1Vector -eq 0 -and
    $wake1Tick -eq $timer0LastFireTick
)

if ($pass) {
    Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE=pass"
    exit 0
}

Write-Output "BAREMETAL_QEMU_PERIODIC_TIMER_PROBE=fail"
if (Test-Path $gdbStdout) {
