...
if ($null -eq $qemu -or $null -eq $gdb) {
    ...
    Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE=skipped"
    return
}
...
$optionsPath = Join-Path $releaseDir "qemu-interrupt-timeout-disable-enable-probe-options.zig"
$mainObj = Join-Path $releaseDir "openclaw-zig-baremetal-main-interrupt-timeout-disable-enable-probe.o"
$bootObj = Join-Path $releaseDir "openclaw-zig-pvh-boot-interrupt-timeout-disable-enable-probe.o"
$artifact = Join-Path $releaseDir "openclaw-zig-baremetal-pvh-interrupt-timeout-disable-enable-probe.elf"
$bootSource = Join-Path $repo "scripts\baremetal\pvh_boot.S"
$linkerScript = Join-Path $repo "scripts\baremetal\pvh_lld.ld"
$gdbScript = Join-Path $releaseDir "qemu-interrupt-timeout-disable-enable-probe.gdb"
$gdbStdout = Join-Path $releaseDir "qemu-interrupt-timeout-disable-enable-probe.gdb.stdout.log"
$gdbStderr = Join-Path $releaseDir "qemu-interrupt-timeout-disable-enable-probe.gdb.stderr.log"
$qemuStdout = Join-Path $releaseDir "qemu-interrupt-timeout-disable-enable-probe.qemu.stdout.log"
$qemuStderr = Join-Path $releaseDir "qemu-interrupt-timeout-disable-enable-probe.qemu.stderr.log"
...
"@ | Set-Content -Path $optionsPath -Encoding Ascii
    ...
    & $zig build-obj `
        ...
        --name "openclaw-zig-baremetal-main-interrupt-timeout-disable-enable-probe" `
        "-femit-bin=$mainObj"
    if ($LASTEXITCODE -ne 0) {
        throw "zig build-obj for interrupt-timeout-disable-enable probe runtime failed with exit code $LASTEXITCODE"
    }

    & $clang -c -target x86_64-unknown-elf $bootSource -o $bootObj
    if ($LASTEXITCODE -ne 0) {
        throw "clang assemble for interrupt-timeout-disable-enable probe PVH shim failed with exit code $LASTEXITCODE"
    }

    & $lld -flavor gnu -m elf_x86_64 -o $artifact -T $linkerScript $mainObj $bootObj $compilerRt
    if ($LASTEXITCODE -ne 0) {
        throw "lld link for interrupt-timeout-disable-enable probe PVH artifact failed with exit code $LASTEXITCODE"
    }
}
...
set `$disabled_tick = 0
set `$disabled_wait_kind0 = 0
set `$disabled_wait_timeout0 = 0
set `$disabled_task0_state = 0
set `$disabled_wake_queue_count = 0
set `$disabled_timer_entry_count = 0
set `$disabled_interrupt_count = 0
set `$pause_target_tick = 0
set `$pre_wake_tick = 0
set `$paused_tick = 0
set `$paused_wait_kind0 = 0
set `$paused_wait_timeout0 = 0
set `$paused_task0_state = 0
set `$paused_wake_queue_count = 0
set `$paused_timer_entry_count = 0
set `$paused_interrupt_count = 0
...
silent
printf "HIT_START\n"
set *(unsigned char*)(0x$statusAddress+6) = 1
...
if `$stage == 8
  if *(unsigned int*)(0x$statusAddress+$statusCommandSeqAckOffset) == 8 && *(unsigned char*)(0x$timerStateAddress+$timerEnabledOffset) == 0 && *(unsigned int*)(0x$wakeQueueCountAddress) == 0 && *(unsigned char*)(0x$schedulerTasksAddress+$taskStateOffset) == $taskStateWaiting && *(unsigned char*)(0x$schedulerWaitKindAddress) == $waitConditionInterruptAny && *(unsigned long long*)(0x$schedulerWaitTimeoutTickAddress) == `$armed_wait_timeout && *(unsigned long long*)(0x$interruptStateAddress+$interruptStateInterruptCountOffset) == 0
    set `$disabled_tick = *(unsigned long long*)(0x$statusAddress+$statusTicksOffset)
    set `$disabled_wait_kind0 = *(unsigned char*)(0x$schedulerWaitKindAddress)
    set `$disabled_wait_timeout0 = *(unsigned long long*)(0x$schedulerWaitTimeoutTickAddress)
    set `$disabled_task0_state = *(unsigned char*)(0x$schedulerTasksAddress+$taskStateOffset)
    set `$disabled_wake_queue_count = *(unsigned int*)(0x$wakeQueueCountAddress)
    set `$disabled_timer_entry_count = *(unsigned char*)(0x$timerStateAddress+$timerEntryCountOffset)
    set `$disabled_interrupt_count = *(unsigned long long*)(0x$interruptStateAddress+$interruptStateInterruptCountOffset)
    set `$pause_target_tick = *(unsigned long long*)(0x$statusAddress+$statusTicksOffset) + $pauseTicks
    set `$stage = 9
  end
  continue
end
if `$stage == 9
  if *(unsigned long long*)(0x$statusAddress+$statusTicksOffset) >= `$pause_target_tick && *(unsigned int*)(0x$wakeQueueCountAddress) == 0 && *(unsigned char*)(0x$schedulerTasksAddress+$taskStateOffset) == $taskStateWaiting && *(unsigned char*)(0x$schedulerWaitKindAddress) == $waitConditionInterruptAny && *(unsigned long long*)(0x$schedulerWaitTimeoutTickAddress) > 0 && *(unsigned long long*)(0x$interruptStateAddress+$interruptStateInterruptCountOffset) == 0 && *(unsigned char*)(0x$timerStateAddress+$timerEnabledOffset) == 0 && *(unsigned long long*)(0x$timerStateAddress+$timerDispatchCountOffset) == 0
    ...
    set `$paused_wait_kind0 = *(unsigned char*)(0x$schedulerWaitKindAddress)
    set `$paused_wait_timeout0 = *(unsigned long long*)(0x$schedulerWaitTimeoutTickAddress)
    set `$paused_task0_state = *(unsigned char*)(0x$schedulerTasksAddress+$taskStateOffset)
    ...
    set `$paused_interrupt_count = *(unsigned long long*)(0x$interruptStateAddress+$interruptStateInterruptCountOffset)
...
if `$stage == 11
  if *(unsigned int*)(0x$wakeQueueCountAddress) == 1 && *(unsigned char*)(0x$schedulerTasksAddress+$taskStateOffset) == $taskStateReady && *(unsigned char*)(0x$schedulerWaitKindAddress) == $waitConditionNone && *(unsigned long long*)(0x$schedulerWaitTimeoutTickAddress) == 0 && *(unsigned long long*)(0x$interruptStateAddress+$interruptStateInterruptCountOffset) == 0 && *(unsigned char*)(0x$timerStateAddress+$timerEntryCountOffset) == 0 && *(unsigned long long*)(0x$timerStateAddress+$timerDispatchCountOffset) == 0
...
if `$stage == 12
  if *(unsigned long long*)(0x$statusAddress+$statusTicksOffset) >= `$post_wake_tick + $postWakeSlackTicks && *(unsigned int*)(0x$wakeQueueCountAddress) == 1 && *(unsigned long long*)(0x$timerStateAddress+$timerDispatchCountOffset) == 0 && *(unsigned long long*)(0x$interruptStateAddress+$interruptStateInterruptCountOffset) == 0
    set `$stage = 13
  end
  continue
end
printf "AFTER_INTERRUPT_TIMEOUT_DISABLE_ENABLE\n"
...
printf "PAUSED_WAIT_KIND0=%u\n", `$paused_wait_kind0
printf "PAUSED_WAIT_TIMEOUT0=%llu\n", `$paused_wait_timeout0
printf "PAUSED_TASK0_STATE=%u\n", `$paused_task0_state
printf "PAUSED_WAKE_QUEUE_COUNT=%u\n", `$paused_wake_queue_count
printf "PAUSED_TIMER_ENTRY_COUNT=%u\n", `$paused_timer_entry_count
printf "PAUSED_INTERRUPT_COUNT=%llu\n", `$paused_interrupt_count
...
$disabledTick = $null
$disabledWaitKind0 = $null
$disabledWaitTimeout0 = $null
$disabledTask0State = $null
$disabledWakeQueueCount = $null
$disabledTimerEntryCount = $null
$disabledInterruptCount = $null
$preWakeTick = $null
$postWakeTick = $null
$waitKind0 = $null
$waitVector0 = $null
...
$wake0Tick = $null
$pausedTick = $null
$pausedWaitKind0 = $null
$pausedWaitTimeout0 = $null
$pausedTask0State = $null
$pausedWakeQueueCount = $null
$pausedTimerEntryCount = $null
$pausedInterruptCount = $null
$interruptCount = $null
...
if (Test-Path $gdbStdout) {
    ...
    $hitAfterInterruptTimeoutTimer = $gdbOutput.Contains("AFTER_INTERRUPT_TIMEOUT_DISABLE_ENABLE")
    ...
    $pausedWaitKind0 = Extract-IntValue -Text $gdbOutput -Name "PAUSED_WAIT_KIND0"
    $pausedWaitTimeout0 = Extract-IntValue -Text $gdbOutput -Name "PAUSED_WAIT_TIMEOUT0"
    $pausedTask0State = Extract-IntValue -Text $gdbOutput -Name "PAUSED_TASK0_STATE"
    ...
    $pausedInterruptCount = Extract-IntValue -Text $gdbOutput -Name "PAUSED_INTERRUPT_COUNT"
...
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_START_ADDR=0x$startAddress"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_SPINPAUSE_ADDR=0x$spinPauseAddress"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_STATUS_ADDR=0x$statusAddress"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_MAILBOX_ADDR=0x$commandMailboxAddress"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_SCHEDULER_STATE_ADDR=0x$schedulerStateAddress"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_SCHEDULER_TASKS_ADDR=0x$schedulerTasksAddress"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_SCHEDULER_WAIT_KIND_ADDR=0x$schedulerWaitKindAddress"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_SCHEDULER_WAIT_INTERRUPT_VECTOR_ADDR=0x$schedulerWaitInterruptVectorAddress"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_SCHEDULER_WAIT_TIMEOUT_TICK_ADDR=0x$schedulerWaitTimeoutTickAddress"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_TIMER_STATE_ADDR=0x$timerStateAddress"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_WAKE_QUEUE_ADDR=0x$wakeQueueAddress"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_WAKE_QUEUE_COUNT_ADDR=0x$wakeQueueCountAddress"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_INTERRUPT_STATE_ADDR=0x$interruptStateAddress"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_HIT_START=$hitStart"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_HIT_AFTER_INTERRUPT_TIMEOUT_DISABLE_ENABLE=$hitAfterInterruptTimeoutTimer"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_ACK=$ack"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_LAST_OPCODE=$lastOpcode"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_LAST_RESULT=$lastResult"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_TICKS=$ticks"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_MAILBOX_OPCODE=$mailboxOpcode"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_MAILBOX_SEQ=$mailboxSeq"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_SCHED_TASK_COUNT=$schedTaskCount"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_TASK0_ID=$task0Id"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_TASK0_STATE=$task0State"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_TASK0_PRIORITY=$task0Priority"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_TASK0_RUN_COUNT=$task0RunCount"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_TASK0_BUDGET=$task0Budget"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_TASK0_BUDGET_REMAINING=$task0BudgetRemaining"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_ARMED_TICKS=$armedTicks"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_ARMED_WAIT_TIMEOUT=$armedWaitTimeout"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_DISABLED_TICK=$disabledTick"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_DISABLED_WAIT_KIND0=$disabledWaitKind0"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_DISABLED_WAIT_TIMEOUT0=$disabledWaitTimeout0"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_DISABLED_TASK0_STATE=$disabledTask0State"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_DISABLED_WAKE_QUEUE_COUNT=$disabledWakeQueueCount"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_DISABLED_TIMER_ENTRY_COUNT=$disabledTimerEntryCount"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_DISABLED_INTERRUPT_COUNT=$disabledInterruptCount"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_PRE_WAKE_TICK=$preWakeTick"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_POST_WAKE_TICK=$postWakeTick"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_WAIT_KIND0=$waitKind0"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_WAIT_VECTOR0=$waitVector0"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_WAIT_TIMEOUT0=$waitTimeout0"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_TIMER_ENABLED=$timerEnabled"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_TIMER_ENTRY_COUNT=$timerEntryCount"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_TIMER_PENDING_WAKE_COUNT=$timerPendingWakeCount"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_TIMER_DISPATCH_COUNT=$timerDispatchCount"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_TIMER_LAST_INTERRUPT_COUNT=$timerLastInterruptCount"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_TIMER_LAST_WAKE_TICK=$timerLastWakeTick"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_WAKE_QUEUE_COUNT=$wakeQueueCount"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_WAKE0_SEQ=$wake0Seq"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_WAKE0_TASK_ID=$wake0TaskId"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_WAKE0_TIMER_ID=$wake0TimerId"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_WAKE0_REASON=$wake0Reason"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_WAKE0_VECTOR=$wake0Vector"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_WAKE0_TICK=$wake0Tick"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_PAUSED_TICK=$pausedTick"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_PAUSED_WAIT_KIND0=$pausedWaitKind0"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_PAUSED_WAIT_TIMEOUT0=$pausedWaitTimeout0"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_PAUSED_TASK0_STATE=$pausedTask0State"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_PAUSED_WAKE_QUEUE_COUNT=$pausedWakeQueueCount"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_PAUSED_TIMER_ENTRY_COUNT=$pausedTimerEntryCount"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_PAUSED_INTERRUPT_COUNT=$pausedInterruptCount"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_INTERRUPT_COUNT=$interruptCount"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_LAST_INTERRUPT_VECTOR=$lastInterruptVector"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_GDB_STDOUT=$gdbStdout"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_GDB_STDERR=$gdbStderr"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_QEMU_STDOUT=$qemuStdout"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_QEMU_STDERR=$qemuStderr"
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE_TIMED_OUT=$timedOut"
...
$probePassed = $hitStart -and
    ...
    ($disabledTick -ge $armedTicks) -and
    ($disabledWaitKind0 -eq $waitConditionInterruptAny) -and
    ($disabledWaitTimeout0 -eq $armedWaitTimeout) -and
    ($disabledTask0State -eq $taskStateWaiting) -and
    ($disabledWakeQueueCount -eq 0) -and
    ($disabledTimerEntryCount -eq 0) -and
    ($disabledInterruptCount -eq 0) -and
    ($preWakeTick -ge ($armedWaitTimeout + $pauseTicks)) -and
    ($pausedTick -eq $preWakeTick) -and
    ($pausedWaitKind0 -eq $waitConditionInterruptAny) -and
    ($pausedWaitTimeout0 -eq $armedWaitTimeout) -and
    ($pausedTask0State -eq $taskStateWaiting) -and
    ($pausedWakeQueueCount -eq 0) -and
    ($pausedTimerEntryCount -eq 0) -and
    ($pausedInterruptCount -eq 0) -and
    ($postWakeTick -ge $wake0Tick) -and
...
Write-Output "BAREMETAL_QEMU_INTERRUPT_TIMEOUT_DISABLE_ENABLE_PROBE=$($(if ($probePassed) { 'pass' } else { 'fail' }))"
