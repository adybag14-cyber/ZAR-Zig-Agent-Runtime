...
- hosted and host validation now prove persistence over both RAM-disk and ATA PIO backends, including the partition-mounted ATA view
  ...
  - bare-metal tool execution is now also on a real freestanding path:
    ...
    - the current FS5.5 workspace slice now adds persisted `/runtime/workspaces/<name>.txt` orchestration state through `src/baremetal/workspace_runtime.zig`, new `tool_exec` builtins (`workspace-list`, `workspace-info`, `workspace-save`, `workspace-apply`, `workspace-delete`), new typed TCP verbs (`WORKSPACELIST`, `WORKSPACEINFO`, `WORKSPACESAVE`, `WORKSPACEAPPLY`, `WORKSPACEDELETE`), RAM-disk and ATA-backed workspace tests, and live RTL8139 TCP proof for save/list/info/apply/delete plus restored canonical package script content, trust-bundle selection, display mode, and app-suite active-plan markers
