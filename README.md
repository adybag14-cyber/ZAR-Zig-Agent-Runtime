...
- Latest local validation: `zig build test --summary all` -> main `203/203` + bare-metal host `116/116` passing
...
- Recent FS6 progress (2026-03-06):
  ...
  - bare-metal syscall control wrapper isolation is now enforced by a dedicated five-wrapper family that fails directly on the final mailbox baseline, re-register token-update without entry-count growth, blocked invoke preservation, disabled invoke preservation, and final unregister/missing-entry cleanup invariants
