...
- `src/baremetal/edid.zig`, `src/baremetal/display_output.zig`, and `src/baremetal/virtio_gpu.zig` now provide EDID-backed display capability export plus bounded render/present/flush proof for the first real controller-specific path, `virtio-gpu-pci`, including digital-input, preferred-timing, CEA, DisplayID, HDMI-vendor-data, and basic-audio capability flags when present in EDID data
  ...
  - the same real controller path now also supports explicit per-output mode retargeting through `display-output-set <index> <width> <height>`, typed `DISPLAYOUTPUTSET <index> <width> <height>`, explicit mode-index activation through `display-output-activate-mode <index> <mode-index>` / `DISPLAYOUTPUTACTIVATEMODE <index> <mode-index>`, explicit connector-preferred activation through `display-activate-preferred <connector>` / `DISPLAYACTIVATEPREFERRED <connector>`, and explicit output-preferred activation through `display-activate-output-preferred <index>` / `DISPLAYACTIVATEOUTPUTPREFERRED <index>`; the live probe now proves the connected output advertises a bounded mode inventory, mode `0` matches the EDID-preferred geometry, a second advertised mode exists, the output can be driven down to `1024x768`, oversized requests are rejected without corrupting the exported state, and the same output can be restored back to the EDID-preferred `1280x800`
  ...
  - `scripts/baremetal-qemu-virtio-gpu-display-probe-check.ps1` now proves live `virtio-gpu-pci` EDID/controller capability export plus output-entry count/entry metadata, per-output mode count and mode inventory readback, explicit mode-index activation, explicit output-index mode change, explicit connector/output preferred reactivation, persisted display-profile save/list/info/apply, and resource-create/attach/set-scanout/flush behavior with non-zero scanout pixel readback over QEMU with `edid=on`
