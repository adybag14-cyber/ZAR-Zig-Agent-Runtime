...
- `scripts/baremetal-qemu-ata-gpt-installer-probe-check.ps1` now proves the freestanding PVH artifact mounts a protective-MBR GPT partition, writes through the logical mounted view, seeds the installer layout, and runs the persisted bootstrap package from disk
  ...
  - TCP/IP is now strict-closed for the FS5.5 acceptance bar:
    ...
    - `scripts/baremetal-qemu-rtl8139-https-post-probe-check.ps1` now proves the freestanding PAL `https://` POST transport path live over the attached RTL8139 device with direct-IP transport (`https://10.0.2.2:8443/...`), TCP request/response exchange, TLS handshake, deterministic pinned-certificate trust, fixed probe time, and allocator-owned response buffering against a deterministic self-hosted TLS harness
