...
pub const DebugInitStage = enum(u8) {
    not_started,
    client_hello_sent,
...
pub fn init(input: *Reader, output: *Writer, options: Options) InitError!Client {
    ...
    var key_share = KeyShare.init(options.entropy[64..if (lightweight_freestanding_keyshare_profile) 128 else 240]) catch |err| switch (err) {
        // Only possible to happen if the seed is all zeroes.
        error.IdentityElement => return error.InsufficientEntropy,
    };

    // The live freestanding guest only needs a lightweight keyshare set to
    // reach ClientHello emission. Keeping hybrid ML-KEM and P-384 out of this
    // path avoids stalling before the first write on QEMU.
    const supported_groups_extension = if (lightweight_freestanding_keyshare_profile)
    ...
    const key_share_extension = if (lightweight_freestanding_keyshare_profile)
    ...
    const extensions_payload = tls.extension(.supported_versions, array(u8, tls.ProtocolVersion, .{
        ...
        .rsa_pkcs1_sha1,
        .ed25519,
    })) ++ supported_groups_extension ++ tls.extension(.psk_key_exchange_modes, array(u8, tls.PskKeyExchangeMode, .{
        .psk_dhe_ke,
    })) ++ key_share_extension;
    ...
    };
    {
        ...
        try output.flush();
        debug_last_init_stage = .client_hello_sent;
    }
            ...
            .handshake => while (true) {
                ...
                switch (handshake_type) {
                    .server_hello => {
                        ...
                        debug_last_init_stage = .server_hello_received;
                    ...
                    .encrypted_extensions => {
                        ...
                        debug_last_init_stage = .encrypted_extensions_received;
                        switch (handshake_cipher) {
                    ...
                    .certificate => cert: {
                        ...
                        debug_last_init_stage = .certificate_received;
                        switch (handshake_cipher) {
                            inline else => |*p| p.transcript_hash.update(wrapped_handshake),
                        }
                    ...
                    .certificate_verify => {
                        ...
                        debug_last_init_stage = .certificate_verify_received;
                        switch (handshake_cipher) {
                            inline else => |*p| {
                    ...
                    .finished => {
                        ...
                        const app_cipher = app_cipher: switch (handshake_cipher) {
                            inline else => |*p, tag| switch (tls_version) {
                                .tls_1_3 => {
                                    ...
                                    debug_last_init_stage = .server_finished_verified;
                                    ...
                                    try output.writeVecAll(&all_msgs_vec);
                                    try output.flush();
                                    debug_last_init_stage = .client_finished_flushed;
                        ...
                        debug_last_init_stage = .init_complete;
                        return .{
                            .input = input,
                            .reader = .{
...
const KeyShare = struct {
    ...
    fn init(seed: []const u8) error{IdentityElement}!KeyShare {
        if (lightweight_freestanding_keyshare_profile) {
            return .{
                .ml_kem768_kp = undefined,
                .secp256r1_kp = undefined,
                .secp384r1_kp = undefined,
                .x25519_kp = try .generateDeterministic(seed[0..32].*),
                .sk_buf = undefined,
                .sk_len = 0,
            };
        }
