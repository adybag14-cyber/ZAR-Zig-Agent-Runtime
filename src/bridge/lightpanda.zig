...
test "provider aliases normalize to canonical bridge providers" {
    ...
    try std.testing.expect(std.mem.eql(u8, try normalizeProvider("qwen-chat-free"), "qwen"));
    try std.testing.expect(std.mem.eql(u8, try normalizeProvider("glm-5"), "zai"));
    try std.testing.expect(std.mem.eql(u8, try normalizeProvider("glm-5-chat-free"), "zai"));
    try std.testing.expect(std.mem.eql(u8, try normalizeProvider("mercury2"), "inception"));
    try std.testing.expect(std.mem.eql(u8, try normalizeProvider("mercury-2-chat-free"), "inception"));
}
