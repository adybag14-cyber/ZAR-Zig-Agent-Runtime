...
const Rtl8139TcpProbeScratch = struct {
    ...
    app_stdout_response_expected_buffer: [128]u8,
...
fn runRtl8139TcpProbe() Rtl8139TcpProbeError!void {
    ...
    const package_display_request = "REQ 42 PKGDISPLAY demo 1280 720";
    const display_set_request = "REQ 43 DISPLAYSET 800 600";
    const package_info_request = "REQ 36 PKGINFO demo";
    const package_dir_request = "REQ 37 LIST /packages/demo";
    const package_asset_relative_path = "config/app.json";
    const package_asset_body = "{\"mode\":\"tcp\"}";
    const package_asset_put_request_id: u32 = 38;
    const package_asset_list_request = "REQ 39 PKGLS demo";
    const package_asset_get_request = "REQ 40 PKGGET demo config/app.json";
    ...
    const package_mutated_install_request_id: u32 = 87;
    const package_restored_asset_get_request = "REQ 88 PKGGET demo config/app.json";
    const package_release_two_name = "r2";
    ...
    const package_mutated_script_body = "mkdir /pkg/out\nwrite-file /pkg/out/result.txt pkg-service-mutated\necho pkg-service-mutated";
    const package_mutated_asset_body = "{\"mode\":\"mutated\"}";
    const package_channel_mutated_script_body = "mkdir /pkg/out\nwrite-file /pkg/out/result.txt pkg-channel-mutated\necho pkg-channel-mutated";
    const package_channel_mutated_asset_body = "{\"mode\":\"channel-mutated\"}";
    const package_output_path = "/pkg/out/result.txt";
    const package_output_expected = "pkg-service-data";
    const package_output_mutated_expected = "pkg-service-mutated";
    const package_output_channel_mutated_expected = "pkg-channel-mutated";
    ...
    const trust_install_backup_request_id: u32 = 46;
    const trust_list_request = "REQ 47 TRUSTLIST";
    const trust_info_request = "REQ 48 TRUSTINFO fs55-root";
    const trust_select_request = "REQ 49 TRUSTSELECT fs55-root";
    const trust_active_request = "REQ 50 TRUSTACTIVE";
    const trust_rotate_request = "REQ 51 TRUSTSELECT fs55-backup";
    const trust_delete_request = "REQ 52 TRUSTDELETE fs55-root";
    const trust_remaining_list_request = "REQ 53 TRUSTLIST";
    const trust_final_active_request = "REQ 54 TRUSTACTIVE";
    const batch_request_id_script: u32 = 57;
    const batch_request_id_package_asset_list: u32 = 58;
    ...
    const app_plan_mutated_install_request_id: u32 = 98;
    const app_plan_mutated_asset_put_request_id: u32 = 99;
    const app_plan_mutated_run_request_id: u32 = 100;
    const app_plan_apply_request_id: u32 = 101;
    const app_plan_active_request_id: u32 = 102;
    const app_plan_app_info_request_id: u32 = 103;
    const app_plan_run_request_id: u32 = 104;
    ...
    const workspace_name = "ops";
    const workspace_save_request_id: u32 = 116;
    ...
    const workspace_release_save_golden_request_id: u32 = 136;
    const workspace_release_save_staging_request_id: u32 = 137;
    const workspace_release_list_request_id: u32 = 138;
    const workspace_release_info_request_id: u32 = 139;
    const workspace_release_activate_request_id: u32 = 140;
    const workspace_release_delete_request_id: u32 = 141;
    const workspace_release_save_fallback_request_id: u32 = 142;
    const workspace_release_prune_request_id: u32 = 143;
    const workspace_release_list_final_request_id: u32 = 144;
    const workspace_release_mutate_request_id: u32 = 145;
    const workspace_release_restored_info_request_id: u32 = 146;
    const workspace_channel_set_staging_request_id: u32 = 147;
    const workspace_channel_list_request_id: u32 = 148;
    const workspace_channel_info_request_id: u32 = 149;
    const workspace_channel_activate_staging_request_id: u32 = 150;
    const workspace_channel_set_golden_request_id: u32 = 151;
    ...
    if (std.mem.indexOf(u8, service_response_long, "package-release-activate") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, service_response_long, "app-autorun-run") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, service_response_long, "trust-delete") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, service_response_long, "app-run") == null) return error.ToolServiceResponseMismatch;

    var service_response_long_offset: usize = 0;
    var service_response_long_chunks: usize = 0;
    while (service_response_long_offset < service_response_long.len) {
    ...
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_info_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_info_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_info_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    const package_info_payload_expected = package_store.manifestAlloc(service_fba.allocator(), package_name, 512) catch return error.ToolServiceFailed;
    const package_info_response_expected = std.fmt.bufPrint(&scratch.package_info_response_expected_buffer, "RESP 36 {d}\n{s}", .{
        package_info_payload_expected.len,
        package_info_payload_expected,
    }) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, package_info_response, package_info_response_expected)) return error.ToolServiceResponseMismatch;

    var package_info_response_offset: usize = 0;
    while (package_info_response_offset < package_info_response.len) {
        const package_info_reply_chunk = server_b.buildPayloadChunk(package_info_response[package_info_response_offset..]) catch |err| return mapTcpSessionProbeError(err);
        const expected_chunk = package_info_response[package_info_response_offset .. package_info_response_offset + package_info_reply_chunk.payload.len];
        _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_info_reply_chunk);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_info_reply_chunk);
        if (!std.mem.eql(u8, package_info_reply_chunk.payload, expected_chunk)) return error.PayloadMismatch;
        const mapped_b_package_info_reply_chunk = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
        if (mapped_b_package_info_reply_chunk != client_b) return error.SessionStateMismatch;
        mapped_b_package_info_reply_chunk.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

        package_info_response_offset += package_info_reply_chunk.payload.len;

        const package_info_reply_chunk_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
        _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_info_reply_chunk_ack);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_info_reply_chunk_ack);
        server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);
    }
    ...
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_app_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_app_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_app_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    const package_app_payload_expected = package_store.appManifestAlloc(service_fba.allocator(), package_name, 256) catch return error.ToolServiceFailed;
    const package_app_response_expected = std.fmt.allocPrint(service_fba.allocator(), "RESP 41 {d}\n{s}", .{
        package_app_payload_expected.len,
        package_app_payload_expected,
    }) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, package_app_response, package_app_response_expected)) return error.ToolServiceResponseMismatch;

    var package_app_response_offset: usize = 0;
    while (package_app_response_offset < package_app_response.len) {
        const package_app_reply_chunk = server_b.buildPayloadChunk(package_app_response[package_app_response_offset..]) catch |err| return mapTcpSessionProbeError(err);
        const expected_chunk = package_app_response[package_app_response_offset .. package_app_response_offset + package_app_reply_chunk.payload.len];
        _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_app_reply_chunk);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_app_reply_chunk);
        if (!std.mem.eql(u8, package_app_reply_chunk.payload, expected_chunk)) return error.PayloadMismatch;
        const mapped_b_package_app_reply_chunk = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
        if (mapped_b_package_app_reply_chunk != client_b) return error.SessionStateMismatch;
        mapped_b_package_app_reply_chunk.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

        package_app_response_offset += package_app_reply_chunk.payload.len;

        const package_app_reply_chunk_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
        _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_app_reply_chunk_ack);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_app_reply_chunk_ack);
        server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);
    }
    if (package_app_response_offset != package_app_response.len) return error.PayloadMismatch;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const package_display_request_payload = client_b.buildPayload(package_display_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_display_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_display_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_display_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    if (!std.mem.eql(u8, package_display_response, "RESP 42 22\nDISPLAY demo 1280x720\n")) return error.ToolServiceResponseMismatch;

    const package_display_reply = server_b.buildPayload(package_display_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_display_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_display_reply);
    const mapped_b_package_display_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_package_display_reply != client_b) return error.SessionStateMismatch;
    mapped_b_package_display_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_display_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_display_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    ...
    if (std.mem.indexOf(u8, package_app_updated, "display_width=1280") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, package_app_updated, "display_height=720") == null) return error.ToolServiceResponseMismatch;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const package_asset_put_request = std.fmt.allocPrint(service_fba.allocator(), "REQ {d} PKGPUT {s} {s} {d}\n{s}", .{
        package_asset_put_request_id,
        package_name,
        package_asset_relative_path,
        package_asset_body.len,
        package_asset_body,
    }) catch return error.ToolServiceFailed;
    const package_asset_put_request_payload = client_b.buildPayload(package_asset_put_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_asset_put_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_asset_put_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_asset_put_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    var package_asset_path_buffer: [filesystem.max_path_len]u8 = undefined;
    const package_asset_path = package_store.assetPath(package_name, package_asset_relative_path, &package_asset_path_buffer) catch return error.ToolServiceFailed;
    const package_asset_put_response_expected = std.fmt.allocPrint(service_fba.allocator(), "RESP {d} 52\nASSET {s} -> {s}\n", .{
        package_asset_put_request_id,
        package_name,
        package_asset_path,
    }) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, package_asset_put_response, package_asset_put_response_expected)) return error.ToolServiceResponseMismatch;

    const package_asset_put_reply = server_b.buildPayload(package_asset_put_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_asset_put_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_asset_put_reply);
    const mapped_b_package_asset_put_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_package_asset_put_reply != client_b) return error.SessionStateMismatch;
    mapped_b_package_asset_put_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_asset_put_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_asset_put_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    ...
    if (!std.mem.eql(u8, package_asset_readback, package_asset_body)) return error.ToolServiceResponseMismatch;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const package_release_save_request = std.fmt.allocPrint(service_fba.allocator(), "REQ {d} PKGRELEASESAVE {s} {s}", .{
        package_release_save_request_id,
        package_name,
        package_release_name,
    }) catch return error.ToolServiceFailed;
    const package_release_save_request_payload = client_b.buildPayload(package_release_save_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_save_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    ...
    const package_release_save_response_expected = std.fmt.allocPrint(service_fba.allocator(), "RESP {d} {d}\n{s}", .{
        package_release_save_request_id,
        package_release_save_payload_expected.len,
        package_release_save_payload_expected,
    }) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, package_release_save_response, package_release_save_response_expected)) return error.ToolServiceResponseMismatch;

    const package_release_save_reply = server_b.buildPayload(package_release_save_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_release_save_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_release_save_reply);
    const mapped_b_package_release_save_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_package_release_save_reply != client_b) return error.SessionStateMismatch;
    mapped_b_package_release_save_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_release_save_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_save_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    ...
    const package_mutated_install_request = std.fmt.allocPrint(service_fba.allocator(), "REQ {d} PKG {s} {d}\n{s}", .{
        package_mutated_install_request_id,
        package_name,
        package_mutated_script_body.len,
        package_mutated_script_body,
    }) catch return error.ToolServiceFailed;
    const package_mutated_install_request_payload = client_b.buildPayload(package_mutated_install_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_mutated_install_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_mutated_install_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_mutated_install_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    const package_mutated_install_response_expected = std.fmt.allocPrint(service_fba.allocator(), "RESP {d} 45\nINSTALLED {s} -> /packages/{s}/bin/main.oc\n", .{
        package_mutated_install_request_id,
        package_name,
        package_name,
    }) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, package_mutated_install_response, package_mutated_install_response_expected)) return error.ToolServiceResponseMismatch;

    const package_mutated_install_reply = server_b.buildPayload(package_mutated_install_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_mutated_install_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_mutated_install_reply);
    const mapped_b_package_mutated_install_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_package_mutated_install_reply != client_b) return error.SessionStateMismatch;
    mapped_b_package_mutated_install_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_mutated_install_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_mutated_install_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_mutated_install_reply_ack);
    server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const package_mutated_asset_put_request = std.fmt.allocPrint(service_fba.allocator(), "REQ {d} PKGPUT {s} {s} {d}\n{s}", .{
        package_mutated_asset_put_request_id,
        package_name,
        package_asset_relative_path,
        package_mutated_asset_body.len,
        package_mutated_asset_body,
    }) catch return error.ToolServiceFailed;
    const package_mutated_asset_put_request_payload = client_b.buildPayload(package_mutated_asset_put_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_mutated_asset_put_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_mutated_asset_put_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_mutated_asset_put_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    const package_mutated_asset_put_response_expected = std.fmt.allocPrint(service_fba.allocator(), "RESP {d} 52\nASSET {s} -> {s}\n", .{
        package_mutated_asset_put_request_id,
        package_name,
        package_asset_path,
    }) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, package_mutated_asset_put_response, package_mutated_asset_put_response_expected)) return error.ToolServiceResponseMismatch;

    const package_mutated_asset_put_reply = server_b.buildPayload(package_mutated_asset_put_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_mutated_asset_put_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_mutated_asset_put_reply);
    const mapped_b_package_mutated_asset_put_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_package_mutated_asset_put_reply != client_b) return error.SessionStateMismatch;
    mapped_b_package_mutated_asset_put_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_mutated_asset_put_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_mutated_asset_put_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_mutated_asset_put_reply_ack);
    server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);
    ...
    const package_release_save_two_request = std.fmt.allocPrint(service_fba.allocator(), "REQ {d} PKGRELEASESAVE {s} {s}", .{
        package_release_save_two_request_id,
        package_name,
        package_release_two_name,
    }) catch return error.ToolServiceFailed;
    const package_release_save_two_request_payload = client_b.buildPayload(package_release_save_two_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_save_two_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_save_two_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_release_save_two_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    const package_release_save_two_payload_expected = std.fmt.allocPrint(service_fba.allocator(), "PKGRELEASESAVE {s} {s}\n", .{
        package_name,
        package_release_two_name,
    }) catch return error.ToolServiceFailed;
    const package_release_save_two_response_expected = std.fmt.allocPrint(service_fba.allocator(), "RESP {d} {d}\n{s}", .{
        package_release_save_two_request_id,
        package_release_save_two_payload_expected.len,
        package_release_save_two_payload_expected,
    }) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, package_release_save_two_response, package_release_save_two_response_expected)) return error.ToolServiceResponseMismatch;

    const package_release_save_two_reply = server_b.buildPayload(package_release_save_two_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_release_save_two_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_release_save_two_reply);
    const mapped_b_package_release_save_two_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_package_release_save_two_reply != client_b) return error.SessionStateMismatch;
    mapped_b_package_release_save_two_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_release_save_two_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_save_two_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_save_two_reply_ack);
    server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const package_release_info_request_payload = client_b.buildPayload(package_release_info_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_info_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_info_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_release_info_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    if (!std.mem.startsWith(u8, package_release_info_response, "RESP 89 ")) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, package_release_info_response, "release=r2") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, package_release_info_response, "saved_seq=2") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, package_release_info_response, "script_checksum=") == null) return error.ToolServiceResponseMismatch;

    var package_release_info_response_offset: usize = 0;
    while (package_release_info_response_offset < package_release_info_response.len) {
        const package_release_info_reply_chunk = server_b.buildPayloadChunk(package_release_info_response[package_release_info_response_offset..]) catch |err| return mapTcpSessionProbeError(err);
        const expected_chunk = package_release_info_response[package_release_info_response_offset .. package_release_info_response_offset + package_release_info_reply_chunk.payload.len];
        _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_release_info_reply_chunk);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_release_info_reply_chunk);
        if (!std.mem.eql(u8, package_release_info_reply_chunk.payload, expected_chunk)) return error.PayloadMismatch;
        const mapped_b_package_release_info_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
        if (mapped_b_package_release_info_reply != client_b) return error.SessionStateMismatch;
        mapped_b_package_release_info_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

        package_release_info_response_offset += package_release_info_reply_chunk.payload.len;

        const package_release_info_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
        _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_info_reply_ack);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_info_reply_ack);
        server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);
    }
    ...
    if (!std.mem.eql(u8, package_output_restored_readback, package_output_expected)) return error.ToolServiceResponseMismatch;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const package_restored_asset_get_request_payload = client_b.buildPayload(package_restored_asset_get_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_restored_asset_get_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_restored_asset_get_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_restored_asset_get_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_restored_asset_get_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_restored_asset_get_reply);
    const mapped_b_package_restored_asset_get_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_package_restored_asset_get_reply != client_b) return error.SessionStateMismatch;
    mapped_b_package_restored_asset_get_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_restored_asset_get_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_restored_asset_get_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_restored_asset_get_reply_ack);
    server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);
    ...
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_save_three_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_release_save_three_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_release_save_three_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_release_save_three_reply);
    const mapped_b_package_release_save_three_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_package_release_save_three_reply != client_b) return error.SessionStateMismatch;
    mapped_b_package_release_save_three_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_release_save_three_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_save_three_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_save_three_reply_ack);
    server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);
    ...
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_delete_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_release_delete_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_release_delete_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_release_delete_reply);
    const mapped_b_package_release_delete_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_package_release_delete_reply != client_b) return error.SessionStateMismatch;
    mapped_b_package_release_delete_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_release_delete_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_delete_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_delete_reply_ack);
    server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const package_release_prune_request_payload = client_b.buildPayload(package_release_prune_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_prune_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_prune_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_release_prune_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    if (!std.mem.eql(u8, package_release_prune_response, "RESP 93 45\nPKGRELEASEPRUNE demo keep=1 deleted=1 kept=1\n")) return error.ToolServiceResponseMismatch;

    const package_release_prune_reply = server_b.buildPayload(package_release_prune_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_release_prune_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_release_prune_reply);
    const mapped_b_package_release_prune_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_package_release_prune_reply != client_b) return error.SessionStateMismatch;
    mapped_b_package_release_prune_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_release_prune_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_prune_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_prune_reply_ack);
    server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const package_release_final_list_request_payload = client_b.buildPayload(package_release_final_list_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_final_list_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_final_list_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_release_final_list_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    if (!std.mem.eql(u8, package_release_final_list_response, "RESP 94 3\nr3\n")) return error.ToolServiceResponseMismatch;

    const package_release_final_list_reply = server_b.buildPayload(package_release_final_list_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_release_final_list_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_release_final_list_reply);
    const mapped_b_package_release_final_list_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_package_release_final_list_reply != client_b) return error.SessionStateMismatch;
    mapped_b_package_release_final_list_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_release_final_list_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_release_final_list_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    ...
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_set_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_channel_set_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_channel_set_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_channel_set_reply);
    const mapped_b_package_channel_set_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_package_channel_set_reply != client_b) return error.SessionStateMismatch;
    mapped_b_package_channel_set_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_channel_set_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_set_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_set_reply_ack);
    server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const package_channel_info_request_payload = client_b.buildPayload(package_channel_info_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_info_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_info_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_channel_info_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    if (!std.mem.startsWith(u8, package_channel_info_response, "RESP 96 ")) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, package_channel_info_response, "channel=stable") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, package_channel_info_response, "release=r3") == null) return error.ToolServiceResponseMismatch;

    const package_channel_info_reply = server_b.buildPayload(package_channel_info_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_channel_info_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_channel_info_reply);
    const mapped_b_package_channel_info_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_package_channel_info_reply != client_b) return error.SessionStateMismatch;
    mapped_b_package_channel_info_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_channel_info_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_info_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_info_reply_ack);
    server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const package_channel_list_request_payload = client_b.buildPayload(package_channel_list_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_list_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_list_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_channel_list_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    if (!std.mem.eql(u8, package_channel_list_response, "RESP 97 7\nstable\n")) return error.ToolServiceResponseMismatch;

    const package_channel_list_reply = server_b.buildPayload(package_channel_list_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_channel_list_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_channel_list_reply);
    const mapped_b_package_channel_list_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_package_channel_list_reply != client_b) return error.SessionStateMismatch;
    mapped_b_package_channel_list_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_channel_list_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_list_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_list_reply_ack);
    server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);
    qemuDebugWrite("T60A\n");

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const package_channel_mutated_install_request = std.fmt.allocPrint(service_fba.allocator(), "REQ {d} PKG {s} {d}\n{s}", .{
        package_channel_mutated_install_request_id,
        package_name,
        package_channel_mutated_script_body.len,
        package_channel_mutated_script_body,
    }) catch return error.ToolServiceFailed;
    const package_channel_mutated_install_request_payload = client_b.buildPayload(package_channel_mutated_install_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_mutated_install_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_mutated_install_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_channel_mutated_install_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    if (std.mem.indexOf(u8, package_channel_mutated_install_response, "INSTALLED demo -> /packages/demo/bin/main.oc\n") == null) return error.ToolServiceResponseMismatch;

    const package_channel_mutated_install_reply = server_b.buildPayload(package_channel_mutated_install_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_channel_mutated_install_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_channel_mutated_install_reply);
    const mapped_b_package_channel_mutated_install_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_package_channel_mutated_install_reply != client_b) return error.SessionStateMismatch;
    mapped_b_package_channel_mutated_install_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_channel_mutated_install_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_mutated_install_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_mutated_install_reply_ack);
    server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const package_channel_mutated_asset_put_request = std.fmt.allocPrint(service_fba.allocator(), "REQ {d} PKGPUT {s} {s} {d}\n{s}", .{
        package_channel_mutated_asset_put_request_id,
        package_name,
        package_asset_relative_path,
        package_channel_mutated_asset_body.len,
        package_channel_mutated_asset_body,
    }) catch return error.ToolServiceFailed;
    const package_channel_mutated_asset_put_request_payload = client_b.buildPayload(package_channel_mutated_asset_put_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_mutated_asset_put_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_mutated_asset_put_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_channel_mutated_asset_put_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    if (!std.mem.eql(u8, package_channel_mutated_asset_put_response, "RESP 100 52\nASSET demo -> /packages/demo/assets/config/app.json\n")) return error.ToolServiceResponseMismatch;

    const package_channel_mutated_asset_put_reply = server_b.buildPayload(package_channel_mutated_asset_put_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_channel_mutated_asset_put_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_channel_mutated_asset_put_reply);
    const mapped_b_package_channel_mutated_asset_put_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_package_channel_mutated_asset_put_reply != client_b) return error.SessionStateMismatch;
    mapped_b_package_channel_mutated_asset_put_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_channel_mutated_asset_put_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_mutated_asset_put_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    ...
    if (!std.mem.eql(u8, package_channel_mutated_output_readback, package_output_channel_mutated_expected)) return error.ToolServiceResponseMismatch;
    qemuDebugWrite("T60B\n");
    ...
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_channel_activate_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_channel_activate_reply);
    const mapped_b_package_channel_activate_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_package_channel_activate_reply != client_b) return error.SessionStateMismatch;
    mapped_b_package_channel_activate_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_channel_activate_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_activate_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    ...
    if (!std.mem.eql(u8, package_channel_restored_output_readback, package_output_expected)) return error.ToolServiceResponseMismatch;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const package_channel_restored_asset_get_request_payload = client_b.buildPayload(package_channel_restored_asset_get_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_restored_asset_get_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_restored_asset_get_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_channel_restored_asset_get_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_channel_restored_asset_get_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_channel_restored_asset_get_reply);
    const mapped_b_package_channel_restored_asset_get_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_package_channel_restored_asset_get_reply != client_b) return error.SessionStateMismatch;
    mapped_b_package_channel_restored_asset_get_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_channel_restored_asset_get_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_restored_asset_get_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_restored_asset_get_reply_ack);
    server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    client_b.local_window = 512;
    const package_channel_window_reopen_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_window_reopen_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_channel_window_reopen_ack);
    server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);
    if (server_b.remote_window != client_b.local_window) return error.WindowUpdateMismatch;
    qemuDebugWrite("T60C\n");

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const package_asset_list_request_payload = client_b.buildPayload(package_asset_list_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_asset_list_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_asset_list_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_asset_list_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    if (!std.mem.eql(u8, package_asset_list_response, "RESP 39 11\ndir config\n")) return error.ToolServiceResponseMismatch;

    const package_asset_list_reply = server_b.buildPayload(package_asset_list_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_asset_list_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_asset_list_reply);
    const mapped_b_package_asset_list_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_package_asset_list_reply != client_b) return error.SessionStateMismatch;
    mapped_b_package_asset_list_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_asset_list_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_asset_list_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    ...
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_asset_get_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_asset_get_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_asset_get_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    const package_asset_get_response_expected = std.fmt.allocPrint(service_fba.allocator(), "RESP 40 {d}\n{s}", .{
        package_asset_body.len,
        package_asset_body,
    }) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, package_asset_get_response, package_asset_get_response_expected)) return error.ToolServiceResponseMismatch;

    const package_asset_get_reply = server_b.buildPayload(package_asset_get_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_asset_get_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_asset_get_reply);
    const mapped_b_package_asset_get_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_package_asset_get_reply != client_b) return error.SessionStateMismatch;
    mapped_b_package_asset_get_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_asset_get_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_asset_get_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_asset_get_reply_ack);
    server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const package_verify_request_payload = client_b.buildPayload(package_verify_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_verify_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_verify_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_verify_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    var package_verify_payload_expected = package_store.verifyPackageAlloc(service_fba.allocator(), package_name, 512) catch return error.ToolServiceFailed;
    defer package_verify_payload_expected.deinit(service_fba.allocator());
    const package_verify_response_expected = std.fmt.allocPrint(service_fba.allocator(), "RESP 34 {d}\n{s}", .{
        package_verify_payload_expected.payload.len,
        package_verify_payload_expected.payload,
    }) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, package_verify_response, package_verify_response_expected)) return error.ToolServiceResponseMismatch;

    var package_verify_response_offset: usize = 0;
    while (package_verify_response_offset < package_verify_response.len) {
        const package_verify_reply_chunk = server_b.buildPayloadChunk(package_verify_response[package_verify_response_offset..]) catch |err| return mapTcpSessionProbeError(err);
        const expected_chunk = package_verify_response[package_verify_response_offset .. package_verify_response_offset + package_verify_reply_chunk.payload.len];
        _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_verify_reply_chunk);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_verify_reply_chunk);
        if (!std.mem.eql(u8, package_verify_reply_chunk.payload, expected_chunk)) return error.PayloadMismatch;
        const mapped_b_package_verify_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
        if (mapped_b_package_verify_reply != client_b) return error.SessionStateMismatch;
        mapped_b_package_verify_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

        package_verify_response_offset += package_verify_reply_chunk.payload.len;

        const package_verify_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
        _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_verify_reply_ack);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_verify_reply_ack);
        server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);
    }
    if (package_verify_response_offset != package_verify_response.len) return error.PayloadMismatch;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const package_dir_request_payload = client_b.buildPayload(package_dir_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_dir_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_dir_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_dir_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    const package_dir_listing = filesystem.listDirectoryAlloc(service_fba.allocator(), "/packages/demo", 128) catch return error.ToolServiceFailed;
    const package_dir_response_expected = std.fmt.allocPrint(service_fba.allocator(), "RESP 37 {d}\n{s}", .{
        package_dir_listing.len,
        package_dir_listing,
    }) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, package_dir_response, package_dir_response_expected)) return error.ToolServiceResponseMismatch;

    const package_dir_reply = server_b.buildPayload(package_dir_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_dir_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, package_dir_reply);
    const mapped_b_package_dir_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_package_dir_reply != client_b) return error.SessionStateMismatch;
    mapped_b_package_dir_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const package_dir_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, package_dir_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    ...
    if (display_state_after_package.current_width != 1280 or display_state_after_package.current_height != 720) {
        return error.ToolServiceResponseMismatch;
    }

    const display_set_request_payload = client_b.buildPayload(display_set_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, display_set_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, display_set_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const display_set_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    if (!std.mem.eql(u8, display_set_response, "RESP 43 16\nDISPLAY 800x600\n")) return error.ToolServiceResponseMismatch;

    const display_set_reply = server_b.buildPayload(display_set_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, display_set_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, display_set_reply);
    const mapped_b_display_set_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_display_set_reply != client_b) return error.SessionStateMismatch;
    mapped_b_display_set_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const display_set_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, display_set_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, display_set_reply_ack);
    server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const display_state_after_set = oc_display_output_state_ptr();
    if (display_state_after_set.current_width != 800 or display_state_after_set.current_height != 600) {
        return error.ToolServiceResponseMismatch;
    }
    ...
    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const trust_install_request = std.fmt.allocPrint(service_fba.allocator(), "REQ {d} TRUSTPUT {s} {d}\n{s}", .{
        trust_install_request_id,
        trust_bundle_name,
        pal_net.httpsProbeTrustAnchorDer().len,
        pal_net.httpsProbeTrustAnchorDer(),
    }) catch return error.ToolServiceFailed;
    const trust_install_request_payload = client_b.buildPayload(trust_install_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_install_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_install_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const trust_install_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        1536,
        256,
    ...
    if (!std.mem.startsWith(u8, trust_install_response, "RESP 45 ")) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, trust_install_response, "/runtime/trust/bundles/fs55-root.der") == null) return error.ToolServiceResponseMismatch;

    var trust_install_response_offset: usize = 0;
    while (trust_install_response_offset < trust_install_response.len) {
        const trust_install_reply_chunk = server_b.buildPayloadChunk(trust_install_response[trust_install_response_offset..]) catch |err| return mapTcpSessionProbeError(err);
        const expected_chunk = trust_install_response[trust_install_response_offset .. trust_install_response_offset + trust_install_reply_chunk.payload.len];
        _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, trust_install_reply_chunk);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, trust_install_reply_chunk);
        if (!std.mem.eql(u8, trust_install_reply_chunk.payload, expected_chunk)) return error.PayloadMismatch;
        const mapped_b_trust_install_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
        if (mapped_b_trust_install_reply != client_b) return error.SessionStateMismatch;
        mapped_b_trust_install_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

        trust_install_response_offset += trust_install_reply_chunk.payload.len;

        const trust_install_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
        _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_install_reply_ack);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_install_reply_ack);
        server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);
    }
    if (trust_install_response_offset != trust_install_response.len) return error.PayloadMismatch;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const trust_install_backup_request = std.fmt.allocPrint(service_fba.allocator(), "REQ {d} TRUSTPUT {s} {d}\n{s}", .{
        trust_install_backup_request_id,
        trust_backup_bundle_name,
        pal_net.httpsProbeTrustAnchorDer().len,
        pal_net.httpsProbeTrustAnchorDer(),
    }) catch return error.ToolServiceFailed;
    const trust_install_backup_request_payload = client_b.buildPayload(trust_install_backup_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_install_backup_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_install_backup_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const trust_install_backup_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        1536,
        256,
    ...
    if (!std.mem.startsWith(u8, trust_install_backup_response, "RESP 46 ")) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, trust_install_backup_response, "/runtime/trust/bundles/fs55-backup.der") == null) return error.ToolServiceResponseMismatch;

    var trust_install_backup_response_offset: usize = 0;
    while (trust_install_backup_response_offset < trust_install_backup_response.len) {
        const trust_install_backup_reply_chunk = server_b.buildPayloadChunk(trust_install_backup_response[trust_install_backup_response_offset..]) catch |err| return mapTcpSessionProbeError(err);
        const expected_chunk = trust_install_backup_response[trust_install_backup_response_offset .. trust_install_backup_response_offset + trust_install_backup_reply_chunk.payload.len];
        _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, trust_install_backup_reply_chunk);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, trust_install_backup_reply_chunk);
        if (!std.mem.eql(u8, trust_install_backup_reply_chunk.payload, expected_chunk)) return error.PayloadMismatch;
        const mapped_b_trust_install_backup_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
        if (mapped_b_trust_install_backup_reply != client_b) return error.SessionStateMismatch;
        mapped_b_trust_install_backup_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

        trust_install_backup_response_offset += trust_install_backup_reply_chunk.payload.len;

        const trust_install_backup_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
        _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_install_backup_reply_ack);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_install_backup_reply_ack);
        server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);
    }
    if (trust_install_backup_response_offset != trust_install_backup_response.len) return error.PayloadMismatch;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    var trust_bundle_path_buffer: [filesystem.max_path_len]u8 = undefined;
    const trust_bundle_path = trust_store.bundlePath(trust_bundle_name, &trust_bundle_path_buffer) catch return error.ToolServiceFailed;
    const trust_bundle_readback = filesystem.readFileAlloc(service_fba.allocator(), trust_bundle_path, pal_net.httpsProbeTrustAnchorDer().len) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, trust_bundle_readback, pal_net.httpsProbeTrustAnchorDer())) return error.ToolServiceResponseMismatch;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    var trust_backup_bundle_path_buffer: [filesystem.max_path_len]u8 = undefined;
    const trust_backup_bundle_path = trust_store.bundlePath(trust_backup_bundle_name, &trust_backup_bundle_path_buffer) catch return error.ToolServiceFailed;
    const trust_backup_bundle_readback = filesystem.readFileAlloc(service_fba.allocator(), trust_backup_bundle_path, pal_net.httpsProbeTrustAnchorDer().len) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, trust_backup_bundle_readback, pal_net.httpsProbeTrustAnchorDer())) return error.ToolServiceResponseMismatch;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const trust_list_request_payload = client_b.buildPayload(trust_list_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_list_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_list_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const trust_list_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    if (!std.mem.eql(u8, trust_list_response, "RESP 47 22\nfs55-root\nfs55-backup\n")) return error.ToolServiceResponseMismatch;

    const trust_list_reply = server_b.buildPayload(trust_list_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, trust_list_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, trust_list_reply);
    const mapped_b_trust_list_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_trust_list_reply != client_b) return error.SessionStateMismatch;
    mapped_b_trust_list_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const trust_list_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_list_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_list_reply_ack);
    server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const trust_info_request_payload = client_b.buildPayload(trust_info_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_info_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_info_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const trust_info_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    if (!std.mem.startsWith(u8, trust_info_response, "RESP 48 ")) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, trust_info_response, "name=fs55-root") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, trust_info_response, "selected=0") == null) return error.ToolServiceResponseMismatch;

    var trust_info_response_offset: usize = 0;
    while (trust_info_response_offset < trust_info_response.len) {
        const trust_info_reply_chunk = server_b.buildPayloadChunk(trust_info_response[trust_info_response_offset..]) catch |err| return mapTcpSessionProbeError(err);
        const expected_chunk = trust_info_response[trust_info_response_offset .. trust_info_response_offset + trust_info_reply_chunk.payload.len];
        _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, trust_info_reply_chunk);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, trust_info_reply_chunk);
        if (!std.mem.eql(u8, trust_info_reply_chunk.payload, expected_chunk)) return error.PayloadMismatch;
        const mapped_b_trust_info_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
        if (mapped_b_trust_info_reply != client_b) return error.SessionStateMismatch;
        mapped_b_trust_info_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

        trust_info_response_offset += trust_info_reply_chunk.payload.len;

        const trust_info_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
        _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_info_reply_ack);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_info_reply_ack);
        server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);
    }
    if (trust_info_response_offset != trust_info_response.len) return error.PayloadMismatch;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const trust_select_request_payload = client_b.buildPayload(trust_select_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_select_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_select_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const trust_select_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    if (!std.mem.eql(u8, trust_select_response, "RESP 49 19\nSELECTED fs55-root\n")) return error.ToolServiceResponseMismatch;

    const trust_select_reply = server_b.buildPayload(trust_select_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, trust_select_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, trust_select_reply);
    const mapped_b_trust_select_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_trust_select_reply != client_b) return error.SessionStateMismatch;
    mapped_b_trust_select_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const trust_select_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_select_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_select_reply_ack);
    server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const trust_active_request_payload = client_b.buildPayload(trust_active_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_active_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_active_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const trust_active_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    if (!std.mem.startsWith(u8, trust_active_response, "RESP 50 ")) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, trust_active_response, "name=fs55-root") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, trust_active_response, "selected=1") == null) return error.ToolServiceResponseMismatch;

    var trust_active_response_offset: usize = 0;
    while (trust_active_response_offset < trust_active_response.len) {
        const trust_active_reply_chunk = server_b.buildPayloadChunk(trust_active_response[trust_active_response_offset..]) catch |err| return mapTcpSessionProbeError(err);
        const expected_chunk = trust_active_response[trust_active_response_offset .. trust_active_response_offset + trust_active_reply_chunk.payload.len];
        _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, trust_active_reply_chunk);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, trust_active_reply_chunk);
        if (!std.mem.eql(u8, trust_active_reply_chunk.payload, expected_chunk)) return error.PayloadMismatch;
        const mapped_b_trust_active_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
        if (mapped_b_trust_active_reply != client_b) return error.SessionStateMismatch;
        mapped_b_trust_active_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

        trust_active_response_offset += trust_active_reply_chunk.payload.len;

        const trust_active_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
        _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_active_reply_ack);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_active_reply_ack);
        server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);
    }
    if (trust_active_response_offset != trust_active_response.len) return error.PayloadMismatch;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const trust_rotate_request_payload = client_b.buildPayload(trust_rotate_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_rotate_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_rotate_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const trust_rotate_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    if (!std.mem.eql(u8, trust_rotate_response, "RESP 51 21\nSELECTED fs55-backup\n")) return error.ToolServiceResponseMismatch;

    const trust_rotate_reply = server_b.buildPayload(trust_rotate_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, trust_rotate_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, trust_rotate_reply);
    const mapped_b_trust_rotate_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_trust_rotate_reply != client_b) return error.SessionStateMismatch;
    mapped_b_trust_rotate_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const trust_rotate_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_rotate_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_rotate_reply_ack);
    server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const active_trust_path = trust_store.activeBundlePathAlloc(service_fba.allocator(), 128) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, active_trust_path, trust_backup_bundle_path)) return error.ToolServiceResponseMismatch;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const trust_delete_request_payload = client_b.buildPayload(trust_delete_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_delete_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_delete_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const trust_delete_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    if (!std.mem.eql(u8, trust_delete_response, "RESP 52 18\nDELETED fs55-root\n")) return error.ToolServiceResponseMismatch;

    const trust_delete_reply = server_b.buildPayload(trust_delete_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, trust_delete_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, trust_delete_reply);
    const mapped_b_trust_delete_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_trust_delete_reply != client_b) return error.SessionStateMismatch;
    mapped_b_trust_delete_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const trust_delete_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_delete_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_delete_reply_ack);
    server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const deleted_trust_bundle_readback = filesystem.readFileAlloc(service_fba.allocator(), trust_bundle_path, pal_net.httpsProbeTrustAnchorDer().len) catch |err| switch (err) {
        error.FileNotFound => null,
        else => return error.ToolServiceFailed,
    };
    if (deleted_trust_bundle_readback != null) return error.ToolServiceResponseMismatch;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const trust_remaining_list_request_payload = client_b.buildPayload(trust_remaining_list_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_remaining_list_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_remaining_list_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const trust_remaining_list_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    if (!std.mem.eql(u8, trust_remaining_list_response, "RESP 53 12\nfs55-backup\n")) return error.ToolServiceResponseMismatch;

    const trust_remaining_list_reply = server_b.buildPayload(trust_remaining_list_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, trust_remaining_list_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, trust_remaining_list_reply);
    const mapped_b_trust_remaining_list_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_trust_remaining_list_reply != client_b) return error.SessionStateMismatch;
    mapped_b_trust_remaining_list_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const trust_remaining_list_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_remaining_list_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_remaining_list_reply_ack);
    server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const trust_final_active_request_payload = client_b.buildPayload(trust_final_active_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_final_active_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_final_active_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const trust_final_active_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    if (!std.mem.startsWith(u8, trust_final_active_response, "RESP 54 ")) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, trust_final_active_response, "name=fs55-backup") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, trust_final_active_response, "selected=1") == null) return error.ToolServiceResponseMismatch;

    var trust_final_active_response_offset: usize = 0;
    while (trust_final_active_response_offset < trust_final_active_response.len) {
        const trust_final_active_reply_chunk = server_b.buildPayloadChunk(trust_final_active_response[trust_final_active_response_offset..]) catch |err| return mapTcpSessionProbeError(err);
        const expected_chunk = trust_final_active_response[trust_final_active_response_offset .. trust_final_active_response_offset + trust_final_active_reply_chunk.payload.len];
        _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, trust_final_active_reply_chunk);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, trust_final_active_reply_chunk);
        if (!std.mem.eql(u8, trust_final_active_reply_chunk.payload, expected_chunk)) return error.PayloadMismatch;
        const mapped_b_trust_final_active_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
        if (mapped_b_trust_final_active_reply != client_b) return error.SessionStateMismatch;
        mapped_b_trust_final_active_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

        trust_final_active_response_offset += trust_final_active_reply_chunk.payload.len;

        const trust_final_active_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
        _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_final_active_reply_ack);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, trust_final_active_reply_ack);
        server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);
    }
    ...
    const batch_service_request = std.fmt.bufPrint(&scratch.service_batch_request_buffer, "REQ {d} GET {s}\nREQ {d} PKGLS {s}\nREQ {d} PKGGET {s} {s}\nREQ {d} DISPLAYINFO\nREQ {d} DISPLAYMODES", .{
        ...
        package_name,
        batch_request_id_package_asset_get,
        package_name,
        package_asset_relative_path,
    ...
    const display_info_payload_expected = std.fmt.bufPrint(
        ...
        .{
            ...
            display_state.scanout_count,
            display_state.active_scanout,
    ...
    while (display_mode_index < oc_framebuffer_supported_mode_count()) : (display_mode_index += 1) {
        const line = std.fmt.bufPrint(
            display_modes_payload_buffer[display_modes_payload_len..],
            "mode {d} {d}x{d}\n",
            .{
                display_mode_index,
                oc_framebuffer_supported_mode_width(display_mode_index),
                oc_framebuffer_supported_mode_height(display_mode_index),
            },
        ) catch return error.ToolServiceFailed;
    ...
    const app_batch_service_request = std.fmt.bufPrint(&scratch.service_batch_request_buffer, "REQ {d} APPCONNECTOR {s} virtual\nREQ {d} APPTRUST {s} {s}\nREQ {d} APPINFO {s}\nREQ {d} APPRUN {s}\nREQ {d} APPSTATE {s}\nREQ {d} APPHISTORY {s}\nREQ {d} APPSTDOUT {s}\nREQ {d} APPSTDERR {s}\nREQ {d} APPLIST", .{
        ...
        package_name,
        batch_request_id_app_stdout,
        package_name,
        batch_request_id_app_stderr,
        package_name,
        batch_request_id_app_list,
    }) catch return error.ToolServiceFailed;
    const app_batch_service_request_payload = client_b.buildPayload(app_batch_service_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_batch_service_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_batch_service_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const app_batch_service_response = tool_service.handleFramedRequestBatch(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    const app_state_payload_expected = std.fmt.bufPrint(
        &scratch.app_state_response_expected_buffer,
        "name={s}\nexit_code=0\nstdout_bytes={d}\nstderr_bytes=0\ndisplay_width={d}\ndisplay_height={d}\nrequested_connector=virtual\nactual_connector=virtual\ntrust_bundle={s}\n",
        .{
            package_name,
    ...
    const app_stdout_payload_expected = std.fmt.bufPrint(
        &scratch.app_stdout_response_expected_buffer,
        "{s}",
        .{package_run_stdout},
    ) catch return error.ToolServiceFailed;
    const app_stderr_payload_expected = std.fmt.bufPrint(
        &scratch.app_stderr_response_expected_buffer,
        "{s}",
        .{""},
    ) catch return error.ToolServiceFailed;
    const app_batch_service_response_expected = std.fmt.bufPrint(&scratch.service_batch_response_expected_buffer, "RESP {d} 26\nAPPCONNECTOR {s} virtual\nRESP {d} 26\nAPPTRUST {s} {s}\nRESP {d} {d}\n{s}RESP {d} {d}\n{s}RESP {d} {d}\n{s}RESP {d} {d}\n{s}RESP {d} {d}\n{s}RESP {d} {d}\n{s}RESP {d} 9\ndemo\naux\n", .{
        ...
        trust_backup_bundle_name,
        batch_request_id_app_info,
        app_info_payload_expected.len,
        app_info_payload_expected,
        batch_request_id_app_run,
        package_run_stdout.len,
        package_run_stdout,
        batch_request_id_app_state,
        ...
        batch_request_id_app_stdout,
        app_stdout_payload_expected.len,
        app_stdout_payload_expected,
        batch_request_id_app_stderr,
        app_stderr_payload_expected.len,
        app_stderr_payload_expected,
        batch_request_id_app_list,
    }) catch return error.ToolServiceFailed;
    ...
    var app_batch_service_response_offset: usize = 0;
    var app_batch_service_response_chunks: usize = 0;
    while (app_batch_service_response_offset < app_batch_service_response.len) {
        const app_batch_service_reply_chunk = server_b.buildPayloadChunk(app_batch_service_response[app_batch_service_response_offset..]) catch |err| return mapTcpSessionProbeError(err);
        const expected_chunk = app_batch_service_response[app_batch_service_response_offset .. app_batch_service_response_offset + app_batch_service_reply_chunk.payload.len];
        _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, app_batch_service_reply_chunk);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, app_batch_service_reply_chunk);
        if (!std.mem.eql(u8, app_batch_service_reply_chunk.payload, expected_chunk)) return error.PayloadMismatch;
        const mapped_b_app_batch_service_reply_chunk = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
        if (mapped_b_app_batch_service_reply_chunk != client_b) return error.SessionStateMismatch;
        mapped_b_app_batch_service_reply_chunk.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

        app_batch_service_response_offset += app_batch_service_reply_chunk.payload.len;
        app_batch_service_response_chunks += 1;

        const app_batch_service_reply_chunk_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
        _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_batch_service_reply_chunk_ack);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_batch_service_reply_chunk_ack);
        server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);
    }
    if (app_batch_service_response_offset != app_batch_service_response.len) return error.PayloadMismatch;
    if (app_batch_service_response_chunks < 2) return error.CounterMismatch;
    ...
    const app_stdout_readback = filesystem.readFileAlloc(service_fba.allocator(), "/runtime/apps/demo/stdout.log", 128) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, app_stdout_readback, app_stdout_payload_expected)) return error.ToolServiceResponseMismatch;

    const app_stderr_readback = filesystem.readFileAlloc(service_fba.allocator(), "/runtime/apps/demo/stderr.log", 64) catch return error.ToolServiceFailed;
    ...
    const app_plan_batch_service_request = std.fmt.bufPrint(
        &scratch.service_batch_request_buffer,
        "REQ {d} APPPLANSAVE {s} {s} {s} {s} virtual 1280 720 1\nREQ {d} APPPLANLIST {s}\nREQ {d} APPPLANINFO {s} {s}",
        .{
            app_plan_save_request_id,
            package_name,
            app_plan_name,
            package_release_three_name,
            trust_backup_bundle_name,
            app_plan_list_request_id,
            package_name,
            app_plan_info_request_id,
            package_name,
            app_plan_name,
        },
    ) catch return error.ToolServiceFailed;
    const app_plan_batch_service_request_payload = client_b.buildPayload(app_plan_batch_service_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_batch_service_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_batch_service_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const app_plan_batch_service_response = tool_service.handleFramedRequestBatch(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    var app_plan_path_buffer: [filesystem.max_path_len]u8 = undefined;
    const app_plan_path = std.fmt.bufPrint(&app_plan_path_buffer, "/runtime/apps/{s}/plans/{s}.txt", .{ package_name, app_plan_name }) catch return error.ToolServiceFailed;
    var app_plan_save_payload_buffer: [40]u8 = undefined;
    const app_plan_save_payload_expected = std.fmt.bufPrint(&app_plan_save_payload_buffer, "APPPLANSAVE {s} {s}\n", .{ package_name, app_plan_name }) catch return error.ToolServiceFailed;
    const app_plan_list_payload_expected = "golden\n";
    var app_plan_info_payload_buffer: [256]u8 = undefined;
    const app_plan_info_payload_expected = std.fmt.bufPrint(
        &app_plan_info_payload_buffer,
        "package={s}\nplan={s}\nrelease={s}\ntrust_bundle={s}\nconnector=virtual\ndisplay_width=1280\ndisplay_height=720\nautorun=1\npath={s}\n",
        .{ package_name, app_plan_name, package_release_three_name, trust_backup_bundle_name, app_plan_path },
    ) catch return error.ToolServiceFailed;
    const app_plan_batch_service_response_expected = std.fmt.bufPrint(
        &scratch.service_batch_response_expected_buffer,
        "RESP {d} {d}\n{s}RESP {d} {d}\n{s}RESP {d} {d}\n{s}",
        .{
            app_plan_save_request_id,
            app_plan_save_payload_expected.len,
            app_plan_save_payload_expected,
            app_plan_list_request_id,
            app_plan_list_payload_expected.len,
            app_plan_list_payload_expected,
            app_plan_info_request_id,
            app_plan_info_payload_expected.len,
            app_plan_info_payload_expected,
        },
    ) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, app_plan_batch_service_response, app_plan_batch_service_response_expected)) return error.ToolServiceResponseMismatch;

    var app_plan_batch_service_response_offset: usize = 0;
    while (app_plan_batch_service_response_offset < app_plan_batch_service_response.len) {
        const app_plan_batch_service_reply_chunk = server_b.buildPayloadChunk(app_plan_batch_service_response[app_plan_batch_service_response_offset..]) catch |err| return mapTcpSessionProbeError(err);
        const expected_chunk = app_plan_batch_service_response[app_plan_batch_service_response_offset .. app_plan_batch_service_response_offset + app_plan_batch_service_reply_chunk.payload.len];
        _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, app_plan_batch_service_reply_chunk);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, app_plan_batch_service_reply_chunk);
        if (!std.mem.eql(u8, app_plan_batch_service_reply_chunk.payload, expected_chunk)) return error.PayloadMismatch;
        const mapped_b_app_plan_batch_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
        if (mapped_b_app_plan_batch_reply != client_b) return error.SessionStateMismatch;
        mapped_b_app_plan_batch_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

        app_plan_batch_service_response_offset += app_plan_batch_service_reply_chunk.payload.len;

        const app_plan_batch_service_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
        _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_batch_service_reply_ack);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_batch_service_reply_ack);
        server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);
    }
    ...
    const app_plan_mutated_install_request = std.fmt.allocPrint(
        service_fba.allocator(),
        "REQ {d} PKG {s} {d}\n{s}",
        .{ app_plan_mutated_install_request_id, package_name, package_mutated_script_body.len, package_mutated_script_body },
    ) catch return error.ToolServiceFailed;
    const app_plan_mutated_install_request_payload = client_b.buildPayload(app_plan_mutated_install_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_mutated_install_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_mutated_install_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const app_plan_mutated_install_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    const app_plan_mutated_install_response_expected = std.fmt.allocPrint(
        service_fba.allocator(),
        "RESP {d} 45\nINSTALLED {s} -> /packages/{s}/bin/main.oc\n",
        .{ app_plan_mutated_install_request_id, package_name, package_name },
    ) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, app_plan_mutated_install_response, app_plan_mutated_install_response_expected)) return error.ToolServiceResponseMismatch;

    var app_plan_mutated_install_response_offset: usize = 0;
    while (app_plan_mutated_install_response_offset < app_plan_mutated_install_response.len) {
        const app_plan_mutated_install_reply_chunk = server_b.buildPayloadChunk(app_plan_mutated_install_response[app_plan_mutated_install_response_offset..]) catch |err| return mapTcpSessionProbeError(err);
        const expected_chunk = app_plan_mutated_install_response[app_plan_mutated_install_response_offset .. app_plan_mutated_install_response_offset + app_plan_mutated_install_reply_chunk.payload.len];
        _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, app_plan_mutated_install_reply_chunk);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, app_plan_mutated_install_reply_chunk);
        if (!std.mem.eql(u8, app_plan_mutated_install_reply_chunk.payload, expected_chunk)) return error.PayloadMismatch;
        const mapped_b_app_plan_mutated_install_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
        if (mapped_b_app_plan_mutated_install_reply != client_b) return error.SessionStateMismatch;
        mapped_b_app_plan_mutated_install_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

        app_plan_mutated_install_response_offset += app_plan_mutated_install_reply_chunk.payload.len;

        const app_plan_mutated_install_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
        _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_mutated_install_reply_ack);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_mutated_install_reply_ack);
        server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);
    }
    ...
    const app_plan_mutated_asset_put_request = std.fmt.allocPrint(
        ...
        "REQ {d} PKGPUT {s} {s} {d}\n{s}",
        .{
            app_plan_mutated_asset_put_request_id,
            package_name,
            package_asset_relative_path,
            package_mutated_asset_body.len,
            package_mutated_asset_body,
    ...
    const app_plan_mutated_asset_put_request_payload = client_b.buildPayload(app_plan_mutated_asset_put_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_mutated_asset_put_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_mutated_asset_put_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const app_plan_mutated_asset_put_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    const app_plan_mutated_asset_put_payload_expected = std.fmt.allocPrint(
        service_fba.allocator(),
        "ASSET {s} -> {s}\n",
        .{ package_name, package_asset_path },
    ) catch return error.ToolServiceFailed;
    const app_plan_mutated_asset_put_response_expected = std.fmt.allocPrint(
        service_fba.allocator(),
        "RESP {d} {d}\n{s}",
        .{
            app_plan_mutated_asset_put_request_id,
            app_plan_mutated_asset_put_payload_expected.len,
            app_plan_mutated_asset_put_payload_expected,
        },
    ) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, app_plan_mutated_asset_put_response, app_plan_mutated_asset_put_response_expected)) return error.ToolServiceResponseMismatch;

    var app_plan_mutated_asset_put_response_offset: usize = 0;
    while (app_plan_mutated_asset_put_response_offset < app_plan_mutated_asset_put_response.len) {
        const app_plan_mutated_asset_put_reply_chunk = server_b.buildPayloadChunk(app_plan_mutated_asset_put_response[app_plan_mutated_asset_put_response_offset..]) catch |err| return mapTcpSessionProbeError(err);
        const expected_chunk = app_plan_mutated_asset_put_response[app_plan_mutated_asset_put_response_offset .. app_plan_mutated_asset_put_response_offset + app_plan_mutated_asset_put_reply_chunk.payload.len];
        _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, app_plan_mutated_asset_put_reply_chunk);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, app_plan_mutated_asset_put_reply_chunk);
        if (!std.mem.eql(u8, app_plan_mutated_asset_put_reply_chunk.payload, expected_chunk)) return error.PayloadMismatch;
        const mapped_b_app_plan_mutated_asset_put_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
        if (mapped_b_app_plan_mutated_asset_put_reply != client_b) return error.SessionStateMismatch;
        mapped_b_app_plan_mutated_asset_put_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

        app_plan_mutated_asset_put_response_offset += app_plan_mutated_asset_put_reply_chunk.payload.len;

        const app_plan_mutated_asset_put_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
        _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_mutated_asset_put_reply_ack);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_mutated_asset_put_reply_ack);
        server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);
    }
    ...
    const app_plan_mutated_run_request_payload = client_b.buildPayload(app_plan_mutated_run_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_mutated_run_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_mutated_run_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const app_plan_mutated_run_response = tool_service.handleFramedRequest(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    if (!std.mem.startsWith(u8, app_plan_mutated_run_response, "RESP 100 ")) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, app_plan_mutated_run_response, "pkg-service-mutated\n") == null) return error.ToolServiceResponseMismatch;

    var app_plan_mutated_run_response_offset: usize = 0;
    while (app_plan_mutated_run_response_offset < app_plan_mutated_run_response.len) {
        const app_plan_mutated_run_reply_chunk = server_b.buildPayloadChunk(app_plan_mutated_run_response[app_plan_mutated_run_response_offset..]) catch |err| return mapTcpSessionProbeError(err);
        const expected_chunk = app_plan_mutated_run_response[app_plan_mutated_run_response_offset .. app_plan_mutated_run_response_offset + app_plan_mutated_run_reply_chunk.payload.len];
        _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, app_plan_mutated_run_reply_chunk);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, app_plan_mutated_run_reply_chunk);
        if (!std.mem.eql(u8, app_plan_mutated_run_reply_chunk.payload, expected_chunk)) return error.PayloadMismatch;
        const mapped_b_app_plan_mutated_run_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
        if (mapped_b_app_plan_mutated_run_reply != client_b) return error.SessionStateMismatch;
        mapped_b_app_plan_mutated_run_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

        app_plan_mutated_run_response_offset += app_plan_mutated_run_reply_chunk.payload.len;

        const app_plan_mutated_run_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
        _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_mutated_run_reply_ack);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_mutated_run_reply_ack);
        server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);
    }
    if (app_plan_mutated_run_response_offset != app_plan_mutated_run_response.len) return error.PayloadMismatch;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const app_plan_mutated_asset_readback = filesystem.readFileAlloc(service_fba.allocator(), package_asset_path, 128) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, app_plan_mutated_asset_readback, package_mutated_asset_body)) return error.ToolServiceResponseMismatch;
    ...
    const app_plan_apply_batch_request = std.fmt.bufPrint(
        &scratch.service_batch_request_buffer,
        "REQ {d} APPPLANAPPLY {s} {s}\nREQ {d} APPPLANACTIVE {s}\nREQ {d} APPINFO {s}\nREQ {d} APPRUN {s}\nREQ {d} APPAUTORUNLIST",
        .{
            app_plan_apply_request_id,
            package_name,
            app_plan_name,
            app_plan_active_request_id,
            package_name,
            app_plan_app_info_request_id,
            package_name,
            app_plan_run_request_id,
            package_name,
            app_plan_autorun_list_request_id,
        },
    ) catch return error.ToolServiceFailed;
    const app_plan_apply_batch_request_payload = client_b.buildPayload(app_plan_apply_batch_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_apply_batch_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_apply_batch_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const app_plan_apply_batch_response = tool_service.handleFramedRequestBatch(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    var app_plan_apply_payload_buffer: [40]u8 = undefined;
    const app_plan_apply_payload_expected = std.fmt.bufPrint(&app_plan_apply_payload_buffer, "APPPLANAPPLY {s} {s}\n", .{ package_name, app_plan_name }) catch return error.ToolServiceFailed;
    var app_plan_active_payload_buffer: [256]u8 = undefined;
    const app_plan_active_payload_expected = std.fmt.bufPrint(
        &app_plan_active_payload_buffer,
        "active_plan={s}\npackage={s}\nplan={s}\nrelease={s}\ntrust_bundle={s}\nconnector=virtual\ndisplay_width=1280\ndisplay_height=720\nautorun=1\npath={s}\n",
        .{ app_plan_name, package_name, app_plan_name, package_release_three_name, trust_backup_bundle_name, app_plan_path },
    ) catch return error.ToolServiceFailed;
    const app_plan_autorun_list_payload_expected = "aux\ndemo\n";
    const app_plan_apply_batch_response_expected = std.fmt.bufPrint(
        &scratch.service_batch_response_expected_buffer,
        "RESP {d} {d}\n{s}RESP {d} {d}\n{s}RESP {d} {d}\n{s}RESP {d} {d}\n{s}RESP {d} {d}\n{s}",
        .{
            app_plan_apply_request_id,
            app_plan_apply_payload_expected.len,
            app_plan_apply_payload_expected,
            app_plan_active_request_id,
            app_plan_active_payload_expected.len,
            app_plan_active_payload_expected,
            app_plan_app_info_request_id,
            app_info_payload_expected.len,
            app_info_payload_expected,
            app_plan_run_request_id,
            ...
            package_run_stdout,
            app_plan_autorun_list_request_id,
            app_plan_autorun_list_payload_expected.len,
            app_plan_autorun_list_payload_expected,
        },
    ) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, app_plan_apply_batch_response, app_plan_apply_batch_response_expected)) return error.ToolServiceResponseMismatch;

    var app_plan_apply_batch_response_offset: usize = 0;
    while (app_plan_apply_batch_response_offset < app_plan_apply_batch_response.len) {
        const app_plan_apply_batch_reply_chunk = server_b.buildPayloadChunk(app_plan_apply_batch_response[app_plan_apply_batch_response_offset..]) catch |err| return mapTcpSessionProbeError(err);
        const expected_chunk = app_plan_apply_batch_response[app_plan_apply_batch_response_offset .. app_plan_apply_batch_response_offset + app_plan_apply_batch_reply_chunk.payload.len];
        _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, app_plan_apply_batch_reply_chunk);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, app_plan_apply_batch_reply_chunk);
        if (!std.mem.eql(u8, app_plan_apply_batch_reply_chunk.payload, expected_chunk)) return error.PayloadMismatch;
        const mapped_b_app_plan_apply_batch_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
        if (mapped_b_app_plan_apply_batch_reply != client_b) return error.SessionStateMismatch;
        mapped_b_app_plan_apply_batch_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

        app_plan_apply_batch_response_offset += app_plan_apply_batch_reply_chunk.payload.len;

        const app_plan_apply_batch_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
        _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_apply_batch_reply_ack);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_apply_batch_reply_ack);
        server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);
    }
    ...
    const app_plan_file_payload_expected = std.fmt.bufPrint(
        &app_plan_file_payload_buffer,
        "package={s}\nplan={s}\nrelease={s}\ntrust_bundle={s}\nconnector=virtual\ndisplay_width=1280\ndisplay_height=720\nautorun=1\n",
        .{ package_name, app_plan_name, package_release_three_name, trust_backup_bundle_name },
    ) catch return error.ToolServiceFailed;
    const app_plan_file_readback = filesystem.readFileAlloc(service_fba.allocator(), app_plan_path, 192) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, app_plan_file_readback, app_plan_file_payload_expected)) return error.ToolServiceResponseMismatch;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const app_plan_active_readback = filesystem.readFileAlloc(service_fba.allocator(), "/runtime/apps/demo/active_plan.txt", 32) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, app_plan_active_readback, app_plan_name)) return error.ToolServiceResponseMismatch;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const app_plan_restored_asset_readback = filesystem.readFileAlloc(service_fba.allocator(), package_asset_path, 128) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, app_plan_restored_asset_readback, package_asset_body)) return error.ToolServiceResponseMismatch;
    ...
    const workspace_save_request = std.fmt.bufPrint(
        &workspace_save_request_buffer,
        "REQ {d} WORKSPACESAVE {s} {s} {s} 1024 768 {s}:{s}:{s}",
        .{ workspace_save_request_id, workspace_name, app_suite_name, trust_backup_bundle_name, package_name, package_channel_name, package_release_three_name },
    ) catch return error.ToolServiceFailed;
    const workspace_save_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
    ...
    const workspace_list_request = std.fmt.bufPrint(
        &workspace_list_request_buffer,
        "REQ {d} WORKSPACELIST",
        .{workspace_list_request_id},
    ) catch return error.ToolServiceFailed;
    const workspace_list_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
    ...
    if (!std.mem.eql(u8, workspace_list_response, "RESP 117 4\nops\n")) return error.ToolServiceResponseMismatch;
    ...
    const workspace_info_response_expected = std.fmt.bufPrint(
        ...
        "RESP {d} {d}\n{s}",
        .{ workspace_info_request_id, workspace_info_payload_expected.len, workspace_info_payload_expected },
    ) catch return error.ToolServiceFailed;
    var workspace_info_request_buffer: [64]u8 = undefined;
    const workspace_info_request = std.fmt.bufPrint(
        &workspace_info_request_buffer,
        "REQ {d} WORKSPACEINFO {s}",
        .{ workspace_info_request_id, workspace_name },
    ) catch return error.ToolServiceFailed;
    const workspace_info_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
    ...
    const workspace_release_golden_save_request = std.fmt.bufPrint(
        &workspace_release_golden_save_request_buffer,
        "REQ {d} WORKSPACERELEASESAVE {s} golden",
        .{ workspace_release_save_golden_request_id, workspace_name },
    ) catch return error.ToolServiceFailed;
    const workspace_release_golden_save_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
        source_ip,
        destination_ip,
        workspace_release_golden_save_request,
        512,
        256,
        512,
    );
    if (!std.mem.startsWith(u8, workspace_release_golden_save_response, "RESP 136 ")) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_release_golden_save_response, "WORKSPACERELEASESAVE ops golden\n") == null) return error.ToolServiceResponseMismatch;

    var workspace_release_golden_path_buffer: [filesystem.max_path_len]u8 = undefined;
    const workspace_release_golden_path = std.fmt.bufPrint(
        &workspace_release_golden_path_buffer,
        "/runtime/workspace-releases/{s}/golden/workspace.txt",
        .{workspace_name},
    ) catch return error.ToolServiceFailed;
    var workspace_release_golden_metadata_path_buffer: [filesystem.max_path_len]u8 = undefined;
    const workspace_release_golden_metadata_path = std.fmt.bufPrint(
        &workspace_release_golden_metadata_path_buffer,
        "/runtime/workspace-releases/{s}/golden/release.txt",
        .{workspace_name},
    ) catch return error.ToolServiceFailed;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const workspace_release_golden_readback = filesystem.readFileAlloc(service_fba.allocator(), workspace_release_golden_path, 256) catch return error.ToolServiceFailed;
    if (std.mem.indexOf(u8, workspace_release_golden_readback, "trust_bundle=fs55-backup") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_release_golden_readback, "display_width=1024") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_release_golden_readback, "display_height=768") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_release_golden_readback, "channel=demo:stable:r3") == null) return error.ToolServiceResponseMismatch;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const workspace_release_golden_metadata_readback = filesystem.readFileAlloc(service_fba.allocator(), workspace_release_golden_metadata_path, 128) catch return error.ToolServiceFailed;
    if (std.mem.indexOf(u8, workspace_release_golden_metadata_readback, "release=golden") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_release_golden_metadata_readback, "saved_seq=1") == null) return error.ToolServiceResponseMismatch;

    var workspace_release_mutate_request_buffer: [160]u8 = undefined;
    const workspace_release_mutate_request = std.fmt.bufPrint(
        &workspace_release_mutate_request_buffer,
        "REQ {d} WORKSPACESAVE {s} {s} {s} 640 400 {s}:{s}:{s}",
        .{ workspace_release_mutate_request_id, workspace_name, app_suite_name, "none", package_name, package_channel_name, package_release_three_name },
    ) catch return error.ToolServiceFailed;
    const workspace_release_mutate_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
        source_ip,
        destination_ip,
        workspace_release_mutate_request,
        512,
        256,
        512,
    );
    if (!std.mem.eql(u8, workspace_release_mutate_response, "RESP 145 18\nWORKSPACESAVE ops\n")) return error.ToolServiceResponseMismatch;

    var workspace_release_staging_save_request_buffer: [96]u8 = undefined;
    const workspace_release_staging_save_request = std.fmt.bufPrint(
        &workspace_release_staging_save_request_buffer,
        "REQ {d} WORKSPACERELEASESAVE {s} staging",
        .{ workspace_release_save_staging_request_id, workspace_name },
    ) catch return error.ToolServiceFailed;
    const workspace_release_staging_save_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
        source_ip,
        destination_ip,
        workspace_release_staging_save_request,
        512,
        256,
        512,
    );
    if (!std.mem.startsWith(u8, workspace_release_staging_save_response, "RESP 137 ")) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_release_staging_save_response, "WORKSPACERELEASESAVE ops staging\n") == null) return error.ToolServiceResponseMismatch;

    var workspace_release_staging_path_buffer: [filesystem.max_path_len]u8 = undefined;
    const workspace_release_staging_path = std.fmt.bufPrint(
        &workspace_release_staging_path_buffer,
        "/runtime/workspace-releases/{s}/staging/workspace.txt",
        .{workspace_name},
    ) catch return error.ToolServiceFailed;
    var workspace_release_staging_metadata_path_buffer: [filesystem.max_path_len]u8 = undefined;
    const workspace_release_staging_metadata_path = std.fmt.bufPrint(
        &workspace_release_staging_metadata_path_buffer,
        "/runtime/workspace-releases/{s}/staging/release.txt",
        .{workspace_name},
    ) catch return error.ToolServiceFailed;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const workspace_release_staging_readback = filesystem.readFileAlloc(service_fba.allocator(), workspace_release_staging_path, 256) catch return error.ToolServiceFailed;
    if (std.mem.indexOf(u8, workspace_release_staging_readback, "trust_bundle=none") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_release_staging_readback, "display_width=640") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_release_staging_readback, "display_height=400") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_release_staging_readback, "channel=demo:stable:r3") == null) return error.ToolServiceResponseMismatch;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const workspace_release_staging_metadata_readback = filesystem.readFileAlloc(service_fba.allocator(), workspace_release_staging_metadata_path, 128) catch return error.ToolServiceFailed;
    if (std.mem.indexOf(u8, workspace_release_staging_metadata_readback, "release=staging") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_release_staging_metadata_readback, "saved_seq=2") == null) return error.ToolServiceResponseMismatch;

    var workspace_release_list_request_buffer: [96]u8 = undefined;
    const workspace_release_list_request = std.fmt.bufPrint(
        &workspace_release_list_request_buffer,
        "REQ {d} WORKSPACERELEASELIST {s}",
        .{ workspace_release_list_request_id, workspace_name },
    ) catch return error.ToolServiceFailed;
    const workspace_release_list_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
        source_ip,
        destination_ip,
        workspace_release_list_request,
        512,
        256,
        512,
    );
    if (!std.mem.eql(u8, workspace_release_list_response, "RESP 138 15\ngolden\nstaging\n")) return error.ToolServiceResponseMismatch;

    var workspace_release_info_request_buffer: [112]u8 = undefined;
    const workspace_release_info_request = std.fmt.bufPrint(
        &workspace_release_info_request_buffer,
        "REQ {d} WORKSPACERELEASEINFO {s} staging",
        .{ workspace_release_info_request_id, workspace_name },
    ) catch return error.ToolServiceFailed;
    const workspace_release_info_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
        source_ip,
        destination_ip,
        workspace_release_info_request,
        512,
        256,
        512,
    );
    if (!std.mem.startsWith(u8, workspace_release_info_response, "RESP 139 ")) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_release_info_response, "workspace=ops") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_release_info_response, "release=staging") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_release_info_response, "saved_seq=2") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_release_info_response, "trust_bundle=none") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_release_info_response, "display=640x400") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_release_info_response, "channel=demo:stable:r3") == null) return error.ToolServiceResponseMismatch;

    var workspace_channel_set_staging_request_buffer: [128]u8 = undefined;
    const workspace_channel_set_staging_request = std.fmt.bufPrint(
        &workspace_channel_set_staging_request_buffer,
        "REQ {d} WORKSPACECHANNELSET {s} stable staging",
        .{ workspace_channel_set_staging_request_id, workspace_name },
    ) catch return error.ToolServiceFailed;
    const workspace_channel_set_staging_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
        source_ip,
        destination_ip,
        workspace_channel_set_staging_request,
        512,
        256,
        512,
    );
    if (!std.mem.startsWith(u8, workspace_channel_set_staging_response, "RESP 147 ")) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_channel_set_staging_response, "WORKSPACECHANNELSET ops stable staging\n") == null) return error.ToolServiceResponseMismatch;

    var workspace_channel_path_buffer: [filesystem.max_path_len]u8 = undefined;
    const workspace_channel_path = std.fmt.bufPrint(
        &workspace_channel_path_buffer,
        "/runtime/workspace-release-channels/{s}/stable.txt",
        .{workspace_name},
    ) catch return error.ToolServiceFailed;
    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const workspace_channel_staging_readback = filesystem.readFileAlloc(service_fba.allocator(), workspace_channel_path, 64) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, std.mem.trim(u8, workspace_channel_staging_readback, "\r\n\t "), "staging")) return error.ToolServiceResponseMismatch;

    var workspace_channel_list_request_buffer: [112]u8 = undefined;
    const workspace_channel_list_request = std.fmt.bufPrint(
        &workspace_channel_list_request_buffer,
        "REQ {d} WORKSPACECHANNELLIST {s}",
        .{ workspace_channel_list_request_id, workspace_name },
    ) catch return error.ToolServiceFailed;
    const workspace_channel_list_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
        source_ip,
        destination_ip,
        workspace_channel_list_request,
        512,
        256,
        512,
    );
    if (!std.mem.eql(u8, workspace_channel_list_response, "RESP 148 7\nstable\n")) return error.ToolServiceResponseMismatch;

    var workspace_channel_info_request_buffer: [128]u8 = undefined;
    const workspace_channel_info_request = std.fmt.bufPrint(
        &workspace_channel_info_request_buffer,
        "REQ {d} WORKSPACECHANNELINFO {s} stable",
        .{ workspace_channel_info_request_id, workspace_name },
    ) catch return error.ToolServiceFailed;
    const workspace_channel_info_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
        source_ip,
        destination_ip,
        workspace_channel_info_request,
        512,
        256,
        512,
    );
    if (!std.mem.startsWith(u8, workspace_channel_info_response, "RESP 149 ")) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_channel_info_response, "workspace=ops") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_channel_info_response, "channel=stable") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_channel_info_response, "release=staging") == null) return error.ToolServiceResponseMismatch;

    var workspace_channel_activate_staging_request_buffer: [128]u8 = undefined;
    const workspace_channel_activate_staging_request = std.fmt.bufPrint(
        &workspace_channel_activate_staging_request_buffer,
        "REQ {d} WORKSPACECHANNELACTIVATE {s} stable",
        .{ workspace_channel_activate_staging_request_id, workspace_name },
    ) catch return error.ToolServiceFailed;
    const workspace_channel_activate_staging_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
        source_ip,
        destination_ip,
        workspace_channel_activate_staging_request,
        512,
        256,
        512,
    );
    if (!std.mem.startsWith(u8, workspace_channel_activate_staging_response, "RESP 150 ")) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_channel_activate_staging_response, "WORKSPACECHANNELACTIVATE ops stable\n") == null) return error.ToolServiceResponseMismatch;

    var workspace_channel_staging_info_request_buffer: [64]u8 = undefined;
    const workspace_channel_staging_info_request = std.fmt.bufPrint(
        &workspace_channel_staging_info_request_buffer,
        "REQ {d} WORKSPACEINFO {s}",
        .{ workspace_channel_staging_info_request_id, workspace_name },
    ) catch return error.ToolServiceFailed;
    const workspace_channel_staging_info_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
        source_ip,
        destination_ip,
        workspace_channel_staging_info_request,
        512,
        256,
        512,
    );
    if (!std.mem.startsWith(u8, workspace_channel_staging_info_response, "RESP 153 ")) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_channel_staging_info_response, "trust_bundle=none") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_channel_staging_info_response, "display=640x400") == null) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_channel_staging_info_response, "channel=demo:stable:r3") == null) return error.ToolServiceResponseMismatch;

    var workspace_channel_set_golden_request_buffer: [128]u8 = undefined;
    const workspace_channel_set_golden_request = std.fmt.bufPrint(
        &workspace_channel_set_golden_request_buffer,
        "REQ {d} WORKSPACECHANNELSET {s} stable golden",
        .{ workspace_channel_set_golden_request_id, workspace_name },
    ) catch return error.ToolServiceFailed;
    const workspace_channel_set_golden_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
        source_ip,
        destination_ip,
        workspace_channel_set_golden_request,
        512,
        256,
        512,
    );
    if (!std.mem.startsWith(u8, workspace_channel_set_golden_response, "RESP 151 ")) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_channel_set_golden_response, "WORKSPACECHANNELSET ops stable golden\n") == null) return error.ToolServiceResponseMismatch;

    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const workspace_channel_golden_readback = filesystem.readFileAlloc(service_fba.allocator(), workspace_channel_path, 64) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, std.mem.trim(u8, workspace_channel_golden_readback, "\r\n\t "), "golden")) return error.ToolServiceResponseMismatch;

    var workspace_channel_activate_golden_request_buffer: [128]u8 = undefined;
    const workspace_channel_activate_golden_request = std.fmt.bufPrint(
        &workspace_channel_activate_golden_request_buffer,
        "REQ {d} WORKSPACECHANNELACTIVATE {s} stable",
        .{ workspace_channel_activate_golden_request_id, workspace_name },
    ) catch return error.ToolServiceFailed;
    const workspace_channel_activate_golden_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
        source_ip,
        destination_ip,
        workspace_channel_activate_golden_request,
        512,
        256,
        512,
    );
    if (!std.mem.startsWith(u8, workspace_channel_activate_golden_response, "RESP 152 ")) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_channel_activate_golden_response, "WORKSPACECHANNELACTIVATE ops stable\n") == null) return error.ToolServiceResponseMismatch;

    var workspace_channel_restored_info_response_buffer: [640]u8 = undefined;
    const workspace_channel_restored_info_response_expected = std.fmt.bufPrint(
        &workspace_channel_restored_info_response_buffer,
        "RESP {d} {d}\n{s}",
        .{ workspace_channel_restored_info_request_id, workspace_info_payload_expected.len, workspace_info_payload_expected },
    ) catch return error.ToolServiceFailed;
    var workspace_channel_restored_info_request_buffer: [64]u8 = undefined;
    const workspace_channel_restored_info_request = std.fmt.bufPrint(
        &workspace_channel_restored_info_request_buffer,
        "REQ {d} WORKSPACEINFO {s}",
        .{ workspace_channel_restored_info_request_id, workspace_name },
    ) catch return error.ToolServiceFailed;
    const workspace_channel_restored_info_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
        source_ip,
        destination_ip,
        workspace_channel_restored_info_request,
        512,
        256,
        512,
    );
    if (!std.mem.eql(u8, workspace_channel_restored_info_response, workspace_channel_restored_info_response_expected)) return error.ToolServiceResponseMismatch;

    var workspace_release_activate_request_buffer: [112]u8 = undefined;
    const workspace_release_activate_request = std.fmt.bufPrint(
        &workspace_release_activate_request_buffer,
        "REQ {d} WORKSPACERELEASEACTIVATE {s} golden",
        .{ workspace_release_activate_request_id, workspace_name },
    ) catch return error.ToolServiceFailed;
    const workspace_release_activate_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
        source_ip,
        destination_ip,
        workspace_release_activate_request,
        512,
        256,
        512,
    );
    if (!std.mem.startsWith(u8, workspace_release_activate_response, "RESP 140 ")) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_release_activate_response, "WORKSPACERELEASEACTIVATE ops golden\n") == null) return error.ToolServiceResponseMismatch;

    var workspace_restored_info_response_buffer: [640]u8 = undefined;
    const workspace_restored_info_response_expected = std.fmt.bufPrint(
        &workspace_restored_info_response_buffer,
        "RESP {d} {d}\n{s}",
        .{ workspace_release_restored_info_request_id, workspace_info_payload_expected.len, workspace_info_payload_expected },
    ) catch return error.ToolServiceFailed;
    var workspace_restored_info_request_buffer: [64]u8 = undefined;
    const workspace_restored_info_request = std.fmt.bufPrint(
        &workspace_restored_info_request_buffer,
        "REQ {d} WORKSPACEINFO {s}",
        .{ workspace_release_restored_info_request_id, workspace_name },
    ) catch return error.ToolServiceFailed;
    const workspace_restored_info_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
        source_ip,
        destination_ip,
        workspace_restored_info_request,
        512,
        256,
        512,
    );
    if (!std.mem.eql(u8, workspace_restored_info_response, workspace_restored_info_response_expected)) return error.ToolServiceResponseMismatch;

    var workspace_release_delete_request_buffer: [112]u8 = undefined;
    const workspace_release_delete_request = std.fmt.bufPrint(
        &workspace_release_delete_request_buffer,
        "REQ {d} WORKSPACERELEASEDELETE {s} staging",
        .{ workspace_release_delete_request_id, workspace_name },
    ) catch return error.ToolServiceFailed;
    const workspace_release_delete_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
        source_ip,
        destination_ip,
        workspace_release_delete_request,
        512,
        256,
        512,
    );
    if (!std.mem.startsWith(u8, workspace_release_delete_response, "RESP 141 ")) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_release_delete_response, "WORKSPACERELEASEDELETE ops staging\n") == null) return error.ToolServiceResponseMismatch;

    var workspace_release_save_fallback_request_buffer: [112]u8 = undefined;
    const workspace_release_save_fallback_request = std.fmt.bufPrint(
        &workspace_release_save_fallback_request_buffer,
        "REQ {d} WORKSPACERELEASESAVE {s} fallback",
        .{ workspace_release_save_fallback_request_id, workspace_name },
    ) catch return error.ToolServiceFailed;
    const workspace_release_save_fallback_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
        source_ip,
        destination_ip,
        workspace_release_save_fallback_request,
        512,
        256,
        512,
    );
    if (!std.mem.startsWith(u8, workspace_release_save_fallback_response, "RESP 142 ")) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_release_save_fallback_response, "WORKSPACERELEASESAVE ops fallback\n") == null) return error.ToolServiceResponseMismatch;

    var workspace_release_prune_request_buffer: [112]u8 = undefined;
    const workspace_release_prune_request = std.fmt.bufPrint(
        &workspace_release_prune_request_buffer,
        "REQ {d} WORKSPACERELEASEPRUNE {s} 1",
        .{ workspace_release_prune_request_id, workspace_name },
    ) catch return error.ToolServiceFailed;
    const workspace_release_prune_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
        source_ip,
        destination_ip,
        workspace_release_prune_request,
        512,
        256,
        512,
    );
    if (!std.mem.startsWith(u8, workspace_release_prune_response, "RESP 143 ")) return error.ToolServiceResponseMismatch;
    if (std.mem.indexOf(u8, workspace_release_prune_response, "WORKSPACERELEASEPRUNE ops keep=1 deleted=1 kept=1\n") == null) return error.ToolServiceResponseMismatch;

    var workspace_release_list_final_request_buffer: [96]u8 = undefined;
    const workspace_release_list_final_request = std.fmt.bufPrint(
        &workspace_release_list_final_request_buffer,
        "REQ {d} WORKSPACERELEASELIST {s}",
        .{ workspace_release_list_final_request_id, workspace_name },
    ) catch return error.ToolServiceFailed;
    const workspace_release_list_final_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
        source_ip,
        destination_ip,
        workspace_release_list_final_request,
        512,
        256,
        512,
    );
    ...
    package_store.installScriptPackage(package_name, package_mutated_script_body, 0) catch return error.ToolServiceFailed;
    framebuffer_console.setMode(640, 400) catch return error.ToolServiceFailed;
    filesystem.writeFile("/runtime/apps/demo/active_plan.txt", "mutated", 0) catch return error.ToolServiceFailed;
    filesystem.writeFile("/runtime/apps/aux/active_plan.txt", "mutated-aux", 0) catch return error.ToolServiceFailed;

    var workspace_apply_request_buffer: [64]u8 = undefined;
    const workspace_apply_request = std.fmt.bufPrint(
        &workspace_apply_request_buffer,
        "REQ {d} WORKSPACEAPPLY {s}",
        .{ workspace_apply_request_id, workspace_name },
    ) catch return error.ToolServiceFailed;
    const workspace_apply_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
    ...
    if (!std.mem.eql(u8, workspace_active_bundle, trust_backup_bundle_name)) return error.ToolServiceResponseMismatch;

    const workspace_display_state = framebuffer_console.statePtr();
    if (workspace_display_state.width != 1024 or workspace_display_state.height != 768) return error.ToolServiceResponseMismatch;

    var workspace_entrypoint_buffer: [filesystem.max_path_len]u8 = undefined;
    const workspace_profile = package_store.loadLaunchProfile(package_name, &workspace_entrypoint_buffer) catch return error.ToolServiceFailed;
    service_fba = std.heap.FixedBufferAllocator.init(&scratch.service_scratch);
    const workspace_script_readback = filesystem.readFileAlloc(service_fba.allocator(), workspace_profile.entrypoint, 160) catch return error.ToolServiceFailed;
    if (!std.mem.eql(u8, workspace_script_readback, package_script_body)) return error.ToolServiceResponseMismatch;
    ...
    _ = workspace_autorun_batch_service_response_expected;
    var workspace_autorun_batch_cursor: usize = 0;
    const workspace_autorun_expected_fragments = [_][]const u8{
        workspace_autorun_add_primary_payload_expected,
        workspace_autorun_add_secondary_payload_expected,
        workspace_autorun_list_payload_expected,
        workspace_autorun_run_payload_expected,
    ...
    for (workspace_autorun_expected_fragments) |fragment| {
        const fragment_index = std.mem.indexOfPos(u8, workspace_autorun_batch_service_response, workspace_autorun_batch_cursor, fragment) orelse {
            return error.ToolServiceResponseMismatch;
        };
        workspace_autorun_batch_cursor = fragment_index + fragment.len;
    }
    ...
    const workspace_delete_request = std.fmt.bufPrint(
        ...
        "REQ {d} WORKSPACEDELETE {s}",
        .{ workspace_delete_request_id, workspace_name },
    ) catch return error.ToolServiceFailed;
    const workspace_delete_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
    ...
    const workspace_list_after_delete_request = std.fmt.bufPrint(
        &workspace_list_after_delete_request_buffer,
        "REQ {d} WORKSPACELIST",
        .{workspace_list_after_delete_request_id},
    ) catch return error.ToolServiceFailed;
    const workspace_list_after_delete_response = try exchangeTcpProbeServiceRequest(
        eth,
        scratch,
        client_b,
        &server_b,
        source_ip,
        destination_ip,
        workspace_list_after_delete_request,
        512,
        256,
        512,
    );
    ...
    if (filesystem.statSummary("/runtime/workspace-releases/ops/fallback/workspace.txt")) |_| {
        return error.ToolServiceResponseMismatch;
    } else |err| switch (err) {
        error.FileNotFound => {},
        else => return error.ToolServiceFailed,
    ...
    const app_plan_delete_batch_request = std.fmt.bufPrint(
        &scratch.service_batch_request_buffer,
        "REQ {d} APPPLANDELETE {s} {s}\nREQ {d} APPPLANLIST {s}",
        .{
            app_plan_delete_request_id,
            package_name,
            app_plan_name,
            app_plan_list_after_delete_request_id,
            package_name,
        },
    ) catch return error.ToolServiceFailed;
    ...
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_delete_batch_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const app_plan_delete_batch_response = tool_service.handleFramedRequestBatch(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    var app_plan_delete_batch_response_offset: usize = 0;
    while (app_plan_delete_batch_response_offset < app_plan_delete_batch_response.len) {
        const app_plan_delete_batch_reply_chunk = server_b.buildPayloadChunk(app_plan_delete_batch_response[app_plan_delete_batch_response_offset..]) catch |err| return mapTcpSessionProbeError(err);
        const expected_chunk = app_plan_delete_batch_response[app_plan_delete_batch_response_offset .. app_plan_delete_batch_response_offset + app_plan_delete_batch_reply_chunk.payload.len];
        _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, app_plan_delete_batch_reply_chunk);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, app_plan_delete_batch_reply_chunk);
        if (!std.mem.eql(u8, app_plan_delete_batch_reply_chunk.payload, expected_chunk)) return error.PayloadMismatch;
        const mapped_b_app_plan_delete_batch_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
        if (mapped_b_app_plan_delete_batch_reply != client_b) return error.SessionStateMismatch;
        mapped_b_app_plan_delete_batch_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

        app_plan_delete_batch_response_offset += app_plan_delete_batch_reply_chunk.payload.len;

        const app_plan_delete_batch_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
        _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_delete_batch_reply_ack);
        try pollTcpProbePacket(eth, &scratch.packet_storage);
        try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, app_plan_delete_batch_reply_ack);
        server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);
    }
    ...
    const uninstall_service_request_payload = client_b.buildPayload(uninstall_service_request) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, uninstall_service_request_payload);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, uninstall_service_request_payload);
    server_b.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const uninstall_service_response = tool_service.handleFramedRequestBatch(
        service_fba.allocator(),
        scratch.packet_storage.payload[0..scratch.packet_storage.payload_len],
        512,
        256,
    ...
    if (!std.mem.eql(u8, uninstall_service_response, uninstall_service_response_expected)) return error.ToolServiceResponseMismatch;

    const uninstall_service_reply = server_b.buildPayload(uninstall_service_response) catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, uninstall_service_reply);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, destination_ip, source_ip, flow_b.remote_port, flow_b.local_port, uninstall_service_reply);
    const mapped_b_uninstall_reply = table.findByInboundPacket(destination_ip, source_ip, tcpPacketView(&scratch.packet_storage)) orelse return error.SessionStateMismatch;
    if (mapped_b_uninstall_reply != client_b) return error.SessionStateMismatch;
    mapped_b_uninstall_reply.acceptPayload(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    const uninstall_service_reply_ack = client_b.buildAck() catch |err| return mapTcpSessionProbeError(err);
    _ = try sendTcpProbeSegment(source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, uninstall_service_reply_ack);
    try pollTcpProbePacket(eth, &scratch.packet_storage);
    try expectTcpProbePacket(&scratch.packet_storage, eth.mac, source_ip, destination_ip, flow_b.local_port, flow_b.remote_port, uninstall_service_reply_ack);
    server_b.acceptAck(tcpPacketView(&scratch.packet_storage)) catch |err| return mapTcpSessionProbeError(err);

    if (filesystem.statSummary("/packages/demo")) |_| {
        return error.ToolServiceResponseMismatch;
    } else |err| switch (err) {
        error.FileNotFound => {},
        else => return error.ToolServiceFailed,
    }
    if (filesystem.statSummary("/runtime/apps/demo")) |_| {
        return error.ToolServiceResponseMismatch;
    } else |err| switch (err) {
        error.FileNotFound => {},
        else => return error.ToolServiceFailed,
    ...
    if (client_a.pollRetransmit(probe_tick + retransmit_interval_ticks - 1) != null) return error.RetransmitTooEarly;
    probe_tick +%= retransmit_interval_ticks;
    const retry_client_fin_a = client_a.pollRetransmit(probe_tick) orelse return error.RetransmitMissing;
    if (retry_client_fin_a.sequence_number != client_fin_a.sequence_number or
        retry_client_fin_a.acknowledgment_number != client_fin_a.acknowledgment_number or
        retry_client_fin_a.flags != client_fin_a.flags or
        retry_client_fin_a.window_size != client_fin_a.window_size or
        retry_client_fin_a.payload.len != 0)
    {
        return error.RetransmitShapeMismatch;
    }
    ...
    if (!server_a.retransmit.armed() or server_a.retransmit.kind != .fin) return error.RetransmitMissing;
    if (server_a.pollRetransmit(probe_tick + retransmit_interval_ticks - 1) != null) return error.RetransmitTooEarly;
    probe_tick +%= retransmit_interval_ticks;
    const retry_server_fin_a = server_a.pollRetransmit(probe_tick) orelse return error.RetransmitMissing;
    if (retry_server_fin_a.sequence_number != server_fin_a.sequence_number or
        retry_server_fin_a.acknowledgment_number != server_fin_a.acknowledgment_number or
        retry_server_fin_a.flags != server_fin_a.flags or
        retry_server_fin_a.window_size != server_fin_a.window_size or
        retry_server_fin_a.payload.len != 0)
    {
        return error.RetransmitShapeMismatch;
    }
...
fn runToolExecProbe() ToolExecProbeError!void {
    ...
    var help = pal_proc.runCaptureFreestanding(allocator, io, &.{"help"}, 1000, 2048, 128) catch |err| switch (err) {
