...
pub fn discoverVirtioNetDevice() ?VirtioNetDevice {
    if (!hardwareBacked() and !(builtin.is_test and mock_enabled)) return null;
    ...
    while (bus < max_bus_count) : (bus += 1) {
        ...
        while (device < max_device_count) : (device += 1) {
            ...
            while (function < function_limit) : (function += 1) {
                const function_id: u8 = @intCast(function);
                const vendor = vendorId(bus_id, device_id0, function_id);
                if (vendor != virtio_vendor_id or deviceId(bus_id, device_id0, function_id) != virtio_net_device_id) continue;
...
pub fn enableVirtioNetMemoryAndBusMaster(location: DeviceLocation) void {
    const command = readConfig16(location.bus, location.device, location.function, 0x04);
    const wanted = command | 0x7;
    if (wanted != command) {
        writeConfig16(location.bus, location.device, location.function, 0x04, wanted);
...
test "pci virtio net scan finds modern capability regions" {
    testResetForTest();
    defer testResetForTest();

    testSetConfig32(0, 6, 0, 0x00, 0x1041_1AF4);
    testSetConfig32(0, 6, 0, 0x04, 0x0000_0010);
    testSetConfig32(0, 6, 0, 0x0C, 0x0000_0000);
    testSetConfig32(0, 6, 0, 0x20, 0xFE20_0000);
    testSetConfig32(0, 6, 0, 0x34, 0x0000_0040);
    testSetConfig32(0, 6, 0, 0x3C, 0x0000_000C);

    testSetConfig32(0, 6, 0, 0x40, 0x0110_5009);
    testSetConfig32(0, 6, 0, 0x44, 0x0000_0004);
    testSetConfig32(0, 6, 0, 0x48, 0x0000_1000);
    testSetConfig32(0, 6, 0, 0x4C, 0x0000_0100);

    testSetConfig32(0, 6, 0, 0x50, 0x0210_7009);
    testSetConfig32(0, 6, 0, 0x54, 0x0000_0004);
    testSetConfig32(0, 6, 0, 0x58, 0x0000_2000);
    testSetConfig32(0, 6, 0, 0x5C, 0x0000_0100);
    testSetConfig32(0, 6, 0, 0x60, 0x0000_0004);

    testSetConfig32(0, 6, 0, 0x70, 0x0310_8009);
    testSetConfig32(0, 6, 0, 0x74, 0x0000_0004);
    testSetConfig32(0, 6, 0, 0x78, 0x0000_3000);
    testSetConfig32(0, 6, 0, 0x7C, 0x0000_0020);

    testSetConfig32(0, 6, 0, 0x80, 0x0410_0009);
    testSetConfig32(0, 6, 0, 0x84, 0x0000_0004);
    testSetConfig32(0, 6, 0, 0x88, 0x0000_4000);
    testSetConfig32(0, 6, 0, 0x8C, 0x0000_0100);

    const nic = discoverVirtioNetDevice() orelse return error.TestUnexpectedResult;
    try std.testing.expectEqual(@as(u16, 0x1AF4), nic.vendor_id);
    try std.testing.expectEqual(@as(u16, 0x1041), nic.device_id);
    try std.testing.expectEqual(@as(u8, 0x0C), nic.irq_line);
    try std.testing.expectEqual(@as(u64, 0xFE20_1000), nic.common_cfg.address);
    try std.testing.expectEqual(@as(u64, 0xFE20_2000), nic.notify_cfg.address);
    try std.testing.expectEqual(@as(u32, 4), nic.notify_off_multiplier);
    try std.testing.expectEqual(@as(u64, 0xFE20_3000), nic.isr_cfg.address);
    try std.testing.expectEqual(@as(u64, 0xFE20_4000), nic.device_cfg.address);

    enableVirtioNetMemoryAndBusMaster(nic.location);
    try std.testing.expectEqual(@as(u16, 0x7), readConfig16(0, 6, 0, 0x04) & 0x7);
}
