...
pub fn probeImage(image: []const u8, base_phys: u64) Error!void {
    ...
    if (madt_addr != 0) {
        if (validateTable(image, base_phys, madt_addr, madt_signature)) |madt| {
            countMadtEntries(madt, &lapic_count, &ioapic_count);
...
fn countMadtEntries(madt: []const u8, lapic_count: *u16, ioapic_count: *u16) void {
    lapic_count.* = 0;
    ioapic_count.* = 0;
    ...
    while (offset + 2 <= madt.len) {
        ...
        switch (entry_type) {
            madt_entry_local_apic => lapic_count.* +%= 1,
...
fn buildSyntheticAcpiImage(image: []u8, use_xsdt: bool) void {
    ...
    var madt = image[madt_addr .. madt_addr + 64];
...
test "acpi probe image discovers rsdt fadt and madt" {
    ...
    try std.testing.expectEqual(@as(u16, 1), snapshot.lapic_count);
