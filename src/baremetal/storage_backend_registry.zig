...
pub fn renderFilesystemSupportAlloc(allocator: std.mem.Allocator, max_bytes: usize) ![]u8 {
    ...
    const rows = [_][]const u8{
        ...
        "filesystem=fat32 detect=1 mount=1 write=1 source=zar_bounded_writable_one_level_83\n",
    };
    for (rows) |row| {
...
test "backend registry exposes filesystem support matrix" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, rendered, "filesystem=fat32 detect=1 mount=1 write=1 source=zar_bounded_writable_one_level_83") != null);
}
