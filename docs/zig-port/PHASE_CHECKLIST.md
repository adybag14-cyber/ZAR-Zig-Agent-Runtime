...
- `FS5.5` framebuffer/console strict closure is now reached locally: `src/baremetal/framebuffer_console.zig` programs a real Bochs/QEMU BGA linear-framebuffer path with bounded mode support for `640x400`, `800x600`, `1024x768`, `1280x720`, and `1280x1024`, `src/baremetal/pci.zig` discovers the selected PCI display adapter as structured metadata, exposes the framebuffer BAR, and enables decode on that function, `src/pal/framebuffer.zig` exposes the surface plus supported-mode enumeration through the PAL, host regressions in `src/baremetal/framebuffer_console.zig` and `src/baremetal_main.zig` prove framebuffer state, adapter metadata, supported-mode enumeration, glyph pixel updates, and bounded mode switching, and the live QEMU+GDB proof `scripts/baremetal-qemu-framebuffer-console-probe-check.ps1` now reads back real MMIO banner pixels plus adapter metadata from the hardware-backed framebuffer BAR over the freestanding PVH artifact at `640x400`, `1024x768`, and `1280x720`; HDMI/DisplayPort/EDID output paths remain future depth and are not claimed here.
