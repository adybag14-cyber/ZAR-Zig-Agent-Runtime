...
- `FS5.5` storage/disk strict closure is now reached locally: `src/baremetal/storage_backend.zig` provides the shared backend selector, `src/baremetal/ata_pio_disk.zig` provides a real ATA PIO `IDENTIFY` / `READ` / `WRITE` / `FLUSH` path plus first-usable-MBR-partition mounting with logical LBA translation, `src/pal/storage.zig` plus `src/baremetal/tool_layout.zig` route through that shared backend, hosted/host regressions prove backend preference, identify-backed capacity, ATA mock-device read/write/flush behavior, first-partition MBR mounting, logical base-LBA translation, and ATA-backed export reporting, and the live QEMU proof `scripts/baremetal-qemu-ata-storage-probe-check.ps1` now boots a real MBR-partitioned raw image and proves raw ATA block mutation + readback, ATA-backed tool-layout persistence, and ATA-backed filesystem persistence through the mounted partition view over the freestanding PVH artifact.
