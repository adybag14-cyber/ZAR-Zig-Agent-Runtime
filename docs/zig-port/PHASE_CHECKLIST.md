...
- [x] Added bare-metal optional QEMU manual-wait interrupt wrapper probes (`scripts/baremetal-qemu-manual-wait-interrupt-baseline-probe-check.ps1`, `scripts/baremetal-qemu-manual-wait-interrupt-after-interrupt-blocked-probe-check.ps1`, `scripts/baremetal-qemu-manual-wait-interrupt-interrupt-telemetry-probe-check.ps1`, `scripts/baremetal-qemu-manual-wait-interrupt-manual-wake-payload-probe-check.ps1`, and `scripts/baremetal-qemu-manual-wait-interrupt-manual-wake-ready-state-probe-check.ps1`) and wired them into `zig-ci` + `release-preview` validate jobs, plus matching host-regression strengthening in `src/baremetal_main.zig` (they reuse the broad manual-wait interrupt QEMU probe but fail directly on the one-task waiting baseline, blocked post-interrupt state with empty wake queue, preserved interrupt telemetry for vector `44`, exact manual wake payload, and final ready-state/mailbox invariants after explicit `command_scheduler_wake_task` recovery).
...
- [x] Added bare-metal optional QEMU feature-flags/tick-batch wrapper probes (`scripts/baremetal-qemu-feature-flags-set-success-probe-check.ps1`, `scripts/baremetal-qemu-tick-batch-valid-update-probe-check.ps1`, `scripts/baremetal-qemu-tick-batch-invalid-preserve-probe-check.ps1`, `scripts/baremetal-qemu-feature-flags-tick-batch-mailbox-state-probe-check.ps1`, and `scripts/baremetal-qemu-feature-flags-tick-batch-state-preserve-probe-check.ps1`) and wired them into `zig-ci` + `release-preview` validate jobs (they reuse the broad feature-flags/tick-batch QEMU probe but fail directly on the narrow boundaries for stage-1 flag success, stage-2 valid batch update, stage-3 invalid-zero preservation, final mailbox opcode/sequence stability, and final preserved flag/batch/tick accumulation over the PVH freestanding artifact).
- [x] Added bare-metal optional QEMU mailbox wrapper probes (`scripts/baremetal-qemu-mailbox-invalid-magic-preserve-state-probe-check.ps1`, `scripts/baremetal-qemu-mailbox-invalid-api-version-preserve-state-probe-check.ps1`, `scripts/baremetal-qemu-mailbox-valid-recovery-probe-check.ps1`, `scripts/baremetal-qemu-mailbox-stale-seq-preserve-state-probe-check.ps1`, and `scripts/baremetal-qemu-mailbox-seq-wraparound-recovery-probe-check.ps1`) and wired them into `zig-ci` + `release-preview` validate jobs, plus matching host-regression tightening in `src/baremetal_main.zig` (they reuse the broad mailbox header/stale-seq/wraparound QEMU probes but fail directly on the narrow boundaries for invalid `magic` preservation, invalid `api_version` preservation, clean valid recovery after header rejection, stale-seq replay no-op preservation, and `u32` sequence wraparound recovery over the PVH freestanding artifact).
