...
- [ ] FS6 - Appliance/bare-metal maturity track
  ...
  - latest required bare-metal evidence now also includes `scripts/baremetal-qemu-task-terminate-mixed-state-probe-check.ps1` (live `command_task_terminate` cleanup of queued wakes plus armed timer state for the terminated task only, with survivor wake preservation and no ghost timer dispatch against the freestanding PVH artifact).
...
- [x] Added bare-metal optional QEMU task-terminate mixed-state probe (`scripts/baremetal-qemu-task-terminate-mixed-state-probe-check.ps1`) and wired it into `zig-ci` + `release-preview` validate jobs, plus matching host regression coverage in `src/baremetal_main.zig` (task termination now scrubs stale wake-queue entries for the terminated task, cancels its armed timer entry, preserves the survivor wake entry, and prevents ghost timer dispatch after idle ticks over the PVH freestanding artifact).
