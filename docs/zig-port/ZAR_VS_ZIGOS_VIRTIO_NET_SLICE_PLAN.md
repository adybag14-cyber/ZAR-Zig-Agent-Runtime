...
- live QEMU raw-frame proof through `scripts/baremetal-qemu-virtio-net-probe-check.ps1` plus `scripts/qemu-virtio-net-dgram-echo.ps1`
...
### V5. Protocol Reuse Later

Future depth after raw-frame closure:
...
5. `zig build test --summary all` is green. Status: `Pending full matrix verification`
6. parity gate is green. Status: `Pending full matrix verification`
7. docs status gate is green. Status: `Pending full matrix verification`
8. `zig-ci` is green. Status: `Pending push verification`
9. `docs-pages` is green. Status: `Pending push verification`
...
- a clean platform for later `virtio-net` protocol reuse
