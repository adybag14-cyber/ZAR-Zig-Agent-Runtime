...
- the remaining i386 AP/SMP gap is now narrower and explicit:
  ...
  - bounded scheduler priority-debt compensation from `4/2/2/2` into `4/3/4/3` with non-zero remaining debt is now observed on the BIOS firmware-boot path
  - that moves the next real closure step to broader SMP bring-up beyond bounded concurrent/owned/redistributed/priority-aware/priority-rotation/priority-fanout/full-table-saturation/saturated-reprioritization/failover/backfill/windowed/fairness-drain/skew-rebalance/priority-debt AP slot dispatch
...
1. widen the current firmware-backed AP execution lane into broader SMP bring-up:
   ...
   - scheduler behavior beyond the current four-slot backlog-drain fairness and skew-rebalance map
