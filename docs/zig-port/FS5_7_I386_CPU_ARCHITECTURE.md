...
- the i386 freestanding runtime now also has a real BIOS firmware-boot priority-quota scheduler lane that proves the same seeded `2/1/1/1` four-slot history can drain a carried debt set of `3` plus a waiting backlog of `8` under explicit slot quotas `3/2/2/1`, exports cumulative quota plus debt telemetry through `/dev/cpu/ap-quota` and `/sys/cpu/ap-quota`, drains the combined backlog with budget `3` and aging step `2`, and finishes with zero remaining debt, `8` waiting tasks, `3` debt tasks, `8` quota tasks, bounded four-slot export/readback, and live proof at both default memory and `1024 MiB`
...
- the remaining i386 AP/SMP gap is now narrower and explicit:
  ...
  - bounded scheduler priority-quota on top of carried debt/admission/aging from a seeded `2/1/1/1` history under explicit slot quotas `3/2/2/1` is now observed on the BIOS firmware-boot path
  - that moves the next real closure step to broader SMP bring-up beyond bounded concurrent/owned/redistributed/priority-aware/priority-rotation/priority-fanout/full-table-saturation/saturated-reprioritization/failover/backfill/windowed/fairness-drain/skew-rebalance/priority-debt/priority-admission/priority-aging/priority-fairshare/priority-quota AP slot dispatch
...
1. widen the current firmware-backed AP execution lane into broader SMP bring-up:
   ...
   - scheduler behavior beyond the current four-slot backlog-drain fairness, skew-rebalance, debt/admission, priority-aging, priority-fairshare, and priority-quota map
