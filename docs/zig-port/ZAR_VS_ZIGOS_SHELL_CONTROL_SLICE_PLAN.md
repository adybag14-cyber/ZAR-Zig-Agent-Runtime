...
- `src/baremetal/tool_exec.zig`
  ...
  - bounded shell escaping for `\;`, `\<`, `\>`, `\\`, escaped whitespace on redirection paths, escaped-whitespace direct path arguments for path-consuming builtins including direct `run-script` script paths plus `mount-bind` target paths, and quoted separators on the parser path
