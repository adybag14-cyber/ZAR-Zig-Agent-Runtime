...
- `FS5.5` hardware-driver pivot update:
  ...
  - Ethernet driver depth advanced locally:
    ...
    - deeper networking depth remains future work above the FS5.5 closure bar:
      - higher-level service/runtime layers beyond the current bounded typed batch file/package/trust metadata seam on the bare-metal TCP path
...
- FS5.5 service/display/runtime depth slice (current head):
  ...
  - `src/baremetal/tool_exec.zig` now exposes `trust-list`, `trust-info`, and `trust-select` builtins on top of that store.
  - `src/baremetal/package_store.zig` now extends the canonical persisted package layout with `/packages/<name>/assets/...`, manifest `asset_root` / `asset_count` / `asset_bytes` metadata, `/packages/<name>/meta/app.txt` launch/display metadata, persisted app-manifest readback, and persisted package-display profile updates.
  - `src/baremetal/tool_exec.zig` now also exposes `package-app`, `package-display`, `package-ls`, `package-cat`, `display-info`, `display-modes`, and `display-set` on top of the same freestanding filesystem/display surfaces.
  - `src/baremetal/tool_service.zig` now extends the typed TCP service seam with `EXEC`, `INSTALL`, `MANIFEST`, `PKGAPP`, `PKGDISPLAY`, `PKGPUT`, `PKGLS`, `PKGGET`, `DISPLAYINFO`, `DISPLAYMODES`, `DISPLAYSET`, `TRUSTPUT`, `TRUSTLIST`, `TRUSTINFO`, and `TRUSTSELECT`, returning structured `exit=... stdout_len=... stderr_len=...` payloads, runtime-layout manifest/install receipts, package-app/package-asset/display payloads, or persisted trust-store receipts on top of the freestanding tool/filesystem path.
  ...
  - `src/baremetal_main.zig` now drives the live RTL8139 TCP proof through typed trust-store install/list/info/select exchange, typed package-app/package-display/package-asset exchange with direct filesystem readback, typed `DISPLAYINFO` / `DISPLAYMODES` / `DISPLAYSET` exchange, selected trust-bundle path readback, live display-mode application during `run-package`, and persisted HTTPS trust-store loading.
  ...
  - full validation after the package-app/display-profile slice is green:
    - `zig build test --summary all` -> hosted `302/302`, bare-metal host `291/291`
    - the only regression found during the slice was a real `help` capture overflow in the freestanding tool-exec proof after the builtin surface expanded; `runToolExecProbe()` now uses a `512`-byte stdout budget for `help`, matching the wider command surface instead of silently truncating.
