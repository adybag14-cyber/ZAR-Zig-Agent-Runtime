...
- `FS5.7` i386 CPU-architecture support is now tracked in:
  ...
  - nineteenth delivered slice adds `src/baremetal/boot_memory.zig`, exports `oc_boot_memory_state_ptr()` plus `/dev/memory/state` and `/sys/memory/state`, records raw Multiboot2 boot info on the direct-loader path in `scripts/baremetal/i386_boot.S`, synthesizes bounded Multiboot2 basic-meminfo in `scripts/baremetal/i386_bios_stage2.asm` for the BIOS firmware path, removes the old fixed `1 MiB` i386 heap in favor of dynamic heap sizing up to `1 GiB`, widens the i386 probe scripts with `-MemoryMiB`, validates live `1024M` direct-loader platform, firmware platform, SMP/AP-startup control, ATA storage, and E1000 full-stack lanes, and wires the key `1 GiB` platform/storage/NIC probes into `zig-ci` and `release-preview`
  - next ZigOS-derived adoption priorities are now tracked in `docs/zig-port/ZAR_VS_ZIGOS_NEXT_ADOPTION_OPPORTUNITIES.md`
  - current boundary is explicit: this is boot/build smoke plus broad real i386 storage/NIC/display/service lanes, a bounded direct-loader platform ACPI/timer-controller/interrupt/CPU-topology/IOAPIC/LAPIC/AP-startup-control seam, and a separate real firmware-boot ACPI proof lane; remaining gaps are actual AP bring-up/SMP execution and the next architecture-hardening seams after current device proof breadth, with the BSP-side warm-reset and startup-IPI path now explicitly proven clean on the direct-loader lane
