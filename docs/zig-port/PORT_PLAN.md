...
- FS5.5 service/display/runtime depth slice (current head):
  ...
  - full validation after the app-output/service slice is green:
    - `zig build test --summary all` -> hosted `355/355`, bare-metal host `302 passed / 1 skipped`
    ...
    - the real regressions found during the slice were stale app-info test sizing, outdated stdout expectations after persisted launch receipts widened, and insufficient fixed scratch sizing in the live RTL8139 probe; the bounded info buffers, expected service payloads, and fixed probe scratch now match the persisted app output surface
