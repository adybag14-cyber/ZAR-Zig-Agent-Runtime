...
- twelfth delivered adoption slice is a bounded shell/control layer through `src/baremetal/tool_exec.zig`, `src/baremetal/tool_service.zig`, `src/baremetal/tool_service/codec.zig`, and `scripts/baremetal-qemu-e1000-tool-service-probe-check.ps1`, with host regressions for bounded shell batching, multi-segment glob expansion, and bounded stdout redirection, typed `SHELLRUN` / `SHELLEXPAND` over the framed TCP seam, live `E1000` proof for shell help plus batch/glob/redirect/readback behavior, and `build.zig` now explicitly wiring `scripts/baremetal/pvh_boot.S` plus `scripts/baremetal/pvh_lld.ld` so the Multiboot2 header stays within the required first `32768` bytes on current Zig `master`
