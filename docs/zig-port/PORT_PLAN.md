...
- Phase 6 complete:
  ...
  - Method surface moved to `168` Zig methods (from `126`) while preserving Lightpanda-only browser policy and green validation gates.
  - Added Zig-OS appliance control-plane methods with stateful behavior:
    - `system.boot.status`, `system.boot.verify`, `system.boot.attest`, `system.boot.policy.get`, `system.boot.policy.set`
    - `system.rollback.plan`, `system.rollback.run`, `system.rollback.cancel`
    - secure-boot measurement/signer verification telemetry, signed attestation (`statementDigest` + optional HMAC signature via `OPENCLAW_ZIG_BOOT_ATTEST_KEY`), policy management (`enforceUpdateGate`, `verificationMaxAgeMs`, `requiredSigner`), and slot-aware rollback plan/apply/cancel flow (`A/B`) now persist through compat state with update-job/event traces.
  ...
  - Method/event parity is now tracked and enforced against Go + original stable + original beta baselines:
    ...
    - Intentional Zig-only extras retained for edge/runtime depth: `33`.
