...
- FS5.5 service/display/runtime depth slice (current head):
  ...
  - full validation after the autorun/runtime slice is green:
    - `zig build test --summary all` -> `361/361` passed
    ...
    - the real regressions found during the slice were stale framed-response sizing in the autorun service tests, stale long-help and package/app list expectations in the live RTL8139 proof after adding the `aux` package path, and insufficient filesystem entry capacity for the deeper persisted runtime state; the framed expectations, probe buffers, and `64`-entry filesystem budget now match the current FS5.5 runtime surface
