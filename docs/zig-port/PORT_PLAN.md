...
- FS5.5 service/display/runtime depth slice (current head):
  ...
  - `src/baremetal/filesystem.zig` now carries an `80`-entry filesystem budget so the deeper FS5.5 package/trust/app/autorun/workspace runtime state fits on the persisted surface without live-service `NoSpace` failures.
