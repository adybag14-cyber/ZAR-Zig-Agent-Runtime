...
- `FS5.5` hardware-driver pivot update:
  ...
  - Ethernet driver depth advanced locally:
    ...
    - deeper networking depth remains future work above the FS5.5 closure bar:
      - higher-level service/runtime layers beyond the current bounded typed batch file/package/trust/display/app metadata seam on the bare-metal TCP path
...
- FS5.5 service/display/runtime depth slice (current head):
  ...
  - `src/baremetal/tool_exec.zig` now exposes `trust-list`, `trust-info`, `trust-select`, `app-list`, `app-info`, `app-state`, `app-trust`, `app-connector`, and `app-run` builtins on top of that store.
  ...
  - `src/baremetal/app_runtime.zig` now provides the persisted app-runtime surface under `/runtime/apps/<name>/last_run.txt`, including bounded app list/info/state queries plus last-run receipt persistence on the shared filesystem/storage path.
  - `src/baremetal/tool_service.zig` now extends the typed TCP service seam with `EXEC`, `INSTALL`, `MANIFEST`, `PKGAPP`, `PKGDISPLAY`, `PKGPUT`, `PKGLS`, `PKGGET`, `APPLIST`, `APPINFO`, `APPSTATE`, `APPTRUST`, `APPCONNECTOR`, `APPRUN`, `DISPLAYINFO`, `DISPLAYMODES`, `DISPLAYSET`, `TRUSTPUT`, `TRUSTLIST`, `TRUSTINFO`, and `TRUSTSELECT`, returning structured `exit=... stdout_len=... stderr_len=...` payloads, runtime-layout manifest/install receipts, package-app/package-asset/display payloads, persisted app-lifecycle receipts, or persisted trust-store receipts on top of the freestanding tool/filesystem path.
  ...
  - `src/baremetal_main.zig` now drives the live RTL8139 TCP proof through typed trust-store install/list/info/select exchange, typed package-app/package-display/package-asset exchange with direct filesystem readback, typed `APPLIST` / `APPINFO` / `APPSTATE` / `APPTRUST` / `APPCONNECTOR` / `APPRUN` exchange with persisted `/runtime/apps/<name>/last_run.txt` readback, typed `DISPLAYINFO` / `DISPLAYMODES` / `DISPLAYSET` exchange, selected trust-bundle path readback, live display-mode application during `run-package`, and persisted HTTPS trust-store loading.
  ...
  - full validation after the app-lifecycle/service slice is green:
