...
- FS5.5 service/display/runtime depth slice (current head):
  ...
  - `src/baremetal/package_store.zig` now extends the canonical persisted package layout with `/packages/<name>/assets/...`, manifest `asset_root` / `asset_count` / `asset_bytes` metadata, and persisted asset install/list/get helpers.
  - `src/baremetal/tool_exec.zig` now also exposes `package-ls`, `package-cat`, `display-info`, and `display-modes` on top of the same freestanding filesystem/display surfaces.
  - `src/baremetal/tool_service.zig` now extends the typed TCP service seam with `EXEC`, `INSTALL`, `MANIFEST`, `PKGPUT`, `PKGLS`, `PKGGET`, `DISPLAYINFO`, `DISPLAYMODES`, `TRUSTPUT`, `TRUSTLIST`, `TRUSTINFO`, and `TRUSTSELECT`, returning structured `exit=... stdout_len=... stderr_len=...` payloads, runtime-layout manifest/install receipts, package-asset/display payloads, or persisted trust-store receipts on top of the freestanding tool/filesystem path.
  ...
  - `src/baremetal_main.zig` now drives the live RTL8139 TCP proof through typed trust-store install/list/info/select exchange, typed package-asset upload/list/get exchange with direct filesystem readback, typed `DISPLAYINFO` / `DISPLAYMODES` query exchange, selected trust-bundle path readback, and persisted HTTPS trust-store loading.
