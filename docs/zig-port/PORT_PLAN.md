...
- Post-release hardening:
  ...
  - bare-metal QEMU manual-wait interrupt wrapper validation shipped:
    - new scripts: `scripts/baremetal-qemu-manual-wait-interrupt-baseline-probe-check.ps1`, `scripts/baremetal-qemu-manual-wait-interrupt-after-interrupt-blocked-probe-check.ps1`, `scripts/baremetal-qemu-manual-wait-interrupt-interrupt-telemetry-probe-check.ps1`, `scripts/baremetal-qemu-manual-wait-interrupt-manual-wake-payload-probe-check.ps1`, and `scripts/baremetal-qemu-manual-wait-interrupt-manual-wake-ready-state-probe-check.ps1` compose the existing broad probe into dedicated checks over the same QEMU+GDB lane.
    - matching host regression strengthening in `src/baremetal_main.zig` now asserts the waiting baseline, blocked post-interrupt state, preserved interrupt telemetry, explicit manual-wake payload, and stable final wake queue state after slack ticks.
  ...
  - bare-metal feature-flags/tick-batch wrapper validation shipped:
    - new scripts: `scripts/baremetal-qemu-feature-flags-set-success-probe-check.ps1`, `scripts/baremetal-qemu-tick-batch-valid-update-probe-check.ps1`, `scripts/baremetal-qemu-tick-batch-invalid-preserve-probe-check.ps1`, `scripts/baremetal-qemu-feature-flags-tick-batch-mailbox-state-probe-check.ps1`, and `scripts/baremetal-qemu-feature-flags-tick-batch-state-preserve-probe-check.ps1`.
    - `scripts/baremetal-qemu-feature-flags-tick-batch-probe-check.ps1` now emits explicit stage snapshots for stage-1 flag success, stage-2 valid tick-batch update, and stage-3 invalid-zero rejection so wrappers fail directly on those boundaries instead of only on the final broad receipt.
    - these wrappers isolate five narrow contracts that were previously only implied by the larger live sequence: initial feature-flag success, valid batch transition to `4`, invalid-zero preservation, final mailbox opcode/sequence stability, and final preserved flag/batch/tick accumulation.
  - bare-metal mailbox wrapper validation shipped:
    - new scripts: `scripts/baremetal-qemu-mailbox-invalid-magic-preserve-state-probe-check.ps1`, `scripts/baremetal-qemu-mailbox-invalid-api-version-preserve-state-probe-check.ps1`, `scripts/baremetal-qemu-mailbox-valid-recovery-probe-check.ps1`, `scripts/baremetal-qemu-mailbox-stale-seq-preserve-state-probe-check.ps1`, and `scripts/baremetal-qemu-mailbox-seq-wraparound-recovery-probe-check.ps1`.
    - `scripts/baremetal-qemu-mailbox-header-validation-probe-check.ps1`, `scripts/baremetal-qemu-mailbox-stale-seq-probe-check.ps1`, and `scripts/baremetal-qemu-mailbox-seq-wraparound-probe-check.ps1` now emit explicit stage snapshots so wrappers fail directly on intermediate mailbox-control boundaries instead of inferring from a single final receipt.
    - these wrappers isolate five narrow contracts that were previously only implied by the larger live sequences: invalid `magic` preservation, invalid `api_version` preservation, valid recovery after header rejection, stale-replay no-op preservation, and clean sequence-wraparound recovery.
