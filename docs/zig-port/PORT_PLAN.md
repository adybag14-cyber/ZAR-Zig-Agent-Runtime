...
- Post-release hardening:
  ...
  - bare-metal QEMU interrupt-timeout disable-enable validation shipped:
    ...
    - current proof path validates `ACK=9`, `LAST_OPCODE=46`, `LAST_RESULT=0`, `ARMED_TICKS=7`, `ARMED_WAIT_TIMEOUT=8`, `PRE_WAKE_TICK=12`, `POST_WAKE_TICK=14`, `WAKE0_REASON=1`, `WAKE0_VECTOR=0`, `WAKE0_TICK=12`, `TIMER_PENDING_WAKE_COUNT=1`, and `INTERRUPT_COUNT=0`, proving the timeout arm survives the disabled window and emits exactly one overdue timer wake after re-enable.
  - bare-metal QEMU interrupt-timeout disable-interrupt validation shipped:
    ...
    - current proof path validates `ACK=9`, `LAST_OPCODE=46`, `LAST_RESULT=0`, `TIMER_ENTRY_COUNT=0`, `TIMER_PENDING_WAKE_COUNT=1`, `TIMER_DISPATCH_COUNT=0`, `WAKE0_REASON=2`, `WAKE0_VECTOR=31`, `WAKE0_TICK=7`, `PAUSED_TICK=7`, and `INTERRUPT_COUNT=1`, proving the interrupt wins while timers are disabled and no stale timeout wake leaks after re-enable.
