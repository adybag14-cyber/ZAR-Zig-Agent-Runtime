...
- `FS5.7` i386 CPU-architecture support is now tracked in:
  ...
  - current boundary is explicit: this is boot/build smoke plus broad real i386 storage/NIC/display/service lanes and a bounded platform ACPI/timer/interrupt seam, not full 32-bit driver/runtime parity; remaining gaps are real firmware ACPI under a firmware boot path, SMP, and the next architecture-hardening seams after current device proof breadth
