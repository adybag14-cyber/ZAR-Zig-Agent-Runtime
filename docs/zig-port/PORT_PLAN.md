...
- Post-release hardening:
  ...
  - bare-metal QEMU vector history clear validation shipped:
    ...
    - current proof path validates live pre-clear payloads for interrupt history event `1` (`vector=200`, `is_exception=0`, `code=0`) and event `2` (`vector=13`, `is_exception=1`, `code=51966`) plus exception history event `1` (`vector=13`, `code=51966`), then proves each mailbox clear command zeroes only its corresponding history ring/overflow counters while aggregate counts remain `interrupt=2` and `exception=1`.
  ...
  - bare-metal allocator/syscall reset-depth validation shipped:
    ...
    - current proof path validates hosted `67/67`, live `ACK=16`, `LAST_OPCODE=37`, `LAST_RESULT=0`, pre-reset allocator/syscall counters (`alloc_ops=1`, `free_ops=1`, `peak_bytes=8192`, `dispatch_count=2`, `last_id=7`), and final reset state with allocator/syscall counters and entry records zeroed back to steady baseline.
