...
- `FS5.5` hardware-driver pivot update:
  ...
  - Ethernet driver depth advanced locally:
    ...
    - deeper networking depth remains future work above the FS5.5 closure bar:
      ...
      - broader multi-root / rotation / revocation trust-store lifecycle beyond the current persisted selected-bundle proof on the live HTTPS path
...
- FS5.5 service/display/runtime depth slice (current head):
  ...
  - `src/baremetal/tool_service.zig` now extends the typed TCP service seam with `EXEC`, `INSTALL`, `MANIFEST`, `TRUSTPUT`, `TRUSTLIST`, `TRUSTINFO`, and `TRUSTSELECT`, returning structured `exit=... stdout_len=... stderr_len=...` payloads, runtime-layout manifest/install receipts, or persisted trust-store receipts on top of the freestanding tool/filesystem path.
  ...
  - `src/baremetal_main.zig` now drives the live RTL8139 TCP proof through typed trust-store install/list/info/select exchange, selected trust-bundle path readback, and persisted HTTPS trust-store loading.
