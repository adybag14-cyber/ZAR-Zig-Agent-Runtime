...
- ZigOS integration planning is now tracked in:
  ...
  - seventh delivered adoption slice is bounded `virtio-net` NIC breadth through `src/baremetal/virtio_net.zig`, `src/baremetal/pci.zig`, `src/pal/net.zig`, `scripts/baremetal-qemu-virtio-net-probe-check.ps1`, `scripts/baremetal-qemu-virtio-net-arp-probe-check.ps1`, `scripts/baremetal-qemu-virtio-net-ipv4-probe-check.ps1`, `scripts/baremetal-qemu-virtio-net-udp-probe-check.ps1`, and `scripts/qemu-virtio-net-dgram-echo.ps1`, with host regressions for mock-backed init/MAC/TX/RX/export state plus `ARP` / `IPv4` / `UDP` protocol reuse and live QEMU raw-frame/L2-L4 proof on `virtio-net-pci`
