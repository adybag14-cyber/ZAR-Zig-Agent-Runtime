...
- `FS5.5` hardware-driver pivot update:
  ...
  - live QEMU+GDB proof `scripts/baremetal-qemu-virtio-gpu-display-probe-check.ps1` reads back real `virtio-gpu-pci` EDID/controller capability state, including scanout geometry, physical size, manufacturer/product IDs, EDID bytes, exported output-entry metadata for the selected scanout, explicit `1024x768` retargeting, persisted display-profile save/list/info/apply, and preferred connector/output restore back to `1280x800`.
