...
- thirteenth delivered adoption slice is a bounded TTY/session control layer through `src/baremetal/tty_runtime.zig`, `src/baremetal/tool_exec.zig`, `src/baremetal/tool_service.zig`, `src/baremetal/tool_service/codec.zig`, `src/baremetal/virtual_fs.zig`, and `scripts/baremetal-qemu-e1000-tool-service-probe-check.ps1`, with host regressions for bounded TTY open/send/read/stdout/stderr/close behavior, virtual `/dev/tty` and `/sys/tty` readback, and live `E1000` proof for the same session lifecycle on the clean-room NIC lane
