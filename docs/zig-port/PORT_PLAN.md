...
- `FS5.7` i386 CPU-architecture support is now tracked in:
  ...
  - current boundary is explicit: this is boot/build smoke plus broad real i386 storage/NIC/display/service lanes and a bounded platform ACPI/timer-controller/interrupt/CPU-topology/IOAPIC/LAPIC/AP-startup-control seam, not full 32-bit driver/runtime parity; remaining gaps are real firmware ACPI under a firmware boot path, actual AP bring-up/SMP execution, and the next architecture-hardening seams after current device proof breadth, with the BSP-side warm-reset and startup-IPI path now explicitly proven clean on the direct-loader lane
