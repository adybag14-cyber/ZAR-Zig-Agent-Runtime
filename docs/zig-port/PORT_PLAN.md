...
- FS5.5 service/display/runtime depth slice (current head):
  ...
  - `src/baremetal/package_store.zig` now snapshots and restores canonical package trees under `/packages/<name>/releases/<release>/...`, including the canonical script, package metadata, app metadata, and asset subtree on both RAM-disk and ATA-backed storage.
  - `src/baremetal/tool_exec.zig` now also exposes `package-release-list`, `package-release-save`, and `package-release-activate` on top of that persisted release surface.
  - `src/baremetal/tool_service.zig` now also extends the typed TCP service seam with `PKGRELEASELIST`, `PKGRELEASESAVE`, and `PKGRELEASEACTIVATE`.
  ...
  - `src/baremetal_main.zig` now also drives the live RTL8139 TCP proof through `PKGRELEASESAVE`, canonical package mutation, `PKGRELEASELIST`, `PKGRELEASEACTIVATE`, restored `PKGRUN`, restored `/packages/<name>/bin/main.oc` readback, and restored package asset readback through `PKGGET`.
  - `src/baremetal/filesystem.zig` now carries a `64`-entry filesystem budget so the deeper FS5.5 package/trust/app/autorun runtime state fits on the persisted surface without live-service `NoSpace` failures.
  ...
  - full validation after the package-release/runtime slice is green:
    - `zig build test --summary all` -> `364/364` passed
    ...
    - the real regressions found during the slice were stale long-help expectations in the live RTL8139 proof after widening the builtin surface, stale framed payload-length expectations after the mutated package install path changed, and too-small live help buffers for the deeper typed service seam; the proof contracts, response sizes, and service buffers now match the current FS5.5 package-release surface
