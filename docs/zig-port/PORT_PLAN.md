...
- ZigOS integration planning is now tracked in:
  ...
  - eleventh delivered adoption slice is bounded read-only external-filesystem mounting through `src/baremetal/ext2_ro.zig`, `src/baremetal/fat32_ro.zig`, `src/baremetal/mounted_external_fs.zig`, `src/baremetal/storage_backend.zig`, `src/baremetal/vfs.zig`, `scripts/baremetal-qemu-virtio-block-ext2-mount-probe-check.ps1`, and `scripts/baremetal-qemu-virtio-block-fat32-mount-probe-check.ps1`, with host regressions for backend retargeting plus mounted `/mnt/external` listing/read/stat and live `virtio-blk-pci` proof for bounded read-only `ext2` and `fat32` signature + payload readback
