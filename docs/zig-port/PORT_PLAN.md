...
- `FS5.7` i386 CPU-architecture support is now tracked in:
  ...
  - current boundary is explicit: this is boot/build smoke plus broad real i386 storage/NIC/display/service lanes, a bounded direct-loader platform ACPI/timer-controller/interrupt/CPU-topology/IOAPIC/LAPIC/AP-startup-control seam, and separate real firmware-boot ACPI plus firmware AP-execution proof lanes; remaining gaps are broader SMP bring-up beyond the first AP execution loop and the next architecture-hardening seams after current device proof breadth, with the BSP-side warm-reset and startup-IPI path now explicitly proven clean on the direct-loader lane
