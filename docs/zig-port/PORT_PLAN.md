...
- `FS5.7` i386 CPU-architecture support is now tracked in:
  ...
  - current boundary is explicit: this is boot/build smoke plus broad real i386 storage/NIC/display/service lanes, not full 32-bit driver/runtime parity; remaining gaps are now optional dedicated i386 display-profile/output wrapper coverage beyond the shared virtio-gpu matrix reuse and the next architecture-hardening seams after device proof breadth
