...
- `FS5.5` hardware-driver pivot update:
  ...
  - Ethernet driver depth advanced locally:
    ...
    - deeper networking depth remains future work above the FS5.5 closure bar:
      - higher-level service/runtime layers beyond the current bounded typed batch file/package metadata seam on the bare-metal TCP path
...
- FS5.5 service/display/runtime depth slice (current head):
  ...
  - `src/baremetal/tool_service.zig` now extends the typed TCP service seam with `EXEC`, `INSTALL`, and `MANIFEST`, returning structured `exit=... stdout_len=... stderr_len=...` payloads or runtime-layout manifest/install receipts on top of the freestanding tool/filesystem path.
