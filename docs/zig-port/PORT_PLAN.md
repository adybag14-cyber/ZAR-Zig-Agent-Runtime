...
- `FS5.7` i386 CPU-architecture support is now tracked in:
  ...
  - twelfth delivered slice adds `src/baremetal/i386_ap_startup.zig`, `scripts/baremetal/i386_ap_trampoline.S`, and `scripts/baremetal-qemu-i386-ap-startup-probe-check.ps1`, exports `oc_i386_ap_startup_state_ptr()` plus `/dev/cpu/ap-startup` and `/sys/cpu/ap-startup`, moves the trampoline to a safe high page at `0x00080000`, and proves bounded BSP-side INIT / deassert / SIPI / SIPI sequencing plus exported stage telemetry on the current direct-loader i386 QEMU path without falsely claiming AP execution
  - next ZigOS-derived adoption priorities are now tracked in `docs/zig-port/ZAR_VS_ZIGOS_NEXT_ADOPTION_OPPORTUNITIES.md`
  - current boundary is explicit: this is boot/build smoke plus broad real i386 storage/NIC/display/service lanes and a bounded platform ACPI/timer/interrupt/CPU-topology/LAPIC/AP-startup-control seam, not full 32-bit driver/runtime parity; remaining gaps are real firmware ACPI under a firmware boot path, actual AP bring-up/SMP execution, and the next architecture-hardening seams after current device proof breadth
