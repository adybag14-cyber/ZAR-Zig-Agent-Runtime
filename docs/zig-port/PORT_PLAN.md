...
- FS5.5 service/display/runtime depth slice (current head):
  ...
  - `src/baremetal/tool_exec.zig` now exposes `trust-list`, `trust-info`, `trust-select`, `app-list`, `app-info`, `app-state`, `app-history`, `app-trust`, `app-connector`, and `app-run` builtins on top of that store.
