...
- FS5.5 service/display/runtime depth slice (current head):
  ...
  - `src/pal/net.zig` now carries a real freestanding `https://` POST transport path on top of the RTL8139 + DNS + TCP seam, keeps structured TLS stage/transport diagnostics for failure classification, and explicitly flushes the underlying transport after TLS writer flush so ciphertext is actually emitted on the live path.
  ...
  - `scripts/baremetal-qemu-rtl8139-https-post-probe-check.ps1` plus `scripts/qemu-rtl8139-https-post-server.ps1` now prove the live freestanding HTTPS transport path end to end against a deterministic self-hosted TLS harness, including DNS resolution, TCP connect, TLS handshake, HTTPS POST write, HTTPS response readback, and allocator-owned body buffering.
