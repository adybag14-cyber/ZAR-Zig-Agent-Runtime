...
- `shell-run` now also supports bounded metacharacter escaping for separators/redirection including escaped `<`, bounded stdin redirection through `<`, bounded stdout redirection through `>` / `>>`, bounded stderr redirection through `2>` / `2>>`, and early rejection of malformed quoted arguments onto the existing filesystem surface without adding pipes or job control
...
- host regressions now prove bounded TTY open/write/pending/send/clear/events/read/stdout/stderr/close behavior plus `tty-shell`, per-command `< file` override inside `tty-shell` while later commands still see drained session stdin, shell transcript/event receipts, and `/dev` and `/sys` TTY readback
