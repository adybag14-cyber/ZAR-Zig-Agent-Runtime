...
- host regressions now prove bounded TTY open/write/pending/send/clear/events/read/stdout/stderr/close behavior plus `tty-shell`, per-command `< file` override inside `tty-shell` while later commands still see drained session stdin, direct `tty-send` file-override precedence through the inner bounded parser, bounded quoted-path shell receipts, shell transcript/event receipts, and `/dev` and `/sys` TTY readback
