...
- a real EDID-backed display capability path now exists beyond the rendered BGA console:
  ...
  - `src/baremetal/display_output.zig` provides the exported display-output ABI surface plus EDID byte export, explicit interface-type export, a bounded per-output entry table, and a bounded per-output mode inventory derived from the parsed EDID timings
