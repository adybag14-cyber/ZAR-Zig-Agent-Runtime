...
- deeper networking depth remains future work above the FS5.5 closure bar:
  - higher-level service/runtime layers beyond the current bounded typed batch + `EXEC` / `LIST` / `INSTALL` / `MANIFEST` / `PKGAPP` / `PKGDISPLAY` / file/package/trust/display metadata seam on the bare-metal TCP path
