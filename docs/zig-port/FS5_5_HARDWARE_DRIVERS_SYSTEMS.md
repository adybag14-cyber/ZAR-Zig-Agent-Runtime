...
- direct `runCapture` now bypasses the outer shell-line parser for wrapper commands such as `shell-run`, `tty-send`, and `tty-shell`, preserving embedded script/subcommand redirection semantics on the bounded control surface
...
- bounded TTY/session control plus shell-execution layer inspired by ZigOS TTY concepts without claiming a GP-OS interactive console
...
- host regressions now prove bounded TTY open/write/pending/send/clear/events/read/stdout/stderr/close behavior plus `tty-shell`, per-command `< file` override inside `tty-shell` while later commands still see drained session stdin, direct `tty-send` file-override precedence through the inner bounded parser, escaped-whitespace file overrides on `tty-send` and `tty-shell`, bounded quoted-path shell receipts, direct escaped-space `run-script` script paths plus `mount-bind` targets through the same bounded direct-path rules, shell transcript/event receipts, and `/dev` and `/sys` TTY readback
- `scripts/baremetal-qemu-e1000-tool-service-probe-check.ps1` now proves the same queued-input, bounded shell-execution, escaped-space direct `run-script` / `mount-bind` path handling, and event-receipt slice live on the clean-room `E1000` tool-service path
