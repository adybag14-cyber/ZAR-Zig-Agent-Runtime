...
- those host/module proofs now also cover:
  ...
  - the current local Windows master-Zig stabilization slice now uses the wider `262144`-byte PVH boot/runtime stack in `scripts/baremetal/pvh_boot.S`; hosted master-Zig validation is green (hosted `378/378`, bare-metal host `346 passed / 1 skipped`), and the broad live RTL8139 TCP QEMU proof now passes on both the local `Debug` and stable `ReleaseSafe` probe lanes alongside the live HTTPS proof
