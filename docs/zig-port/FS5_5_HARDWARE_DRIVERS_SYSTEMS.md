...
- deeper networking depth remains future work above the FS5.5 closure bar:
  - higher-level service/runtime layers beyond the current bounded typed batch + `EXEC` / `LIST` / `INSTALL` / `MANIFEST` / `PKGAPP` / `PKGDISPLAY` / `PKGPUT` / `PKGLS` / `PKGGET` / `PKGDELETE` / `APPLIST` / `APPINFO` / `APPSTATE` / `APPHISTORY` / `APPTRUST` / `APPCONNECTOR` / `APPRUN` / `APPDELETE` / file/package/trust/display/app metadata seam on the bare-metal TCP path
