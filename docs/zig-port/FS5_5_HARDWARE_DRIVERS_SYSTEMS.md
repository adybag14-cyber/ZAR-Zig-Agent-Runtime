...
- those host/module proofs now also cover:
  ...
  - the current local Windows master-Zig stabilization slice now keeps the broad live RTL8139 TCP QEMU proof on `ReleaseSafe` for the stable probe lane because the same broad proof still times out under `Debug` on the refreshed local Windows master toolchain; hosted master-Zig validation is green (`706/707`, `1 skipped`), live TCP/HTTPS probes are green, and the remaining `Debug` hang is tracked as toolchain/runtime depth rather than project correctness closure
