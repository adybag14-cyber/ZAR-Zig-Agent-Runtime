...
- bare-metal tool execution is now also on a real freestanding path in `FS5.5`:
  ...
  - `src/baremetal/package_store.zig` provides the canonical persisted package layout under `/packages/<name>/...`, including `/packages/<name>/assets/...`, manifest `script_checksum`, `app_manifest_checksum`, and `asset_tree_checksum` fields, and `/packages/<name>/meta/app.txt` launch/display metadata
