...
- TCP/IP is now also strict-closed for the FS5.5 acceptance bar above that L2 proof:
  ...
  - host/module validation now also proves typed TCP file-service and package-service behavior on top of the bare-metal filesystem, including structured `EXEC` exit/stdout/stderr responses, `PUT`, `GET`, `STAT`, `LIST`, `INSTALL`, `MANIFEST`, `PKG`, `PKGLIST`, `PKGINFO`, `PKGRUN`, `PKGAPP`, `PKGDISPLAY`, `PKGPUT`, `PKGLS`, `PKGGET`, `DISPLAYINFO`, `DISPLAYMODES`, `DISPLAYSET`, `TRUSTPUT`, `TRUSTLIST`, `TRUSTINFO`, `TRUSTACTIVE`, `TRUSTSELECT`, `TRUSTDELETE`, persisted `run-script`, canonical `run-package`, package manifest readback, package app-manifest readback, package display-profile persistence, package asset install/list/get, direct-child directory introspection, display capability/mode export, live display-mode application during `run-package`, explicit `display-set` mode changes, trust-bundle rotation/revocation, and mixed typed batch handling with concatenated framed responses through that service seam
...
- bare-metal tool execution is now also on a real freestanding path in `FS5.5`:
  - `src/baremetal/tool_exec.zig` provides the builtin command substrate used by the bare-metal PAL instead of a hosted-process stub, including persisted `run-script` execution, canonical `run-package`, `package-app`, `package-display`, `package-ls`, `package-cat`, `display-info`, `display-modes`, and `display-set`
