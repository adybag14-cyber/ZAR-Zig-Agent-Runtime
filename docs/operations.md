...
- bare-metal tool execution is now also on a real freestanding path in `FS5.5`:
  - `src/baremetal/tool_exec.zig` provides the builtin command substrate used by the bare-metal PAL instead of a hosted-process stub, including persisted `run-script` execution, canonical `run-package`, `package-app`, `package-display`, `package-ls`, `package-cat`, `package-delete`, `app-list`, `app-info`, `app-state`, `app-trust`, `app-connector`, `app-run`, `app-delete`, `display-info`, `display-modes`, and `display-set`
