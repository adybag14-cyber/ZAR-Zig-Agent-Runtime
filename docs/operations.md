...
- TCP/IP is now also strict-closed for the FS5.5 acceptance bar above that L2 proof:
  ...
  - `scripts/baremetal-qemu-rtl8139-https-post-probe-check.ps1` now proves the freestanding PAL `https://` POST transport path live over RTL8139 with direct-IP transport (`https://10.0.2.2:8443/...`), TCP, TLS handshake, deterministic pinned-certificate trust, fixed probe time, and allocator-owned response buffering against a deterministic self-hosted TLS harness
